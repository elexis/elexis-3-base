...
/**
 * A Slice contains immutable information about a logical shard (all replicas
 * that share the same shard id).
 */
public class Slice extends ZkNodeProps implements Iterable<Replica> {
	public final String collection;

	/**
	 * Loads multiple slices into a Map from a generic Map that probably came from
	 * deserialized JSON.
	 */
	...
	public enum State {
		 ...
		 * <a href=
		 * "https://lucene.apache.org/solr/guide/collections-api.html#splitshard"> the
		 ...
		 * split operation is in progress. It's also used when the shard is undergoing
		 * data restoration. A shard in this state still receives update requests from
		 * the parent shard leader, however does not participate in distributed search.
		 */
		CONSTRUCTION,

		/**
		 * Sub-shards of a split shard are put in that state, when they need to create
		 * replicas in order to meet the collection's replication factor. A shard in
		 * that state still receives update requests from the parent shard leader,
		 * however does not participate in distributed search.
		 */
		RECOVERY,

		/**
		 * Sub-shards of a split shard are put in that state when the split is deemed
		 * failed by the overseer even though all replicas are active because either the
		 * leader node is no longer live or has a different ephemeral owner (zk session
		 * id). Such conditions can potentially lead to data loss. See SOLR-9438 for
		 * details. A shard in that state will neither receive update requests from the
		 * parent shard leader, nor participate in distributed search.
		 */
		RECOVERY_FAILED;
	...
	public static final String LEADER = "leader"; // FUTURE: do we want to record the leader as a slice property in the
													// JSON (as opposed to isLeader as a replica property?)
	 ...
	 * @param replicas The replicas of the slice. This is used directly and a copy
	 *                 is not made. If null, replicas will be constructed from
	 *                 props.
	 * @param props    The properties of the slice - a shallow copy will always be
	 *                 made.
	 */
	...
	public Slice(String name, Map<String, Replica> replicas, Map<String, Object> props, String collection) {
		 ...
		 * null) { System.out.println("###### NO RANGE for " + name + " props=" +
		 * props); }
		 **/
		...
		// add the replicas *after* the other properties (for aesthetics, so it's easy
		// to find slice properties in the JSON output)
