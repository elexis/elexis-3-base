...
public class CollectionProperties {
	...
	public Map<String, String> getCollectionProperties(String collection) throws IOException {
		try {
			return (Map<String, String>) Utils
					.fromJSON(client.getData(ZkStateReader.getCollectionPropsPath(collection), null, new Stat(), true));
		} catch (KeeperException.NoNodeException e) {
			return Collections.emptyMap();
		} catch (KeeperException | InterruptedException e) {
			throw new IOException("Error reading properties for collection " + collection,
		...
		while (true) {
			...
			try {
				if (client.exists(znodePath, true)) {
					Map<String, String> properties = (Map<String, String>) Utils
							.fromJSON(client.getData(znodePath, null, s, true));
					if (propertyValue == null) {
						...
						if (!propertyValue.equals(properties.put(propertyName, propertyValue))) { // Don't update ZK
																									// unless absolutely
																									// necessary.
							client.setData(znodePath, Utils.toJSON(properties), s.getVersion(), true);
						}
				...
				throw new IOException("Error setting property for collection " + collection,
						SolrZkClient.checkInterrupted(e));
			}
			break;
		}
