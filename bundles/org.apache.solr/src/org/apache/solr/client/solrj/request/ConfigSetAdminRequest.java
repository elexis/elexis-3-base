...
public abstract class ConfigSetAdminRequest<Q extends ConfigSetAdminRequest<Q, R>, R extends ConfigSetAdminResponse>
	 ...
	 *
	 * When creating a new configset, the file to be uploaded must be a ZIP file
	 * containing the entire configset being uploaded. When modifing an existing
	 * configset, the file to be uploaded should either be a ZIP file containing the
	 * entire configset being uploaded, or an individual file to upload if
	 * {@link #setFilePath} is being used.
	 */
	public static class Upload extends ConfigSetSpecificAdminRequest<Upload> {
		...
		/**
		 * Optional {@link ConfigSetParams#FILE_PATH} to indicate a single file is being
		 * uploaded into an existing configset
		 */
		...
		/**
		 * A convinience method for specifying an existing File to use as the upload
		 * data.
		 *
		 * This should either be a ZIP file containing the entire configset being
		 * uploaded, or an individual file to upload into an existing configset if
		 * {@link #setFilePath} is being used.
		 *
		 * @see #setUploadStream
		 */
	...
	/**
	 * Creates a new config set by cloning an existing "base" configset. To create a
	 * new configset from scratch using a ZIP file you wish to upload, use the
	 * {@link Upload} command instead
	 */
