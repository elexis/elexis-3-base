...
public class RankStream extends TupleStream implements Expressible {
	...
	public RankStream(StreamExpression expression, StreamFactory factory) throws IOException {
		...
		if (null == nParam || null == nParam.getParameter()
				|| !(nParam.getParameter() instanceof StreamExpressionValue)) {
			throw new IOException(String.format(Locale.ROOT,
					"Invalid expression %s - expecting a single 'n' parameter of type positive integer but didn't find one",
					expression));
		}
		...
		try {
			...
			if (nInt <= 0) {
				throw new IOException(String.format(Locale.ROOT,
						"invalid expression %s - topN '%s' must be greater than 0.", expression, nStr));
			}
		} catch (NumberFormatException e) {
			throw new IOException(String.format(Locale.ROOT,
					"invalid expression %s - topN '%s' is not a valid integer.", expression, nStr));
		}
	...
	private void init(TupleStream tupleStream, int size, StreamComparator comp) throws IOException {
		...
		// Rank stream does not demand that its order is derivable from the order of the
		// incoming stream. No derivation check required
	}
	...
	public Explanation toExplanation(StreamFactory factory) throws IOException {
		...
		return new StreamExplanation(getStreamNodeId().toString())
				...
				.withExpression(toExpression(factory, false).toString()).withHelper(comp.toExplanation(factory));
	}
