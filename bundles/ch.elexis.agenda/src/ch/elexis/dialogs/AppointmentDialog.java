...
public class AppointmentDialog extends Dialog {
	...
	private void sendEmailIfConfirmationChecked() {
		...
		if (detailComposite.getEmailCheckboxStatus() && !alreadySent) {
			contextService.setNamed("ch.elexis.core.mail.image.elexismailappointmentqr",
			...
			contextService.setNamed("ch.elexis.core.mail.image.elexismailappointmentqr", null);
		}
		contextService.getRootContext().setNamed("mail.alreadySent", null);
	}
	...
	private class AppointmentQrSupplier implements Supplier<IImage> {
		...
		public IImage get() {
			...
			try {
				...
				IImage ret = coreModelService.create(IImage.class);
