...
public class ShortestPathStream extends TupleStream implements Expressible {
	...
	public ShortestPathStream(String zkHost, String collection, String fromNode, String toNode, String fromField,
			String toField, SolrParams queryParams, int joinBatchSize, int threads, int maxDepth) {

		init(zkHost, collection, fromNode, toNode, fromField, toField, queryParams, joinBatchSize, threads, maxDepth);
	}
	...
	public ShortestPathStream(StreamExpression expression, StreamFactory factory) throws IOException {
		...
		if (depthExpression == null) {
			throw new IOException(
					String.format(Locale.ROOT, "invalid expression %s - maxDepth param is required", expression));
		} else {
		...
		for (StreamExpressionNamedParameter namedParam : namedParams) {
			if (!namedParam.getName().equals("zkHost") && !namedParam.getName().equals("to")
					&& !namedParam.getName().equals("from") && !namedParam.getName().equals("edge")
					&& !namedParam.getName().equals("maxDepth") && !namedParam.getName().equals("threads")
	...
	private void init(String zkHost, String collection, String fromNode, String toNode, String fromField,
			String toField, SolrParams queryParams, int joinBatchSize, int threads, int maxDepth) {
		this.zkHost = zkHost;
	...
	public void open() throws IOException {
		...
		try {
			...
			threadPool = ExecutorUtil.newMDCAwareFixedThreadPool(threads,
					new SolrNamedThreadFactory("ShortestPathStream"));
	...
	private class JoinRunner implements Callable<List<Edge>> {
		...
		public List<Edge> call() {
			...
			try {
				stream = new UniqueStream(new CloudSolrStream(zkHost, collection, joinParams),
						new MultipleFieldEqualitor(new FieldEqualitor(toField), new FieldEqualitor(fromField)));
	...
	private boolean cycle(String node, List<Map<String, List<String>>> allVisited) {
		// Check all visited trees for each level to see if we've encountered this node
		// before.
