 ...
 * computation across collections using a rollup of metrics from each
 * collection.
 */
public interface ParallelMetricsRollup {

	/**
	 * Given a list of collections, return an array of TupleStream for each
	 * partition.
	 *
	 * @param partitions A list of collections to parallelize metrics computation
	 *                   across.
	 * @return An array of TupleStream for each partition requested.
	 * @throws IOException if an error occurs while constructing the underlying
	 *                     TupleStream for a partition.
	 */
	TupleStream[] parallelize(List<String> partitions) throws IOException;

	/**
	 * Get the rollup for the parallelized streams that is sorted based on the
	 * original (non-parallel) sort order.
	 *
	 * @param plistStream   A parallel list stream to fetch metrics from each
	 *                      partition concurrently
	 * @param rollupMetrics An array of metrics to rollup
	 * @return A rollup over parallelized streams that provide metrics; this is
	 *         typically a SortStream.
	...
	/**
	 * Given a list of partitions (collections), open a select stream that projects
	 * the dimensions and metrics produced by rolling up over a parallelized group
	 * of streams. If it's not possible to rollup the metrics produced by the
	 * underlying metrics stream, this method returns Optional.empty.
	 *
	 * @param context    The current streaming expression context
	 * @param partitions A list of collections to parallelize metrics computation
	 *                   across.
	 * @param metrics    A list of metrics to rollup.
	 * @return Either a TupleStream that performs a rollup over parallelized streams
	 *         or empty if parallelization is not possible.
	 * @throws IOException if an error occurs reading tuples from the parallelized
	 *                     streams
	 */
	default Optional<TupleStream> openParallelStream(StreamContext context, List<String> partitions, Metric[] metrics)
			throws IOException {
		...
		TupleStream parallelStream = getSortedRollupStream(new ParallelListStream(parallelize(partitions)),
				maybeRollupMetrics.get());
	 ...
	 * @return Either an array of metrics that can be parallelized and rolled up or
	 *         empty.
	 */
