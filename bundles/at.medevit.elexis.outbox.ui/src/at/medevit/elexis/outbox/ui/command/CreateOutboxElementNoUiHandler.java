...
public class CreateOutboxElementNoUiHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (StringUtils.isNotEmpty(dburi)) {
			...
			List<IOutboxElement> existingElements = OutboxServiceComponent.get().getOutboxElements(uri, State.NEW);
			if (existingElements.size() >= 1) {
				ret = existingElements.get(0);
			} else {
				ret = OutboxServiceComponent.get().createOutboxElement(patient, mandant, uri);
			}
			if (ret != null && sent) {
