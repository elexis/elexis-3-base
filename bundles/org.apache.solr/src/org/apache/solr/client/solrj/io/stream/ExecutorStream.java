...
/**
 * The executor function wraps a stream with Tuples containing Streaming
 * Expressions and executes them in parallel. Sample syntax:
 *
 * executor(thread=10, topic(storedExpressions, q="*:*", fl="expr_s, id",
 * id="topic1"))
 *
 * The Streaming Expression to execute is taken from the expr field in the
 * Tuples.
 *
 * @since 6.3.0
 */
...
public class ExecutorStream extends TupleStream implements Expressible {
	...
	private StreamExpression toExpression(StreamFactory factory, boolean includeStreams) throws IOException {
		...
		if (includeStreams) {
			if (stream instanceof Expressible) {
				...
				throw new IOException(
						"The ExecuteStream contains a non-expressible TupleStream - it cannot be converted to an expression");
			}
		}
	...
	public void open() throws IOException {
		executorService = ExecutorUtil.newMDCAwareFixedThreadPool(threads,
				new SolrNamedThreadFactory("ExecutorStream"));
		stream.open();
	}
	...
	public static class StreamTask implements Runnable {
		...
		public StreamTask(@SuppressWarnings({ "rawtypes" }) ArrayBlockingQueue queue, StreamFactory streamFactory,
