...
public class LeftOuterJoinStream extends BiJoinStream implements Expressible {
	...
	public Tuple read() throws IOException {
		...
		while (true) {
			...
			// If the right stream is at the EOF, we just return the next element from the
			// left stream
			...
			if (eq.test(leftTupleGroup.get(0), rightTupleGroup.get(0))) {
				...
				if (c < 0) {
					// If there's no match, we still advance the left stream while returning every
					// element.
					// Because it's a left-outer join we still return the left tuple if no match on
					// right.
					return leftTupleGroup.removeFirst();
				} else {
					// advance right
