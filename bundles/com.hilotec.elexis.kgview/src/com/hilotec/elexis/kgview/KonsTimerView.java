	...
	protected void createButtonControl(Composite parent) {
		...
		setBtn.addMouseListener(new MouseListener() {
			public void mouseUp(MouseEvent e) {
				...
				InputDialog id = new InputDialog(getSite().getShell(), "Zeit ändern", "Zeit des Timers ändern",
						timerLbl.getText(), iv);
		...
		resetBtn.addMouseListener(new MouseListener() {
			public void mouseDown(MouseEvent e) {
				if (konsData != null && konsData.getKonsZeit() > 0 && SWTHelper.askYesNo("Timer zurücksetzen",
						"Soll der Konsultationstimer wirklich auf 00:00:00 " + "zurückgesetzt werden?")) {
					resetTimer();
				}
			}
