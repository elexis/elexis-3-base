...
public class CstStateItem extends PersistentObject {
	...
	static final String create = "CREATE TABLE `cstgroups` (" + "	`ID` VARCHAR(25) NOT NULL,"
			...
			+ "	`Date` CHAR(8) NULL DEFAULT '0'," + "	`DateCreated` CHAR(8) NULL DEFAULT '0',"
			+ "	`ParentID` VARCHAR(25) NULL DEFAULT NULL," + "	`ProfileID` VARCHAR(25) NULL DEFAULT NULL,"
			+ "	`ItemType` CHAR(2) NULL DEFAULT NULL," + "	`Name` VARCHAR(256) NULL DEFAULT NULL,"
			+ "	`Description` VARCHAR(256) NULL DEFAULT NULL," + "	`Parameter` BLOB NULL, " + "	PRIMARY KEY (`ID`)"
			+ ")" + " COlLATE='utf8_general_ci' " + " ENGINE=InnoDB;" + " INSERT INTO " + TABLENAME
			+ " (ID, name) VALUES (" + JdbcLink.wrap(VERSIONID) + "," + JdbcLink.wrap(VERSION) + ");";

	static {
		addMapping(TABLENAME, "name=Name", "date=Date", "dateCreated=DateCreated", "description=Description",
				"mandantId=MandantID", "parentId=ParentID", "profileId=ProfileID", "itemType=ItemType",
				"parameter=Parameter");
	...
	public CstStateItem(String date, String name, StateType type, String profileID, String parentID, String mandantID) {
		/*
		 * CstStateItem existing = getByTypeAndProfileAndMandant(name, profileID,
		 * parentID); if (existing != null) { throw new IllegalArgumentException( String
		 * .format("Mapping for origin id [%s] - [%s] already exists can not create multiple instances."
		 * , //$NON-NLS-1$ name, description)); }
		 */
	...
	public CstStateItem(String date, String name, StateType type, String mandantID) {
		/*
		 * CstStateItem existing = getByTypeAndMandant(name, mandantID); if (existing !=
	...
	public boolean delete() {
		int ret = getConnection().exec("DELETE FROM  " + TABLENAME + " WHERE ID =" + getWrappedId());
