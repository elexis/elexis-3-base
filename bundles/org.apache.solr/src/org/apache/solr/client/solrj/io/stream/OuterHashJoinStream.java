...
/**
 * Takes two streams (fullStream and hashStream) and joins them similar to an
 * LeftOuterJoinStream. The difference in a OuterHashJoinStream is that the
 * tuples in the hashStream will all be read and hashed when this stream is
 * opened. This provides a few optimizations iff the hashStream has a relatively
 * small number of documents. The difference between this and a HashJoinStream
 * is that a tuple in the fullStream will be returned even if it doesn't have
 * any matching tuples in the hashStream. You are expected to provide a set of
 * fields for which the hash will be calculated from. If a tuple from the
 * hashStream does not contain a value (ie, null) for one of the fields the hash
 * is being computed on then that tuple will not be considered a match to
 * anything. If a tuple from the fullStream does not contain a value (ie, null)
 * for one of the fields the hash is being computed on then that tuple will be
 * returned without any joined tuples from the hashStream
 *
 * @since 6.0.0
 **/
public class OuterHashJoinStream extends HashJoinStream implements Expressible {
	...
	public StreamExpression toExpression(StreamFactory factory) throws IOException {
		...
		if (hashStream instanceof Expressible && fullStream instanceof Expressible) {
			...
			expression.addParameter(
					new StreamExpressionNamedParameter("hashed", ((Expressible) hashStream).toExpression(factory)));
		} else {
	...
	public Tuple read() throws IOException {
		...
		if (null == workingFullTuple) {
			...
			// If fullTuple doesn't have a valid hash or the hash cannot be found in the
			// hashedTuples then
			// return the tuple from fullStream.
			// This is an outer join so there is no requirement there be a matching value in
			// the hashed stream
		...
		// Due to the check at the end, before returning, we know we have at least one
		// to match with left
