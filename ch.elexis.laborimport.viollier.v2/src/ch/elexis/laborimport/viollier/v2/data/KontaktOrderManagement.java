...
/**
 * Auftragsverwaltungstabelle: Derzeit können Auftragsnummern aus verschiedenen Domänen, welche
 * Auftragsnummern vergeben verwaltet werden (z.B. die unterschiedlichen Laborauftragsnummern des
 * Labors und des Arztes)
 * 
 */
...
public class KontaktOrderManagement extends PersistentObject {
	...
	// Die Kombination von ORDER_NR_DOMAIN und ORDER_NR muss weltweit eindeiutig sein!
	// Als Domain ID werden ausschliesslich GLN oder OID akzeptiert. Andere Einträge werden ohne
	...
	public static final String ORDER_DOMAIN_LAB_ORDER_FILLER_VIOLLIER = "7601002132732"; // Hauptsitz Viollier AG, Allschwil //$NON-NLS-1$
	// ================================================================
	...
	private static final String setVersionSQL =
		"UPDATE " + TABLENAME + " SET " + FLD_VERSION + "='" + VERSION //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			+ "' WHERE ID='VERSION'; "; //$NON-NLS-1$
	
	private static final String index1SQL =
		"CREATE INDEX " + TABLENAME + "_idx_kontakt on " + TABLENAME + "(" + FLD_KONTAKT_ID + ");"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
	private static final String index2SQL =
		"CREATE INDEX " + TABLENAME + "_idx1 on " + TABLENAME + "(" + FLD_KONTAKT_ID + ", " + FLD_ORDER_NR + ", " + FLD_ORDER_NR_DOMAIN + ");"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
	private static final String index3SQL =
		"CREATE UNIQUE INDEX " + TABLENAME + "_idx_order on " + TABLENAME + "(" + FLD_ORDER_NR + ", " + FLD_ORDER_NR_DOMAIN + ");"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
	...
	private static final String createDB = "CREATE TABLE " //$NON-NLS-1$
		...
		+ FLD_ORDER_NR_DOMAIN + " VARCHAR(255)," // Domäne, innerhalb welcher die Auftragsnummer eindeutig ist (z.B. OID oder GLN des Labors oder auch GLN des Mandanten) //$NON-NLS-1$
	...
	/**
	 * Prüft, ob tatsächlich nur bekannte Inhalte in der Tabelle drin sind. Wenn nicht, wird eine
	 * Warnung ausgegeben.
	 */
	private static void checkInvalidEntries(){
		try {
			Query<KontaktOrderManagement> query =
			...
			if (list.size() > 0) {
				SWTHelper.showError("KontaktOrderManagement", "Invalid Database Entries", "Table "
					+ TABLENAME + " contains " + Integer.toString(list.size())
	...
	/**
	 * Prüft, ob schon eine frühere Tabelle existiert. Hinweis: Es gibt eine alte Medics Labor
	 * Importer Version, welche diese Tabelle ins Leben gerufen hat aber leider keinen VERSION
	 * Eintrag schreibt... Deshalb diese etwas "unschöne" Art der Prüfung.
	 * 
	...
	/**
	 * Update-Skript zum Aktualisieren auf Version 2 Bei Version 2 kommen die Spalte ORDER_NR_DOMAIN
	 * und Indexe hinzu Erstellung Update: 3.7.2012 durch medshare GmbH wegen Update Viollier Labor
	 * Importer (ch.elexis.labor.viollier.v2)
	 * 
	 */
	...
	public KontaktOrderManagement(String kontaktID, String orderNrString, String orderNrDomain){
		create(null);
		set(new String[] {
			FLD_KONTAKT_ID, FLD_ORDER_NR, FLD_ORDER_NR_DOMAIN
		}, kontaktID, orderNrString, orderNrDomain);
	}
	...
	public KontaktOrderManagement(Kontakt kontakt, String orderNrString, String orderNrDomain){
		...
		set(new String[] {
			FLD_KONTAKT_ID, FLD_ORDER_NR, FLD_ORDER_NR_DOMAIN
		}, kontakt.getId(), orderNrString, orderNrDomain);
	}
	 ...
	 * 
	 * @param value
	 *            Identifikation der Domäne, welche die Auftragsnummer vergeben hat
	 */
