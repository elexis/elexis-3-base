...
public abstract class DocRouter {
	...
	static {
		...
		// instead of doing back compat this way, we could always convert the
		// clusterstate on first read to "plain" if it doesn't have any properties.
		routerMap.put(null, plain); // back compat with 4.0
		routerMap.put(PlainIdRouter.NAME, plain);
		routerMap.put(CompositeIdRouter.NAME,
		...
		// NOTE: careful that the map keys (the static .NAME members) are filled in by
		// making them final
	}
	...
	public Range fromString(String range) {
		...
		long min = Long.parseLong(minS, 16); // use long to prevent the parsing routines from potentially worrying about
												// overflow
	 ...
	 * @param partitions number of partitions
	 * @param range      range to split
	 * @param fuzz       value between 0 (inclusive) and 0.5 (exclusive) indicating
	 *                   inexact split, i.e. percentage of variation in resulting
	 *                   ranges - odd ranges will be larger and even ranges will be
	 *                   smaller by up to that percentage.
	 */
	...
	/**
	 * Returns the Slice that the document should reside on, or null if there is not
	 * enough information
	 */
	public abstract Slice getTargetSlice(String id, SolrInputDocument sdoc, String route, SolrParams params,
	 ...
	 * accepts a single shard key (or null). If you have a comma separated list of
	 * shard keys, call getSearchSlices
	 **/
	public abstract Collection<Slice> getSearchSlicesSingle(String shardKey, SolrParams params,
			DocCollection collection);

	/**
	 * This method is consulted to determine what search range (the part of the hash
	 * ring) should be queried for a request when an explicit shards parameter was
	 * not used. This method only accepts a single shard key (or null).
	 */
	public Range getSearchRangeSingle(String shardKey, SolrParams params, DocCollection collection) {
		throw new UnsupportedOperationException();
	}

	public abstract boolean isTargetSlice(String id, SolrInputDocument sdoc, SolrParams params, String shardId,
			DocCollection collection);
	...
	/**
	 * This method is consulted to determine what slices should be queried for a
	 * request when an explicit shards parameter was not used. This method accepts a
	 * multi-valued shardKeys parameter (normally comma separated from the
	 * shard.keys request parameter) and aggregates the slices returned by
