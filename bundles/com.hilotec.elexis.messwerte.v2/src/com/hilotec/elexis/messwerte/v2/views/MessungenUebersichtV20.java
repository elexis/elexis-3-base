...
public class MessungenUebersichtV20 extends ViewPart implements ElexisEventListener {
	...
	/**
	 * Ein einzelner Tab fuer einen bestimmten Messungstyp. In diesem wird dann eine
	 * Tabelle mit allen Messungen des aktuell ausgewaehlten Patienten angezeigt.
	 *
	 * @author Antoine Kaufmann
	 *
	 */
	class MessungstypSeite extends Composite {
		...
		public MessungstypSeite(Composite parent, MessungTyp dt) {
			...
			menu.createControlContextMenu(table, editAktion, copyAktion, loeschenAktion, neuAktion, exportAktion);
		}
		...
		exportAktion = new Action(Messages.MessungenUebersicht_action_export) {
			...
			public void run() {
				...
				try {
					...
					String fqfilename = System.getProperty("user.home") + File.separatorChar + filename; //$NON-NLS-1$
					...
					SWTHelper.showInfo(
							MessageFormat.format(Messages.MessungenUebersicht_action_export_title, ci.getText()),
							MessageFormat.format(Messages.MessungenUebersicht_action_export_success, fqfilename));
	 ...
	 *
	 * @param patient Ausgewaehlter Patient oder null falls keiner ausgewaehlt ist.
	 */
