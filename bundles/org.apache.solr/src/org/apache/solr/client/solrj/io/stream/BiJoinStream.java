...
/**
 * Joins leftStream with rightStream based on a Equalitor. Both streams must be
 * sorted by the fields being joined on. Resulting stream is sorted by the
 * equalitor.
 *
 * @since 6.0.0
 **/
...
public abstract class BiJoinStream extends JoinStream implements Expressible {
	...
	// This is used to determine whether we should iterate the left or right side
	...
	private void init() throws IOException {
		...
		// iterationComparator is a combination of the equalitor and the comp from each
		// stream. This can easily be done by
		// grabbing the first N parts of each comp where N is the number of parts in the
		// equalitor. Because we've already
	...
	private StreamComparator createIterationComparator(StreamEqualitor eq, StreamComparator comp) throws IOException {
		if (eq instanceof MultipleFieldEqualitor && comp instanceof MultipleFieldComparator) {
			// we know the comp is at least as long as the eq because we've already
			// validated the tuple order
