...
/**
 * This class provides several singletons of entity resolvers used by SAX and
 * StAX in the Java API. This is needed to make secure XML parsers, that don't
 * resolve external entities from untrusted sources.
 * <p>
 * This class also provides static methods to configure SAX and StAX parsers to
 * be safe.
 * <p>
 * Parsers will get an empty, closed stream for every external entity, so they
 * will not fail while parsing (unless the external entity is needed for
 * processing!).
 */
public final class EmptyEntityResolver {
	...
	/**
	 * Configures the given {@link SAXParserFactory} to do secure XML processing of
	 * untrusted sources. It is required to also set {@link #SAX_INSTANCE} on the
	 * created {@link org.xml.sax.XMLReader}.
	 *
	 * @see #SAX_INSTANCE
	 */
	 ...
	 * No further configuration on is needed, all required entity resolvers are
	 * configured.
	 */
