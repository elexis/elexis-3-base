...
public class AgendaDefinitionPreferencePage extends PreferencePage
	...
	public Control createContents(Composite parent){
		...
		listViewerArea.addSelectionChangedListener(sc -> {
			...
			if (type != null) {
				if (type.startsWith(AreaType.CONTACT.name())) {
					Kontakt contact =
						Kontakt.load(type.substring(AreaType.CONTACT.name().length() + 1));
		...
		comboViewerAreaType.addSelectionChangedListener(sc -> {
			if (AreaType.CONTACT
				.equals(comboViewerAreaType.getStructuredSelection().getFirstElement())) {
			...
			public void mouseDown(MouseEvent e){
				if (AreaType.CONTACT
					...
					KontaktSelektor ks = new KontaktSelektor(UiDesk.getTopShell(), Anwender.class,
						"Bitte Kontakt auswählen", "Selektieren Sie den zugeordneten Kontakt",
						null);
					int retVal = ks.open();
					if (retVal == Dialog.OK) {
						String area =
		...
		listViewerAppointmentTypes =
			new ListViewer(compAppointmentTypes, SWT.BORDER | SWT.V_SCROLL);
		...
		Menu listViewerAppointmentTypesmenu =
			listViewerAppointmentTypesMenuManager.createContextMenu(listAppointmentTypes);
		...
		listViewerAppointmentStatus =
			new ListViewer(compAppointmentStatus, SWT.BORDER | SWT.V_SCROLL);
		...
		Menu listViewerAppointmentStatusmenu =
			listViewerAppointmentStatusMenuManager.createContextMenu(listAppointmentStatus);
