 ...
 *
 * Allows access to all top-level facet properties (e.g. {@code minX},
 * {@code maxY}, etc.) as well as the heatmap data itself in one of two forms.
 */
public class HeatmapJsonFacet {
	...
	/**
	 * A NamedList is a proper "heatmap" response if it contains <i>all</i> expected
	 * properties
	 *
	 * We try to be rather strict in determining whether
	 * {@code potentialHeatmapValues} is a "heatmap". Users can name subfacets
	 * arbitrarily, so having some names match those expected in a "heatmap"
	 * response could just be coincidence.
	 * <p>
	...
	public static boolean isHeatmapFacet(NamedList<Object> potentialHeatmapValues) {
		...
		return potentialHeatmapValues.size() == 8 && hasGridLevel && hasColumns && hasRows && hasMinX && hasMaxX
				&& hasMinY && hasMaxY && hasCountGrid;
	}
}
