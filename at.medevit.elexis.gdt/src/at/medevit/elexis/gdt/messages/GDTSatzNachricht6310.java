...
public class GDTSatzNachricht6310 extends GDTSatzNachricht {
	...
	public GDTSatzNachricht6310(String patientenkennung, 
		String nachnamePatient, 
		String vornamePatient, 
		String geburtsdatumPatient, 
		String namenszusatz,
		String titelPatient, 
		String versichertenNrPatient, 
		String wohnortPatient,
		String strassePatient, 
		String versichertenartPatient, 
		String geschlechtPatient,
		String groessePatient, 
		String gewichtPatient, 
		String muttersprachePatient,
		String guvskf, 
		String tagErhebungBehandlungsdaten, 
		String uhrzeitErhebungBehandlungsdaten,
		String[] aktuelleDiagnose,	// 6205
		String[] befund,			// 6220
		...
		// TODO
		TestIdent[] testIdent, 
		DateiArchivierungsKenner[]	dateiArchivierungsKenner, // 6202, beinhaltet 6203, 6204, 6205
		...
		super(GDTConstants.SATZART_DATEN_EINER_UNTERSUCHUNG_UEBERMITTELN, gdtIdReceiver,
			gdtIdSender, zeichensatz, gdtVersion);
	...
	public static GDTSatzNachricht6310 createfromStringArray(String[] satznachricht){
		...
		for (int i = 0; i < satznachricht.length; i++) {
			...
			if(feldkennung >=6302 && feldkennung<=6305) {
				handleCurrentDateiArchivierungsKenner(dateiArchivierungskenner, currDateiArchivierungsKenner, feldkennung, value);
				continue;
			}
			switch (feldkennung) {
				case GDTConstants.FELDKENNUNG_AKTUELLE_DIAGNOSE: aktDiag.add(value); break;
				case GDTConstants.FELDKENNUNG_BEFUND: befund.add(value); break;
				case GDTConstants.FELDKENNUNG_FREMDBEFUND: fremdBefund.add(value); break;
				case GDTConstants.FELDKENNUNG_KOMMENTAR: kommentar.add(value); break;
				case GDTConstants.FELDKENNUNG_ANZAHL_FOLGEZEILEN: anzahlFortsetzungszeilen6228.add(value); break;
				case GDTConstants.FELDKENNUNG_ERGEBNISTABELLENTEXT_FORMATIERT: ergebnistabellenText.add(value); break;
				default: ds.put(feldkennung, value); break;
			}
		}

		if(currDateiArchivierungsKenner!=null) dateiArchivierungskenner.add(currDateiArchivierungsKenner);
		if(currFreieKategorie!=null) freieKategorien.add(currFreieKategorie);
		if(currTestIdent!=null) testIdents.add(currTestIdent);
		...
		return new GDTSatzNachricht6310(ds.get(GDTConstants.FELDKENNUNG_PATIENT_KENNUNG),
			...
			ds.get(GDTConstants.FELDKENNUNG_TAG_DER_ERHEBUNG_VON_BEHANDLUNGSDATEN), ds.get(GDTConstants.FELDKENNUNG_UHRZEIT_DER_ERHEBUNG_VON_BEHANDLUNGSDATEN),
			aktDiag.toArray(new String[] {}), befund.toArray(new String[] {}), 
			fremdBefund.toArray(new String[] {}), kommentar.toArray(new String[] {}),
			anzahlFortsetzungszeilen6228.toArray(new String[] {}), ergebnistabellenText.toArray(new String[] {}),
			testIdents.toArray(new TestIdent[] {}), dateiArchivierungskenner.toArray(new DateiArchivierungsKenner[] {}), 
			freieKategorien.toArray(new FreieKategorie[] {}), 
			ds.get(GDTConstants.FELDKENNUNG_SIGNATUR), 
	 ...
	 * @param value
	 */
	private static TestIdent handleCurrentTestIdent(LinkedList<TestIdent> testIdents,
		TestIdent currTestIdent, int feldkennung, String value){
		switch (feldkennung) {
	...
	private static void handleCurrentFreieKategorie(LinkedList<FreieKategorie> freieKategorien,
		...
		case GDTConstants.FELDKENNUNG_NAME_DER_FREIEN_KATEGORIE: {
			...
			currFreieKategorie.setName(value); break;
		}
		default: 
			if(currFreieKategorie==null) {
				currFreieKategorie = new FreieKategorie();
	...
	/**
	 * Bearbeite einen Eintrag des Types Datei Archivierungskenner mit seinen Folge-Einträgen (abhängigkeit).
	 * 
	...
	protected void createMessage(){
		...
		if(aktuelleDiagnose!=null) for (int i = 0; i < aktuelleDiagnose.length; i++) addLine(GDTConstants.FELDKENNUNG_AKTUELLE_DIAGNOSE+aktuelleDiagnose[i]);
		if(befund!=null) for (int i = 0; i < befund.length; i++) addLine(GDTConstants.FELDKENNUNG_BEFUND+befund[i]);
		if(fremdBefund!=null) for (int i = 0; i < fremdBefund.length; i++) addLine(GDTConstants.FELDKENNUNG_FREMDBEFUND+fremdBefund[i]);
		if(kommentar!=null) for (int i = 0; i < kommentar.length; i++) addLine(GDTConstants.FELDKENNUNG_KOMMENTAR+kommentar[i]);
		if(anzahlFortsetzungsZeilen6228!=null) for (int i = 0; i < anzahlFortsetzungsZeilen6228.length; i++) addLine(GDTConstants.FELDKENNUNG_ANZAHL_FOLGEZEILEN+anzahlFortsetzungsZeilen6228[i]);
		if(ergebnistabellenText!=null) for (int i = 0; i < ergebnistabellenText.length; i++) addLine(GDTConstants.FELDKENNUNG_ERGEBNIS_TEXT+ergebnistabellenText[i]);
		...
		if(testIdent!=null) {
			for (int i = 0; i < testIdent.length; i++) {
				...
				if(t.getTestBezeichnung()!=null) addLine(GDTConstants.FELDKENNUNG_TESTBEZEICHNUNG+t.getTestBezeichnung());
				if(t.getProbenmaterialIdent()!=null) addLine(GDTConstants.FELDKENNUNG_PROBENMATERIAL_IDENT+t.getProbenmaterialIdent());
				if(t.getProbenmaterialIndex()!=null) addLine(GDTConstants.FELDKENNUNG_PROBENMATERIAL_INDEX+t.getProbenmaterialIndex());
				if(t.getProbenmaterialBezeichnung()!=null) addLine(GDTConstants.FELDKENNUNG_PROBENMATERIAL_BEZEICHNUNG+t.getProbenmaterialBezeichnung());
				...
				if(probenMaterialSpezfikation != null && probenMaterialSpezfikation.size()>0) {
					for (String string : probenMaterialSpezfikation) {
						if(string!=null) addLine(GDTConstants.FELDKENNUNG_PROBENMATERIAL_SPEZIFIKATION+string);
					}
				} 
				
				if(t.getEinheitenFuerDatenstrom()!=null) addLine(GDTConstants.FELDKENNUNG_EINHEIT_FUER_DATENSTROM+t.getEinheitenFuerDatenstrom());
				List<String> datenstrom = t.getDatenStrom();
				if(datenstrom != null && datenstrom.size()>0) {
					for (String string : datenstrom) {
						if(string!=null) addLine(GDTConstants.FELDKENNUNG_DATENSTROM+string);
					}
				}
				
				if(t.getTestStatus()!=null) addLine(GDTConstants.FELDKENNUNG_TESTSTATUS+t.getTestStatus());
				if(t.getErgebnisWert()!=null) addLine(GDTConstants.FELDKENNUNG_ERGEBNIS_WERT+t.getErgebnisWert());
				if(t.getEinheit()!=null) addLine(GDTConstants.FELDKENNUNG_EINHEIT+t.getEinheit());
				if(t.getAbnahmeDatum()!=null) addLine(GDTConstants.FELDKENNUNG_ABNAHME_DATUM+t.getAbnahmeDatum());
				if(t.getAbnahmeZeit()!=null) addLine(GDTConstants.FELDKENNUNG_ABNAHME_ZEIT+t.getAbnahmeZeit());
				if(t.getNormalwertText()!=null) addLine(GDTConstants.FELDKENNUNG_NORMALWERT_TEXT+t.getNormalwertText());
				if(t.getNormalwertUntereGrenze()!=null) addLine(GDTConstants.FELDKENNUNG_NORMALWERT_UNTERE_GRENZE+t.getNormalwertUntereGrenze());
				if(t.getNormalwertObereGrenze()!=null) addLine(GDTConstants.FELDKENNUNG_NORMALWERT_OBERE_GRENZE+t.getNormalwertObereGrenze());
				
				List<String> anmerkungen = t.getAnmerkung();
				if(anmerkungen != null) {
					for (String string : anmerkungen) {
						if(string!=null) addLine(GDTConstants.FELDKENNUNG_TEST_IDENT_ANMERKUNG+string);
					}
				}
				List<String> ergebnisTexte = t.getErgebnisText();
				if(ergebnisTexte != null) {
					for (String string : ergebnisTexte) {
						if(string!=null) addLine(GDTConstants.FELDKENNUNG_ERGEBNIS_TEXT+string);
					}
				}
