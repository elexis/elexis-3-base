...
/**
 * Emits tuples from streamA which also exist in streamB. Resulting tuples are
 * ordered the same as they were in streamA. Both streams must be sorted by the
...
public class IntersectStream extends TupleStream implements Expressible {
	...
	public Explanation toExplanation(StreamFactory factory) throws IOException {
		...
		return new StreamExplanation(getStreamNodeId().toString())
				.withChildren(
						new Explanation[] { streamA.toExplanation(factory), originalStreamB.toExplanation(factory) })
				.withFunctionName(factory.getFunctionName(this.getClass()))
				.withImplementingClass(this.getClass().getName()).withExpressionType(ExpressionType.STREAM_DECORATOR)
				.withExpression(toExpression(factory, false).toString()).withHelper(eq.toExplanation(factory));
	}
	...
	public Tuple read() throws IOException {
		...
		while (true) {
			...
			// We're not at the end and they're not equal. We now need to decide which we
			// can
			// throw away. This is accomplished by checking which is less than the other.
			// The
			...
			// is derived from the sorts of both streamA and streamB we can rest assured
			// that
