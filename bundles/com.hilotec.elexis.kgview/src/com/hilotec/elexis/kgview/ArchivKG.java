...
public class ArchivKG extends ViewPart implements IRefreshable, HeartListener {
	...
	public void createPartControl(Composite parent) {
		...
		menus.createToolbar(actNeueKons, actNeueTelKons, actNeuerHausbesuch, null, actKonsAendern, actAutoAkt,
	...
	public static class NeueKonsAct extends Action {
		...
		public NeueKonsAct(int typ) {
			...
			if (typ == KonsData.KONSTYP_TELEFON) {
				...
				setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin(Activator.PLUGIN_ID, "rsc/phone.png"));
		...
		public void run() {
			...
			if (fall == null || !fall.isOpen() || !fall.getPatient().equals(pat)) {
				MessageDialog.openError(null, "Kein offener Fall ausgewählt",
						"Um eine neue Konsultation erstellen zu können, muss " + "ein offener Fall ausgewählt werden");
				return;
			}
			...
			public void run() {
				...
				try {
					...
					ElexisStatus status = new ElexisStatus(ElexisStatus.WARNING, Hub.PLUGIN_ID, ElexisStatus.CODE_NONE,
							"Error opening part", ex, ElexisStatus.LOG_WARNINGS);
				...
				if (kons == null) {
					SWTHelper.showError("Keine Konsultation aktiv", "Es wird"
							+ "eine aktive Konsultation benötigt um die KG " + "drucken und ablegen zu können.");
					return;
				}
