...
public class MediportOutputter extends ch.elexis.TarmedRechnung.XMLExporter {
	...
	public Result<Rechnung> doOutput(final IRnOutputter.TYPE type, final Collection<Rechnung> rnn,
		...
		try {
			progressService.runInUI(PlatformUI.getWorkbench().getProgressService(), new IRunnableWithProgress() {
				...
				public void run(final IProgressMonitor monitor) {
					monitor.beginTask(Messages.MediportOutputter_info_exportiereRechnungen, rnn.size()); // $NON-NLS-1$
					...
					for (Rechnung rn : rnn) {
						...
						if (outputMethod == OUTPUT_ERROR) {
							...
							String errorMsg = Messages.MediportOutputter_error_NotMediPortParticipant; // $NON-NLS-1$
						...
						if (outputMethod == OUTPUT_MEDIPORT) {
							...
							if (checkTier(rn, param.getDocattr())) {
								if (doExport(rn, billFilenamePath, type, true) == null) {
									res.add(Result.SEVERITY.ERROR, 1, Messages.MediportOutputter_error_Rechnung // $NON-NLS-1$
											+ rn.getNr(), rn, true);
								}
							} else {
								String errorMsg = Messages.MediportOutputter_error_WrongTier; // $NON-NLS-1$
								if (rn.getStatus() == RnStatus.OFFEN) {
									rn.reject(REJECTCODE.INTERNAL_ERROR, errorMsg);
								} else {
									String title = Messages.MediportOutputter_error_Rechnung; // $NON-NLS-1$
			...
			if (alternativeAusgabe != null && alternativeRnn.size() > 0) {
				boolean abort = false;
				log.log("doLicensedOutput: alternativeAusgabe.doOutput alternativeRnn.size()=" + alternativeRnn.size()
						+ "; type=" + type.toString(), Log.DEBUGMSG);
		...
		if (!res.isOK()) {
			ErrorDialog.openError(null, Messages.MediportOutputter_error_Output, // $NON-NLS-1$
	...
	public boolean canStorno(final Rechnung rn) {
		...
		// Nach Rücksprache mit MediData (Jan Gehrig) ist Storno von MediData gar nicht
		// empfohlen.
	...
	private boolean clientParamsOk(ClientParam param) {
		...
		if (props != null) {
			...
			} else if (MediportMainPrefPage.LBL_SERVER_TEST.equals(prefs.getString(MediportMainPrefPage.MPC_SERVER))) {
	...
	private String getRecipientEAN(Rechnung rn) {
		String retVal = TarmedRequirements.getRecipientEAN(
				CoreModelServiceHolder.get().load(getKostentraeger(rn).getId(), IContact.class).orElse(null));
		if (retVal.equals("unknown")) {
			retVal = TarmedRequirements.getEAN(
					CoreModelServiceHolder.get().load(getKostentraeger(rn).getId(), IContact.class).orElse(null));
		}
		return retVal;
	}
	...
	/**
	 * We try to figure out whether we should use Tiers Payant or Tiers Garant. if
	 * unsure, we make it TG
	 */
