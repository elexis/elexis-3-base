...
public class ArtikelstammImporterPage extends ImporterPage {
	...
	public IStatus doImport(IProgressMonitor monitor) throws Exception{
		...
		try {
					...
					public void run(){
						if (MessageDialog.openQuestion(Display.getDefault().getActiveShell(), "",
							"Der Importer ist durch einen anderen Benutzer gestartet.\nDie Artikelstammeinträge werden bereits importiert.\n\n"
								+ "Startzeit: "
								+ new TimeTool(lock.getLockCurrentMillis())
									.toString(TimeTool.LARGE_GER)
			...
			LoggerFactory.getLogger(getClass())
				.info("ArtikelstammImporterPage.doImport " + results[0]);
			IReferenceDataImporter refImporter = getImporter();
			IStatus status =
				refImporter.performImport(monitor, new FileInputStream(results[0]), null);
			if (!status.isOK()) {
	...
	public Composite createPage(Composite parent){
		...
		if (importSetCreationDate != null) {
			sb.append(" / " + ArtikelstammHelper.monthAndYearWritten
				.format(VersionUtil.getImportSetCreationDate()));
		}
	...
	private static void validateLeistungsblockReferences(IProgressMonitor monitor){
		...
		List<ICodeElementBlock> codeElementBlocks =
			CoreModelServiceHolder.get().getQuery(ICodeElementBlock.class).execute();
		monitor.beginTask("Checking Artikelstamm-References in Leistungsblock",
			codeElementBlocks.size());
