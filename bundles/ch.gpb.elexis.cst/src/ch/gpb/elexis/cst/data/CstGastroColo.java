...
public class CstGastroColo extends PersistentObject {
	...
	static final String create = "CREATE TABLE `cstprofile_gastrocolo` " + "(`ID` VARCHAR(25) NOT NULL,	"
			+ "`lastupdate` BIGINT(20) NULL DEFAULT NULL,	" + "`deleted` CHAR(1) NULL DEFAULT '0',	"
			+ "`ProfileID` VARCHAR(25) NULL DEFAULT NULL,	" + "`DatumGastro` CHAR(8) NULL DEFAULT NULL,	"
			+ "`DatumColo` CHAR(8) NULL DEFAULT NULL,	" + "`Text1` TEXT NULL,	" + "`Text2` TEXT NULL,	"
			+ "`Text3` TEXT NULL,	" + "`Text4` TEXT NULL,	" + "`GastroMakroBefund` CHAR(1) NULL DEFAULT '0',	"
			+ "`GastroHistoBefund` CHAR(1) NULL DEFAULT '0',	" + "`ColoMakroBefund` CHAR(1) NULL DEFAULT '0',	"
			+ "`ColoHistoBefund` CHAR(1) NULL DEFAULT '0',	PRIMARY KEY (`ID`)) "
			+ "CoLLATE='utf8_general_ci' ENGINE=InnoDB;"

			+ "INSERT INTO " + TABLENAME + " (ID, text1) VALUES (" + JdbcLink.wrap(VERSIONID) + ","
			+ JdbcLink.wrap(VERSION) + ")";

	static {
		addMapping(TABLENAME, "profileId=ProfileId", "datumColo=DatumColo", "datumGastro=DatumGastro", "text1=Text1",
				"text2=Text2", "text3=Text3", "text4=Text4", "gastroMakroBefund=GastroMakroBefund",
				"gastroHistoBefund=GastroHistoBefund", "coloMakroBefund=ColoMakroBefund",
	...
	public static CstGastroColo getByProfileId(String profileId) {
		...
		if (res.isEmpty()) {
			...
			if (res.size() > 1) {
				throw new IllegalArgumentException(
						String.format("There is already a category of name [%s] - [%s]", profileId));
			}
			return res.get(0);
		}
	}
