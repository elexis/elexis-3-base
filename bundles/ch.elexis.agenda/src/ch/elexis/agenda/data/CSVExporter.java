...
public class CSVExporter {
	...
	public void doExport(String filename, List<Termin> termine) {
		try {
			...
			String[] header = new String[] { "UUID", "Bereich", "Typ", "Datum", "Startzeit", "Dauer", "Grund",
					"Patient-UUID-oder-Name" };
			String[] fields = new String[] { "ID", Termin.FLD_BEREICH, Termin.FLD_TERMINTYP, Termin.FLD_TAG,
			...
			csv.close();
			SWTHelper.showInfo("Termine exportiert", "Der Export nach " + filename + " ist abgeschlossen");
		} catch (Exception ex) {
			ExHandler.handle(ex);
