...
public class InputHandler {
	 ...
	 *
	 * @param input - the input String sent by Mythic22
	...
	/**
	 * parses the input string and puts all defined values into m_inputMap for
	 * further processing
	 *
	 * @param input
	 */
	...
	/**
	 * puts the matrices into the m_inputMap and returns a string, without the
	 * matrices
	 *
	 * @param input
	...
	private String processMatrices(String input) {
		try {
			...
			String lmneMatrix = input.substring(input.indexOf(Mythic22Result.LMNEMATRIX),
					input.indexOf(";T\n", input.indexOf(Mythic22Result.LMNEMATRIX)));
			String lmneShadeMatrix = input.substring(input.indexOf(Mythic22Result.LMNESHADEMATRIX),
			...
			m_inputMap.put(Mythic22Result.LMNESHADEMATRIX, lmneMatrix.substring(lmneMatrix.indexOf('\n') + 1));
			m_inputMap.put(Mythic22Result.THRES5DLMNEMATRIX, lmneMatrix.substring(thres5dLmneMatrix.indexOf('\n') + 1));
	...
	/**
	 *
	 * @param str         - the string to look for occurrences in
	 * @param regex       - the expression to be searched
	 * @param fromIndex   - the index to start searching from
	 * @param occurrences - the index of the occurrence which should be searched
	 *                    (example: 3 looks for the third occurrence)
