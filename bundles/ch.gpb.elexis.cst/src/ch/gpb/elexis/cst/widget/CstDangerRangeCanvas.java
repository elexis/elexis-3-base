...
public class CstDangerRangeCanvas extends CstCanvas {
	...
	void paintControl(PaintEvent e) {
		...
		// der Anzeigebereich wird mit dem h�chsten Wert berechnet, zB wenn
		// RangeEnd gr�sser als der eigentlich Wert ist, nimmt man diesen
		...
		// Wenn bereits das Aufrunden des h�chsten Wertes eine 10er Potenz
		...
		if (dRangeStart > 0) {
			// System.out.println("end x/xLenToFill: " + (xoffset + greenEnd - 20) + " / " +
			// xLenToFill);
			gc.fillGradientRectangle(xoffset + greenStart, yoffset, 40, 10, false);
			// System.out.println("Verlauf links xoff/yoff/xlen: "+ (xoffset +
			// greenStart-20) + " / " + yoffset +" / "+ ((greenEnd - greenStart)+20));
		}
		...
		if (!(alphanumValue.toLowerCase().indexOf("neg") > -1 || alphanumValue.toLowerCase().indexOf("norm") > -1
				|| alphanumValue.toLowerCase().indexOf("pos") > -1 || alphanumValue.toLowerCase().indexOf("-") > -1
				|| alphanumValue.toLowerCase().indexOf("+") > -1)) {
		...
		} else if (alphanumValue.toLowerCase().indexOf("neg") > -1 || alphanumValue.toLowerCase().indexOf("norm") > -1
				|| alphanumValue.toLowerCase().indexOf("-") > -1) {

			/*
			 * if (alphanumValue != null && (alphanumValue.toLowerCase().indexOf("pos") > -1
