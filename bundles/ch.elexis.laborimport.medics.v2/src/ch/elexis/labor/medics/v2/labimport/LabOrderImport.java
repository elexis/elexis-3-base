...
public class LabOrderImport extends ImporterPage {
			...
			protected IVirtualFilesystemHandle[] sortListHandles(IVirtualFilesystemHandle[] iVirtualFilesystemHandles) {
				Arrays.parallelSort(iVirtualFilesystemHandles, new Comparator<IVirtualFilesystemHandle>() {
					...
					public int compare(IVirtualFilesystemHandle left, IVirtualFilesystemHandle right) {
						...
						if (leftParts.length > 1 && StringUtils.isNotBlank(leftParts[1]) && rightParts.length > 1
								&& StringUtils.isNotBlank(rightParts[1])) {
	...
	public IStatus doImport(IProgressMonitor monitor) throws Exception {
		MedicsLogger.getLogger()
				.println(MessageFormat.format("{0}: Medics Laborimport gestartet", df.format(new Date()))); //$NON-NLS-1$
