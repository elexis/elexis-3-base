...
public class ByteUtils {
	 ...
	 * Full characters are read, even if this reads past the length passed (and can
	 * result in an ArrayOutOfBoundsException if invalid UTF8 is passed). Explicit
	 * checks for valid UTF8 are not performed. The char[] out should probably have
	 * enough room to hold the worst case of each byte becoming a Java char.
	 */
	...
	/**
	 * Writes UTF8 into the byte array, starting at offset. The caller should ensure
	...
	/**
	 * Writes UTF8 into the given OutputStream by first writing to the given scratch
	 * array and then writing the contents of the scratch array to the OutputStream.
	 * The given scratch byte array is used to buffer intermediate data before it is
