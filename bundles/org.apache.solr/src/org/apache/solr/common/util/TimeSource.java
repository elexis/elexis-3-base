 ...
 * Source of time.
 * <p>
 * NOTE: depending on implementation returned values may not be related in any
 * way to the current Epoch or calendar time, and they may even be negative -
 * but the API guarantees that they are always monotonically increasing.
 * </p>
 */
public abstract class TimeSource {
	...
	/**
	 * Implementation that uses {@link System#currentTimeMillis()}. This
	 * implementation's {@link #getTimeNs()} returns the same values as
	 * {@link #getEpochTimeNs()}.
	 */
	...
	/**
	 * Implementation that uses {@link System#nanoTime()}. Epoch time is initialized
	 * using {@link CurrentTimeSource}, and then calculated as the elapsed number of
	 * nanoseconds as measured by this implementation.
	 */
	...
	/**
	 * Implementation that uses {@link #NANO_TIME} accelerated by a double
	 * multiplier.
	 */
	public static final class SimTimeSource extends TimeSource {
		...
		/**
		 * Create a simulated time source that runs faster than real time by a
		 * multiplier.
		 *
	 ...
	 *
	 * @param type supported types: <code>currentTime</code>, <code>nanoTime</code>
	 *             and accelerated time with a double factor in the form of
	 *             <code>simTime:FACTOR</code>, eg. <code>simTime:2.5</code>
	...
	/**
	 * Return a time value, in nanosecond units. Depending on implementation this
	 * value may or may not be related to Epoch time.
	 */
	public abstract long getTimeNs();

	/**
	 * Return Epoch time. Implementations that are not natively based on epoch time
	 * may return values that are consistently off by a (small) fixed number of
	 * milliseconds from the actual epoch time.
	 */
	public abstract long getEpochTimeNs();

	/**
	 * Return both the source's time value and the corresponding epoch time value.
	 * This method ensures that epoch time calculations use the same internal value
	 * of time as that reported by {@link #getTimeNs()}.
	 *
	...
	/**
	 * Sleep according to this source's notion of time. Eg. accelerated time source
	 * such as {@link SimTimeSource} will sleep proportionally shorter, according to
	 * its multiplier.
	 *
	...
	/**
	 * This method allows using TimeSource with APIs that require providing just
	 * plain time intervals, eg. {@link Object#wait(long)}. Values returned by this
	 * method are adjusted according to the time source's notion of time - eg.
	 * accelerated time source provided by {@link SimTimeSource} will return
	 * intervals that are proportionally shortened by the multiplier.
	 * <p>
	 * NOTE: converting small values may significantly affect precision of the
	 * returned values due to rounding, especially for accelerated time source, so
	 * care should be taken to use time units that result in relatively large
	 * values. For example, converting a value of 1 expressed in seconds would
	 * result in less precision than converting a value of 1000 expressed in
	 * milliseconds.
	 * </p>
	 *
	 ...
	 * @return converted value, possibly scaled by the source's notion of
	 *         accelerated time (see {@link SimTimeSource})
	 */
