...
public class MoveSzBillingToSzCoverage extends ExternalMaintenance {
	...
	public String executeMaintenance(IProgressMonitor pm, String DBVersion){
		...
		IQuery<IEncounter> encountersQuery =
			CoreModelServiceHolder.get().getQuery(IEncounter.class);
		...
		try (IQueryCursor<IEncounter> cursor = encountersQuery.executeAsCursor()) {
			pm.beginTask(
				"Bitte warten, COVID Test Zertifikat Selbstzahler Verrechnung wird korrigiert ...",
				cursor.size());
	...
	private void move(IEncounter encounter){
		...
		Result<IEncounter> result =
			EncounterServiceHolder.get().transferToCoverage(encounter, privateCoverage, false);
		if (!result.isOK()) {
			System.err.println("Move failed for encounter " + encounter.getLabel() + " ["
				+ result.toString() + "]");
		}
	}
	...
	private boolean canMove(IEncounter encounter){
		...
		if (!editableResult.isOK()) {
			System.err.println("Can not move encounter " + encounter.getLabel() + " ["
				+ editableResult.toString() + "]");
		}
