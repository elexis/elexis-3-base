...
/**
 * The command, which is used in the Lab toolbar to start and stop the listener
 * for incoming mythic22 results
 *
	...
	protected void executeToggle(ExecutionEvent event, boolean checked) {
		...
		if (checked) {
			...
			if (m_netlistener == null)
				m_netlistener = new NetListener(Integer.parseInt(CoreHub.localCfg.get(Preferences.CFG_PORT, "1200")));
			m_netlistener.startContinousRead();

			if (!earlyStartup)
				MessageDialog.openInformation(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),
						Messages.ServerControl_1, Messages.ServerControl_3);
			...
			if (m_netlistener != null) {
				...
				if (!earlyStartup)
					MessageDialog.openInformation(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(),
							Messages.ServerControl_1, Messages.ServerControl_5);
			}
		}
	}
