...
/**
 * Represents a query using the
 * <a href="https://lucene.apache.org/solr/guide/json-request-api.html">JSON
 * Query DSL</a>
 *
 * This class constructs the request using setters for individual properties.
 * For a more monolithic approach to constructing the JSON request, see
 * {@link DirectJsonQueryRequest}
 */
public class JsonQueryRequest extends QueryRequest {
	 ...
	 *
	 * This method may be called multiple times, but each call overwrites the value
	 * specified by previous calls.
	 *
	 * @param query a String in either of two formats: a query string for the
	 *              default deftype (e.g. "title:solr"), or a localparams query
	 *              (e.g. "{!lucene df=text v='solr'}" )
	 *
	 ...
	 * {'lucene': {'df':'genre_s', 'query': 'scifi'}}}". The query subtree of this
	 * request is: "{'lucene': {'df': 'genre_s', 'query': 'scifi'}}". You would
	 * represent this query JSON as follows:
	 *
	 * <pre>
	 * {
	 * 	&#64;code
	 ...
	 * </pre>
	 *
	 * @param queryJson a Map of values representing the query subtree of the JSON
	 ...
	 * </pre>
	 *
	 * @param queryWriter a MapWriter capable of writing out the query subtree of
	 *                    the JSON request you wish to send.
	 ...
	 *
	 * This method may be called multiple times. Each call made with a different
	 * {@code facetName} value will add a new top-level facet. Repeating
	 * {@code facetName} values will cause previous facets with that
	 * {@code facetName} to be overwritten.
	 * <p>
	 * <b>Example:</b> You wish to send the JSON request: {"query": "*:*", "facet":
	 * { "top_cats":{"type": "terms", "field":"cat"}}}. You would represent (and
	 * attach) the facet in this request as follows:
	 *
	 * <pre>
	 * {
	 * 	&#64;code
	 ...
	 * </pre>
	 *
	 * @param facetName the name of the top-level facet you'd like to add. Avoid
	 *                  choosing facet names which overload properties already
	 *                  present in the JSON response schema (e.g. "count", "val",
	 *                  "minX", etc.)
	 * @param facetJson a Map of values representing the facet you wish to add to
	 *                  the request
	 */
	 ...
	 * <p>
	 * <b>Example:</b> You wish to send the JSON request: {"query": "*:*", "facet":
	 * { "top_cats":{"type": "terms", "field":"cat"}}}. You would represent the
	 * facet in this request as follows:
	 *
	 * <pre>
	 ...
	 *                    present in the JSON response schema (e.g. "count", "val",
	 *                    "minX", etc.)
	 * @param facetWriter a MapWriter representing the facet you wish to add to the
	 *                    request
	 */
	...
	/**
	 * Specify a simple stat or aggregation facet to be sent as a part of this JSON
	 * request.
	 *
	 * This method may be called multiple times. Each call made with a different
	 * {@code facetName} value will add a new top-level facet. Repeating
	 ...
	 * <p>
	 * <b>Example:</b> You wish to send the JSON request: {"query": "*:*", "facet":
	 * {"avg_price": "avg(price)"}}. You would represent the facet in this request
	 * as follows:
	 *
	 * <pre>
	 * {@code
	 ...
	 *                   Avoid choosing facet names which overload properties
	 *                   already present in the JSON response schema (e.g. "count",
	 *                   "val", "minX", etc.)
	 * @param facetValue a String representing the stat/agg facet computation to
	 *                   perform.
	 */
	...
	/**
	 * Specify whether results should be fetched starting from a particular offset
	 * (or 'start').
	 *
	 * Defaults to 0 if not set.
	 *
	 * @param offset a non-negative integer representing the offset (or 'start') to
	 *               use when returning results
	 *
	 ...
	 *
	 * @param limit a non-negative integer representing the maximum results to
	...
	/**
	 * Specify how results to the JSON request should be sorted before being
	 * returned by Solr
	 *
	 * @param sort a string representing the desired result sort order (e.g. "price
	 *             asc")
	 *
	 ...
	 *
	 * This method may be called multiple times; each call will add a new filter to
	 * the request
	 *
	 * @param filterQuery a String in either of two formats: a query string for the
	 *                    default deftype (e.g. "title:solr"), or a localparams
	 *                    query (e.g. "{!lucene df=text v='solr'}" )
	 ...
	 * the request
	 * <p>
	 * <b>Example:</b> You wish to send the JSON request: "{'query':'*:*', 'filter':
	 * [{'lucene': {'df':'genre_s', 'query': 'scifi'}}]}". The filter you want to
	 * add is: "{'lucene': {'df': 'genre_s', 'query': 'scifi'}}". You would
	 * represent this filter query as follows:
	 *
	 * <pre>
	 * {
	 * 	&#64;code
	 ...
	 * </pre>
	 *
	 * @param filterQuery a Map of values representing the filter request you wish
	 *                    to send.
	 * @throws IllegalArgumentException if {@code filterQuery} is null
	 */
	@SuppressWarnings({ "unchecked" })
	 ...
	 *
	 * This method may be called multiple times; each call will add a new field to
	 * the list of those to be returned.
	 *
	 ...
	 *
	 * The JSON query DSL has special support for a few query parameters
	 * (limit/rows, offset/start, filter/fq, etc.). But many other query parameters
	 * are not explicitly covered by the query DSL. This method can be used to add
	 * any of these other parameters to the JSON request.
	 * <p>
	 * This method may be called multiple times; each call with a different
	 * {@code name} will add a new param name/value to the params subtree.
	 * Invocations that repeat a {@code name} will overwrite the previously
	 * specified parameter values associated with that name.
	 *
	 * @param name  the name of the parameter to add
	 * @param value the value of the parameter to add. Usually a String, Number
	 *              (Integer, Long, Double), or Boolean.
	 *
	 * @throws IllegalArgumentException if either {@code name} or {@code value} are
	 *                                  null
	 */
	@SuppressWarnings({ "unchecked" })
	public JsonQueryRequest withParam(String name, Object value) {
		...
		((Map<String, Object>) jsonRequestMap.computeIfAbsent("params", s -> new HashMap<String, Object>())).put(name,
				value);
		return this;
	}
