...
public abstract class AbstractHaematologyData implements IProbe {
	...
	public void write(Patient patient) throws PackageException{
		...
		wbcVal.fetchValue(patient, wbc, "", getDate()); //$NON-NLS-1$
		
		Value rbcVal = getValue("RBC"); //$NON-NLS-1$
		rbcVal.fetchValue(patient, rbc, "", getDate()); //$NON-NLS-1$
		
		Value hgbVal = getValue("HGB"); //$NON-NLS-1$
		hgbVal.fetchValue(patient, hgb, "", getDate()); //$NON-NLS-1$
		
		Value hctVal = getValue("HCT"); //$NON-NLS-1$
		hctVal.fetchValue(patient, hct, "", getDate()); //$NON-NLS-1$
		
		Value mcvVal = getValue("MCV"); //$NON-NLS-1$
		mcvVal.fetchValue(patient, mcv, "", getDate()); //$NON-NLS-1$
		
		Value mchVal = getValue("MCH"); //$NON-NLS-1$
		mchVal.fetchValue(patient, mch, "", getDate()); //$NON-NLS-1$
		
		Value mchcVal = getValue("MCHC"); //$NON-NLS-1$
		mchcVal.fetchValue(patient, mchc, "", getDate()); //$NON-NLS-1$
		
		Value pltVal = getValue("PLT"); //$NON-NLS-1$
		pltVal.fetchValue(patient, plt, "", getDate()); //$NON-NLS-1$
		
		Value lymPercentVal = getValue("LYM%"); //$NON-NLS-1$
		lymPercentVal.fetchValue(patient, lym_percent, "", getDate()); //$NON-NLS-1$
		
		Value mxdPercentVal = getValue("MXD%"); //$NON-NLS-1$
		mxdPercentVal.fetchValue(patient, mxd_percent, "", getDate()); //$NON-NLS-1$
		
		Value neutPercentVal = getValue("NEUT%"); //$NON-NLS-1$
		neutPercentVal.fetchValue(patient, neut_percent, "", getDate()); //$NON-NLS-1$
		
		Value lymVolumeVal = getValue("LYM#"); //$NON-NLS-1$
		lymVolumeVal.fetchValue(patient, lym_volume, "", getDate()); //$NON-NLS-1$
		
		Value mxdVolumeVal = getValue("MXD#"); //$NON-NLS-1$
		mxdVolumeVal.fetchValue(patient, mxd_volume, "", getDate()); //$NON-NLS-1$
		
		Value neutVolumeVal = getValue("NEUT#"); //$NON-NLS-1$
		neutVolumeVal.fetchValue(patient, neut_volume, "", getDate()); //$NON-NLS-1$
		
		if (rdw_sd != null) {
			Value rdwSdVal = getValue("RDW-SD"); //$NON-NLS-1$
			rdwSdVal.fetchValue(patient, rdw_sd, "", getDate()); //$NON-NLS-1$
		}
		
		if (rdw_cv != null) {
			Value rdwCvVal = getValue("RDW-CV"); //$NON-NLS-1$
			rdwCvVal.fetchValue(patient, rdw_cv, "", getDate()); //$NON-NLS-1$
		}
		
		Value pdwVal = getValue("PDW"); //$NON-NLS-1$
		pdwVal.fetchValue(patient, pdw, "", getDate()); //$NON-NLS-1$
		
		Value mpvVal = getValue("MPV"); //$NON-NLS-1$
		mpvVal.fetchValue(patient, mpv, "", getDate()); //$NON-NLS-1$
		
		Value pLcrVal = getValue("P-LCR"); //$NON-NLS-1$
		pLcrVal.fetchValue(patient, p_lcr, "", getDate()); //$NON-NLS-1$
	}
