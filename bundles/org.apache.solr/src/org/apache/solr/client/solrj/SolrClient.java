...
public abstract class SolrClient implements Serializable, Closeable {
	 ...
	 * Adds a single document
	 *
	 * Many {@link SolrClient} implementations have drastically slower indexing
	 * performance when documents are added individually. Document batching
	 * generally leads to better indexing performance and should be used whenever
	 * possible.
	 *
	 * @param doc the input document
	 *
	 ...
	 * indexing
	 *
	 * waitFlush=true and waitSearcher=true to be inline with the defaults for plain
	 * HTTP access
	 * <p>
	 * Be very careful when triggering commits from the client side. Commits are
	 * heavy operations and WILL impact Solr performance when executed too often or
	 * too close together. Instead, consider using 'commitWithin' when adding
	 * documents or rely on your core's/collection's 'autoCommit' settings.
	 *
	...
	public UpdateResponse commit(String collection, boolean waitFlush, boolean waitSearcher, boolean softCommit)
			throws SolrServerException, IOException {
		return new UpdateRequest().setAction(UpdateRequest.ACTION.COMMIT, waitFlush, waitSearcher, softCommit)
	...
	public UpdateResponse optimize(String collection, boolean waitFlush, boolean waitSearcher, int maxSegments)
			throws SolrServerException, IOException {
		return new UpdateRequest().setAction(UpdateRequest.ACTION.OPTIMIZE, waitFlush, waitSearcher, maxSegments)
				.process(this, collection);
	}
	 ...
	 * <p>
	 * Also note that rollbacks reset changes made by <i>all</i> clients. Use this
	 * method carefully when multiple clients, or multithreaded clients are in use.
	 *
	 ...
	 * @param ids        the list of document IDs to delete; must be non-null and
	 *                   contain elements
	 *
	 * @return an {@link org.apache.solr.client.solrj.response.UpdateResponse}
	 *         containing the response from the server
	 ...
	 * Deletes a list of documents by unique ID. Doesn't work for child/nested docs.
	 *
	 * @param ids the list of document IDs to delete; must be non-null and contain
	 *            elements
	 *
	 * @return an {@link org.apache.solr.client.solrj.response.UpdateResponse}
	 *         containing the response from the server
	 ...
	 *
	 * @param collection     the Solr collection to delete the documents from
	 * @param ids            the list of document IDs to delete; must be non-null
	...
	/**
	 * Deletes documents from the index based on a query, specifying max time before
	 * commit
	 *
	 * @param collection     the Solr collection to delete the documents from
	 * @param query          the query expressing what documents to delete
	 ...
	 *
	 * @return a {@link org.apache.solr.client.solrj.response.SolrPingResponse}
	 *         containing the response from the server
	 *
	 * @throws IOException         If there is a low-level I/O error.
	 * @throws SolrServerException if there is an error on the server
	 ...
	 *                   the request
	 *
	 * @return a {@link org.apache.solr.client.solrj.response.QueryResponse}
	 *         containing the response from the server
	 *
	 * @throws IOException         If there is a low-level I/O error.
	 * @throws SolrServerException if there is an error on the server
	...
	/**
	 * Query solr, and stream the results. Unlike the standard query, this will send
	 * events for each Document rather then add them to the QueryResponse.
	 *
	 * Although this function returns a 'QueryResponse' it should be used with care
	 * since it excludes anything that was passed to callback. Also note that future
	 * version may pass even more info to the callback and may not return the
	 * results in the QueryResponse.
	 *
	 * @param collection the Solr collection to query
	 * @param params     an object holding all key/value parameters to send along
	 *                   the request
	...
	/**
	 * Retrieves the SolrDocument associated with the given identifier and uses the
	 * SolrParams to execute the request.
	 *
	 * @param collection the Solr collection to query
	 * @param id         the id
	 ...
	 * @throws SolrServerException if there is an error on the server
	 */
	public abstract NamedList<Object> request(@SuppressWarnings({ "rawtypes" }) final SolrRequest request,
	 ...
	 * @throws SolrServerException if there is an error on the server
	 */
	public final NamedList<Object> request(@SuppressWarnings({ "rawtypes" }) final SolrRequest request)
			throws SolrServerException, IOException {
		return request(request, null);
	}

	/**
	 * Get the {@link org.apache.solr.client.solrj.beans.DocumentObjectBinder} for
	 * this client.
	 *
