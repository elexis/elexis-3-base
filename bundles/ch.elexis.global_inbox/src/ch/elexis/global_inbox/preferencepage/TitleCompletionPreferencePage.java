...
public class TitleCompletionPreferencePage extends PreferencePage
	...
	protected Control createContents(Composite parent){
		...
		popupMenu.addMenuListener(new MenuAdapter() {
			...
			public void menuShown(MenuEvent e){
				popupMenu.getItem(1)
					.setEnabled(tableViewer.getStructuredSelection().getFirstElement() != null);
			}
		});
		...
		mntmDelete.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e){
				TitleEntry selection =
					(TitleEntry) tableViewer.getStructuredSelection().getFirstElement();
				if (selection != null) {
	...
	protected void performApply(){
		java.util.List<String> completions = storedCompletions.stream()
			.map(le -> le.getTitle() + STORE_SEPARATOR + le.getCategoryName())
