...
public class MedikarteView extends ViewPart implements IRefreshable {
	...
	public void createPartControl(Composite parent) {
		...
		String[] ueberschriften = { "Ordnungszahl", "Medikament", "Mo", "Mi", "Ab", "Na", "Einh", "Zweck",
	...
	private void makeActions() {
		...
		actDelete = new Action("Löschen", Action.AS_PUSH_BUTTON) {
			...
			public void run() {
				...
				if (!SWTHelper.askYesNo("Verschreibung loeschen",
						"Soll der " + "markierte Eintrag wirklich permanent gelöscht werden?"))
					return;
				presc.remove();
				refresh();
			}
		...
		actDrucken = new Action("Drucken", Action.AS_PUSH_BUTTON) {
			...
			public void run() {
				...
				if (patient == null || kons == null) {
					SWTHelper.alert("Keine Konsultation ausgewählt", "Eine Konsutlation muss ausgewählt sein in der "
				...
				Brief doc = tv.getTextContainer().createFromTemplateName(kons, TT_MEDICARD, Brief.UNKNOWN, patient,
						"Medikamentenkarte");
				tv.openDocument(doc);
			}
		};
