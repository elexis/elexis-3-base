...
public class SignificantTermsStream extends TupleStream implements Expressible {
	...
	public SignificantTermsStream(String zkHost, String collectionName, @SuppressWarnings({ "rawtypes" }) Map params,
			String field, float minDocFreq, float maxDocFreq, int minTermLength, int numTerms) throws IOException {
	...
	private void init(String collectionName, String zkHost, @SuppressWarnings({ "rawtypes" }) Map params, String field,
			float minDocFreq, float maxDocFreq, int minTermLength, int numTerms) throws IOException {
		this.zkHost = zkHost;
	...
	public void open() throws IOException {
		...
		this.executorService = ExecutorUtil
				.newMDCAwareCachedThreadPool(new SolrNamedThreadFactory("SignificantTermsStream"));
	}
	...
	private List<Future<NamedList>> callShards(List<String> baseUrls) throws IOException {
		...
		for (String baseUrl : baseUrls) {
			SignificantTermsCall lc = new SignificantTermsCall(baseUrl, this.params, this.field, this.minDocFreq,
					this.maxDocFreq, this.minTermLength, this.numTerms);
	...
	public Tuple read() throws IOException {
		try {
			if (tupleIterator == null) {
				...
				Future<NamedList> getTopTermsCall : callShards(getShards(zkHost, collection, streamContext))) {
					@SuppressWarnings({ "rawtypes" })
				...
				for (Map.Entry<String, int[]> entry : mergeFreqs.entrySet()) {
					...
					float score = (float) (Math.log(freqs[1]) + 1.0)
	...
	protected class SignificantTermsCall implements Callable<NamedList> {
		...
		public SignificantTermsCall(String baseUrl, Map<String, String> paramsMap, String field, float minDocFreq,
				float maxDocFreq, int minTermLength, int numTerms) {
