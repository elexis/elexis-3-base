...
public abstract class ContentStreamBase implements ContentStream {
	...
	private static final List<String> UNHELPFUL_TYPES = Arrays
			.asList(ContentType.APPLICATION_OCTET_STREAM.getMimeType(), "application/gzip", "content/unknown");
	...
	public static class StringStream extends ContentStreamBase {
		...
		public static String detect(String str) {
			...
			for (int i = 0; i < lim; i++) {
				...
				if (ch == '#' // single line comment
						|| (ch == '/' && (str.charAt(i + 1) == '/' || str.charAt(i + 1) == '*')) // single line or
																									// multi-line
																									// comment
		...
		/**
		 * If an charset is defined (by the contentType) use that, otherwise use a
		 * StringReader
		 */
		@Override
	...
	/**
	 * Base reader implementation. If the contentType declares a charset use it,
	 * otherwise use "utf-8".
	 */
	@Override
	public Reader getReader() throws IOException {
		String charset = getCharsetFromContentType(getContentType());
		return charset == null ? new InputStreamReader(getStream(), DEFAULT_CHARSET)
	...
	public static ContentStream create(RequestWriter requestWriter, @SuppressWarnings({ "rawtypes" }) SolrRequest req)
			throws IOException {
