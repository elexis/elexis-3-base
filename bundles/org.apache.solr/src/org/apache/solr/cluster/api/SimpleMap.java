 ...
 * datasets without consuming lot of memory The objective is to provide
 * implementations that are cheap and memory efficient to implement and consume.
 * The keys are always {@link CharSequence} objects, The values can be of any
 * type
 */
public interface SimpleMap<T> extends MapWriter {
	 ...
	 * implementations must do it more efficiently
	 */
	...
	/**
	 * iterate through all keys but abort in between if required The default impl is
	 * suboptimal. Proper implementations must do it more efficiently
	 *
	 * @param fun Consume each key and return a boolean to signal whether to proceed
	 *            or not. If true , continue. If false stop
	 */
	...
	/**
	 * Navigate through all key-values but abort in between if required. The default
	 * impl is suboptimal. Proper implementations must do it more efficiently
	 *
	 * @param fun Consume each entry and return a boolean to signal whether to
	 *            proceed or not. If true, continue, if false stop
	 */
