...
public class AbacusJuniorAction extends Action implements ComPortListener {
	...
	public AbacusJuniorAction() {
		...
		setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin("ch.medshare.connect.abacusjunior",
				"icons/abacusjunior.ico"));

		_ctrl = new Connection(Messages.AbacusJuniorAction_ConnectionName,
				CoreHub.localCfg.get(Preferences.PORT, Messages.AbacusJuniorAction_DefaultPort),
				CoreHub.localCfg.get(Preferences.PARAMS, Messages.AbacusJuniorAction_DefaultParams), this);

		if (CoreHub.localCfg.get(Preferences.LOG, "n").equalsIgnoreCase("y")) {
			try {
				_log = new Logger(System.getProperty("user.home") + File.separator + "elexis" + File.separator
						+ "abacusjunior.log");
	...
	public void run() {
		if (isChecked()) {
			KontaktSelektor ksl = new KontaktSelektor(Hub.getActiveShell(), Patient.class,
					Messages.AbacusJuniorAction_Patient_Title, Messages.AbacusJuniorAction_Patient_Text,
