...
public class FastJavaBinDecoder implements DataEntry.FastDecoder {
	...
	public enum Tag {
		_NULL(NULL, LOWER_5_BITS, DataEntry.Type.NULL), _BOOL_TRUE(BOOL_TRUE, LOWER_5_BITS, DataEntry.Type.BOOL) {
			@Override
		...
		/**
		 * This applies to only container Objects. This is invoked only if there is a
		...
		/**
		 * This should read the minimal data about the entry . if the data is a
		 * primitive type , read the whole thing
		 */
		public void lazyRead(EntryImpl entry, StreamCodec codec) throws IOException {
		 ...
		 * {@link JavaBinCodec#readObject(DataInputInputStream)}
		 */
		public Object readObject(StreamCodec codec, EntryImpl entry) throws IOException {
	...
	private static void addObj(DataEntry e) {
		if (e.type().isContainer) {
			Object ctx = e.type() == DataEntry.Type.KEYVAL_ITER ? new LinkedHashMap(getSize(e))
