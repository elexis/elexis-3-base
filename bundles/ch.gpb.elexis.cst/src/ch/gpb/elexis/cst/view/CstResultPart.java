 ...
 * 
 * this is the super class for the custom result views. 
 * child classes must implement the layout method (createPartControl)
 * 
 * 595 / 842
 * 794 / 1123
 */
...
public abstract class CstResultPart extends ViewPart implements IActivationListener {
    ...
    /**
     * This is a default implemetation of an ElexisEventListener. It listens for SelectionEVents and UnselectionEvents
     * on Instances of the Patient class.
     */
    ...
    /**
     * On disposal,the IActivationListener MUST be unregistered. Also, our RestrictedAction must be unregistered from
     * the AutoAdapt queue to prevent memory leaks.
     */
    @Override
    public void dispose() {
	...
	*/
    ...
    /**
     * As the Elexis User changes, he or she has propably different righs to see and modify our data, than the previous
     * user. So we catch the USER_CHANGED Event and react accordingly.
     */
	...
	// but now, this needs some work because the width isn't correct anymore in PDF output.
	...
	String title = Messages.CstProfileEditor_Datum + ": "
		+ CstService.getReadableDateAndTime() + "    ("
    ...
    /**
     * Check the length of the composite and add a Label with a vertical indent
     * that matches the remaining heigth.
     * @param comp
     */
	...
	if (cstProimmun != null) {
	    ...
	    String sLabeltxt = Messages.Cst_Text_Proimmun + "\r\n" + Messages.Cst_Text_IggAntikoerper + "\r\n"
		    ...
		    + cstProimmun.getToBeTested() + " " + Messages.Cst_Text_getesteten
	...
	for (String befundFeld : befundFelder) {
		...
		if (separator != null) {
		    ...
		    String[] splits = befundFeld.split(Messwert.SETUP_CHECKSEPARATOR);
		    ValueSingleTimelineCanvas bdGewicht = new ValueSingleTimelineCanvas(compoValueSingle,
			    SWT.FILL, befundParameter + " (" + splits[0] + ")", splits[0]);
	...
	actionScreenshot.setImageDescriptor(PlatformUI.getWorkbench().getSharedImages()
    ...
    /**
     * Holt die Werte der Befunde (so wie in den Anzeigeoptionen ausgew�hlt) f�r Einzelwerte
     * @param pat
     * @param parm
     * @param fieldName
     * @param separator
     ...
     */
    List<ValuePairTimeline> getValuesForValuePairTimeline(
	    final Patient pat,
	    final String parm,
    ...
    /**
     * Holt die Werte der Befunde (so wie in den Anzeigeoptionen ausgew�hlt) f�r Wertepaaare
     * 
     * @param pat
     * @param parm
     * @param fieldName
     ...
     */
    List<ValueSingleTimeline> getValuesForSingleValueTimeline(
	    final Patient pat,
	    final String parm,
	    final String fieldName) {
    ...
    /**
     * From IActivationListener: The View changes visibility Our listeners need only to be active,if gthe view is
     * visible. So we untregister them if the view disappears to save resources. If the view becomes visible again, we
     * must send an event, because the listeners don't know what happend, before the start listening
     * 
     * @param mode
	...
	if (profile == null) {
	    ...
	    System.out.println("Profile is: " + profile.getName());
