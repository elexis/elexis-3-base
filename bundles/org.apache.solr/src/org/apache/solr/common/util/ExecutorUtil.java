...
public class ExecutorUtil {
	...
	public synchronized static void addThreadLocalProvider(InheritableThreadLocalProvider provider) {
		for (InheritableThreadLocalProvider p : providers) {// this is to avoid accidental multiple addition of
															// providers in tests
			if (p.getClass().equals(provider.getClass()))
				return;
		}
	...
	/**
	 * Any class which wants to carry forward the threadlocal values to the threads
	 * run by threadpools must implement this interface and the implementation
	...
	public interface InheritableThreadLocalProvider {
		...
		/**
		 * This is invoked in the Threadpool thread. set the appropriate values in the
		 * threadlocal of this thread.
		 */
		public void set(AtomicReference<?> ctx);

		/**
		 * This method is invoked in the threadpool thread after the execution clean all
		 * the variables set in the set method
		 */
	...
	public static ExecutorService newMDCAwareFixedThreadPool(int nThreads, ThreadFactory threadFactory) {
		return new MDCAwareThreadPoolExecutor(nThreads, nThreads, 0L, TimeUnit.MILLISECONDS,
	...
	public static ExecutorService newMDCAwareSingleThreadExecutor(ThreadFactory threadFactory) {
		return new MDCAwareThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<>(),
				threadFactory);
	}
	...
	public static ExecutorService newMDCAwareCachedThreadPool(ThreadFactory threadFactory) {
		return new MDCAwareThreadPoolExecutor(0, Integer.MAX_VALUE, 60L, TimeUnit.SECONDS, new SynchronousQueue<>(),
				threadFactory);
	}

	public static ExecutorService newMDCAwareCachedThreadPool(int maxThreads, ThreadFactory threadFactory) {
		return new MDCAwareThreadPoolExecutor(0, maxThreads, 60L, TimeUnit.SECONDS,
				new LinkedBlockingQueue<>(maxThreads), threadFactory);
	}
	...
	public static class MDCAwareThreadPoolExecutor extends ThreadPoolExecutor {
		...
		public MDCAwareThreadPoolExecutor(int corePoolSize, int maximumPoolSize, long keepAliveTime, TimeUnit unit,
				BlockingQueue<Runnable> workQueue, ThreadFactory threadFactory, boolean enableSubmitterStackTrace) {
		...
		public void execute(final Runnable command) {
			...
			final String submitterContextStr = ctxStr.length() <= MAX_THREAD_NAME_LEN ? ctxStr
					: ctxStr.substring(0, MAX_THREAD_NAME_LEN);
			final Exception submitterStackTrace = enableSubmitterStackTrace ? new Exception("Submitter stack trace")
					: null;
			final List<InheritableThreadLocalProvider> providersCopy = providers;
			@SuppressWarnings({ "rawtypes" })
			final ArrayList<AtomicReference> ctx = providersCopy.isEmpty() ? null
					: new ArrayList<>(providersCopy.size());
			if (ctx != null) {
			...
			super.execute(() -> {
				...
				if (ctx != null) {
					for (int i = 0; i < providersCopy.size(); i++)
						providersCopy.get(i).set(ctx.get(i));
				}
