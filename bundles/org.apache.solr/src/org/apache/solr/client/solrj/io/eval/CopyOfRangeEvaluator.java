...
public class CopyOfRangeEvaluator extends RecursiveNumericEvaluator implements ManyValueWorker {
	...
	public Object doWork(Object... values) throws IOException {
			...
			if (values.length >= 2) {
				...
				if (values.length >= 3) {
					if (values[2] instanceof Number) {
						...
						throw new IOException(String.format(Locale.ROOT,
								"Invalid expression %s - if third parameter is provided then it must be a valid number but found %s instead",
		...
		if (startIdx > endIdx) {
			throw new IOException(
					String.format(Locale.ROOT, "Invalid expression %s - startIdx (%d) must be less than endIdx (%d)",
							toExpression(constructingFactory), startIdx, endIdx));
		}

		if (endIdx > sourceValues.size()) {
			throw new IOException(String.format(Locale.ROOT,
					"Invalid expression %s - endIdx (%d) must not be greater then size of source array (%d)",
					toExpression(constructingFactory), endIdx, sourceValues.size()));
		}
