...
import ch.elexis.ordermanagement.ui.dialogs.DailyConsumptionOrderDialog;
import ch.elexis.ordermanagement.ui.dialogs.HistoryDialog;
...
public class OrderManagementActionFactory {
	...
	private IOrder findReusableStockOrder() {
		for (IOrder o : OrderCoreLogic.getOpenOrders()) {
	...
	private void handleAutomaticOrder() {
		...
		if (reusableStockOrder != null) {
			...
			actOrder = OrderCoreLogic.createOrder(Messages.OrderManagement_StockOrder_DefaultName, orderService);
		}
	...
	private void handleDeleteOrder(TableViewer orderTableViewer) {
		...
		String orderDate = OrderCoreLogic.formatDate(selectedOrder.getTimestamp());
