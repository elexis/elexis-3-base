...
public interface TermsParams {
	...
	/**
	 * Optional. The lower bound term to start at. The TermEnum will start at the
	 * next term after this term in the dictionary.
	 *
	...
	/**
	 * Optional. If true, include the upper bound term in the results. False by
	 * default.
	 */
	public static final String TERMS_UPPER_INCLUSIVE = TERMS_PREFIX + "upper.incl";

	/**
	 * Optional. If true, include the lower bound term in the results, otherwise
	 * skip to the next one. True by default.
	 */
	 ...
	 * {@link org.apache.solr.common.params.CommonParams#ROWS}. If that's not
	 * specified, uses
	 * {@link org.apache.solr.common.params.CommonParams#ROWS_DEFAULT}.
	 */
	...
	public static enum TermsRegexpFlag {
		UNIX_LINES(Pattern.UNIX_LINES), CASE_INSENSITIVE(Pattern.CASE_INSENSITIVE), COMMENTS(Pattern.COMMENTS),
		MULTILINE(Pattern.MULTILINE), LITERAL(Pattern.LITERAL), DOTALL(Pattern.DOTALL),
		UNICODE_CASE(Pattern.UNICODE_CASE), CANON_EQ(Pattern.CANON_EQ);
	...
	/**
	 * Optional. The maximum value of docFreq to be returned. -1 by default means no
	 * boundary
	 */
	String TERMS_MAXCOUNT = TERMS_PREFIX + "maxcount";

	/**
	 * Optional. If true, return the raw characters of the indexed term, regardless
	 * of if it is readable. For instance, the index form of numeric numbers is not
	 * human readable. The default is false.
	 */
