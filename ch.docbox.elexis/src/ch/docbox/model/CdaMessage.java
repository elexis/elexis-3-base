...
public class CdaMessage extends PersistentObject {
	...
	public static CdaMessage getCdaMessage(Anwender anwender, String documentId,
		...
		if (cdaMessages.length > 1) {
			logger
				.error("CdaMessage Query should give only one object back but got multiple with AnwenderID=  "
	...
	public CdaMessage(String documentId, String title, GregorianCalendar date){
		...
		timeTool.set(date);
		set(new String[] {
			"AnwenderID", "DocumentID", "Title", "Date", "CreationDate", "Unread"
	...
	public String getCda(){
		...
		} catch (UnsupportedEncodingException e) {} catch (NullPointerException e) {}
		return null;
	}
	...
	/**
	 * unzips the attachments to the specified directory in a subdirectory and sets the files
	 * extracted in the field fileslistings
	 * 
	 * @param attachment
