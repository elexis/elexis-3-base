...
public class ConcurrentUpdateHttp2SolrClient extends SolrClient {
					...
					try {
						...
						if (statusCode != HttpStatus.OK_200) {
							...
							try {
								...
								solrExc = new HttpSolrClient.RemoteSolrException(basePath, statusCode, msg.toString(),
										null);
								if (metadata != null) {
	...
	private void addRunner() {
		MDC.put("ConcurrentUpdateHttp2SolrClient.url", String.valueOf(client.getBaseURL())); // MDC can't have null
																								// value
		try {
			Runner r = new Runner();
			runners.add(r);
		...
		while (!queue.isEmpty()) {
			...
			synchronized (queue) {
				try {
					...
					// i t's going to return immediately
...
//            break;
	...
	/**
	 * Constructs {@link ConcurrentUpdateHttp2SolrClient} instances from provided
	 * configuration.
	 */
	public static class Builder {
		 ...
		 * {@link ConcurrentUpdateHttp2SolrClient#request(SolrRequest)} calls block
		 * waiting to add requests to the queue.
		 *
		 ...
		 * {@link ConcurrentUpdateHttp2SolrClient}s queue.
		 *
		 ...
		 * fills up and {@link ConcurrentUpdateHttp2SolrClient#request(SolrRequest)}
		 ...
		 *
		 * With this option set when the created ConcurrentUpdateHttp2SolrClient sents a
