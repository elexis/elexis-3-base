...
public class CobasMiraMapping {
	...
	public static void initializeMapping() {
		...
		String csvFilename = CoreHub.localCfg.get(Preferences.MAPPINGSCSVFILE,
		...
		try {
			...
			Status status = new Status(IStatus.WARNING, Activator.PLUGIN_ID, "CSV Mapping File nicht gefunden", e);
	...
	private static ColumnPositionMappingStrategy getMappingStrategy() {
		...
		String[] columns = new String[] { "testNameCM", "testNameShort", "TestName", "laborwertID", "refM", "refW",
				"noDecPlaces" };
	...
	public static int getNoDecPlaces(String testName) {
		for (CobasMiraMappingLabitem cMMLI : cobasMappings) {
			try {
				...
				Status status = new Status(IStatus.WARNING, Activator.PLUGIN_ID, "Error parsing integer string", e);
				StatusManager.getManager().handle(status, StatusManager.SHOW);
			}
		}
		return 0;
