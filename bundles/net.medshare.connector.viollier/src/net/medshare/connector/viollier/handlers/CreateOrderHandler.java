...
public class CreateOrderHandler extends AbstractHandler {
	...
	private String getSenderSoftware(){
		...
		if (CoreHub.APPLICATION_NAME.contains(" ")) {
			sb.append(CoreHub.APPLICATION_NAME.split(" ")[0]);
		} else {
			sb.append(CoreHub.APPLICATION_NAME);
		}
		sb.append(" ");
		String[] parts = CoreHub.Version.split("\\.");
		if(parts.length > 3) {
			sb.append(parts[0]).append(".").append(parts[1]).append(".").append(parts[2]);
		} else {
			sb.append(CoreHub.Version);
		}
		return sb.toString();
	}
