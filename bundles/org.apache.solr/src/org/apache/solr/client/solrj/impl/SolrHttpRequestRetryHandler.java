...
public class SolrHttpRequestRetryHandler implements HttpRequestRetryHandler {
	 ...
	 *
	 * @param retryCount how many times to retry; 0 means no retries true if it's OK
	 *                   to retry requests that have been sent
	 * @param clazzes    the IOException types that should not be retried
	 */
	protected SolrHttpRequestRetryHandler(final int retryCount,
	 ...
	 *
	 * @param retryCount how many times to retry; 0 means no retries true if it's OK
	 *                   to retry non-idempotent requests that have been sent
	 */
	@SuppressWarnings("unchecked")
	public SolrHttpRequestRetryHandler(final int retryCount) {
		this(retryCount, Arrays.asList(InterruptedIOException.class, UnknownHostException.class, ConnectException.class,
				SSLException.class));
	}

	/**
	 * Create the request retry handler with a retry count of 3,
	 * requestSentRetryEnabled false and using the following list of non-retriable
	 * IOException classes: <br>
	 * <ul>
	 * <li>InterruptedIOException</li>
	 * <li>UnknownHostException</li>
