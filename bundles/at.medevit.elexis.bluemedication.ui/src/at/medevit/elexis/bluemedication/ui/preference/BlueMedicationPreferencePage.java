...
public class BlueMedicationPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
	...
	protected void createFieldEditors() {
		StringFieldEditor editor = new StringFieldEditor(BlueMedicationConstants.CFG_HIN_PROXY_HOST, "HIN Proxy Host",
	...
	public void init(IWorkbench workbench) {
		String initialvalue = ConfigServiceHolder.get().getLocal(BlueMedicationConstants.CFG_HIN_PROXY_HOST, null);
		if (initialvalue == null) {
			initialvalue = ConfigServiceHolder.get().get(BlueMedicationConstants.CFG_HIN_PROXY_HOST, null);
			if (initialvalue != null) {
				ConfigServiceHolder.get().setLocal(BlueMedicationConstants.CFG_HIN_PROXY_HOST, initialvalue);
			} else {
		...
		initialvalue = ConfigServiceHolder.get().getLocal(BlueMedicationConstants.CFG_HIN_PROXY_PORT, null);
		if (initialvalue == null) {
			initialvalue = ConfigServiceHolder.get().get(BlueMedicationConstants.CFG_HIN_PROXY_PORT, null);
			if (initialvalue != null) {
				ConfigServiceHolder.get().setLocal(BlueMedicationConstants.CFG_HIN_PROXY_PORT, initialvalue);
			} else {
