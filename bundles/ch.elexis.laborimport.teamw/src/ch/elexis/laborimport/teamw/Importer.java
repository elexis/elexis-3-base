...
public class Importer extends ImporterPage {
	...
	private Result<?> importFile(final String filepath) {
		...
		Result<?> result = mfParser.importFromFile(file,
				new DefaultImportStrategyFactory().setPDFImportCategory(ConfigServiceHolder
						.getGlobal(PreferencePage.DOCUMENT_CATEGORY, PreferencePage.DEFAULT_DOCUMENT_CATEGORY))
	...
	private Result<?> importDirectFtp() {
		...
		try {
			...
			String header = MessageFormat.format(ch.elexis.laborimport.teamw.Messages.Importer_import_header, // $NON-NLS-1$
					new Object[] { MY_LAB });
			String question = MessageFormat.format(ch.elexis.laborimport.teamw.Messages.Importer_import_message, // $NON-NLS-1$
			...
			ResultAdapter.displayResult(result, ch.elexis.laborimport.teamw.Messages.Importer_error_import); // $NON-NLS-1$
	...
	private class LabImporter extends Composite {
		...
		public LabImporter(final Composite parent, final ImporterPage home) {
			...
			String batchOrFtp = ConfigServiceHolder.getGlobal(PreferencePage.BATCH_OR_FTP, PreferencePage.FTP);
			String direktHerkunft = ch.elexis.laborimport.teamw.Messages.Importer_ftp_label; // $NON-NLS-1$
			if (PreferencePage.BATCH.equals(batchOrFtp)) {
				direktHerkunft = ch.elexis.laborimport.teamw.Messages.Importer_batch_label; // $NON-NLS-1$
			}
			bDirect = new Button(this, SWT.RADIO);
			bDirect.setText(
					ch.elexis.laborimport.teamw.Messages.Importer_label_importDirect + " (" + direktHerkunft + ")"); //$NON-NLS-1$ //$NON-NLS-2$
