...
public class KnnRegressionEvaluator extends RecursiveObjectEvaluator implements ManyValueWorker {
	...
	public static class KnnRegressionTuple extends Tuple {
		...
		public KnnRegressionTuple(Matrix observations, double[] outcomes, int k, DistanceMeasure distanceMeasure,
				Map<?, ?> map, boolean scale, boolean robust, boolean bivariate) {
			super(map);
		...
		public double[] scale(double[] predictors) {
			...
			// We need to scale the columns of the data matrix with along with the
			// predictors
		...
		public Matrix scale(Matrix predictors) {
			...
			for (int i = 0; i < observationFeatureRows.length; i++) {
				...
				System.arraycopy(predictorFeatureRow, 0, combinedFeatureRow, observationFeatureRow.length,
						predictorFeatureRow.length);

				double[] scaledFeatures = MinMaxScaleEvaluator.scale(combinedFeatureRow, 0, 1);
				System.arraycopy(scaledFeatures, 0, observationFeatureRow, 0, observationFeatureRow.length);
				System.arraycopy(scaledFeatures, observationFeatureRow.length, predictorFeatureRow, 0,
						predictorFeatureRow.length);
			}
			...
			Array2DRowRealMatrix scaledTransposedPredictorMatrix = (Array2DRowRealMatrix) scaledPredictorMatrix
					.transpose();
