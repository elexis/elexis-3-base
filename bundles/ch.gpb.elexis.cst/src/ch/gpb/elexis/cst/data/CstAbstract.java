...
public class CstAbstract extends PersistentObject {
	...
	static final String create = ("CREATE TABLE `" + TABLENAME + "` (" + "  `ID` varchar(25) NOT NULL,"
			+ "  `lastupdate` bigint(20) DEFAULT NULL," + "  `deleted` char(1) DEFAULT '0',"
			+ "  `ItemID` varchar(25) DEFAULT NULL," + "  `Description1` varchar(1024) DEFAULT NULL,"
			+ "  `Description2` varchar(1024) DEFAULT NULL," + "  PRIMARY KEY (`ID`)) "
			+ "COLLATE='utf8_general_ci' ENGINE=InnoDB;" + "INSERT INTO " + TABLENAME + " (ID, description1) VALUES ("
			+ JdbcLink.wrap(VERSIONID) + "," + JdbcLink.wrap(VERSION) + ")");

	static {
		addMapping(TABLENAME, "itemId=ItemID", "description1=Description1", "description2=Description2");
	...
	public static CstAbstract getByLaboritemId(String itemId) {
		...
		if (res.isEmpty()) {
			...
			if (res.size() > 1) {
				throw new IllegalArgumentException(
						String.format("There is already a category of name [%s] - [%s]", itemId));
			}
			return res.get(0);
		}
	}
