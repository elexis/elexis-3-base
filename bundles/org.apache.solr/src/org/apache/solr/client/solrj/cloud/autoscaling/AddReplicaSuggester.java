	...
	SolrRequest tryEachNode(boolean strict) {
		...
		for (Pair<String, String> shard : shards) {
			...
			for (int i = getMatrix().size() - 1; i >= 0; i--) {
				...
				if (!isNodeSuitableForReplicaAddition(row, null))
					continue;
				...
				if (!containsNewErrors(errs)) {
					if ((errs.isEmpty() && isLessDeviant()) || // there are no violations but this is deviating less
							isLessSerious(errs, leastSeriousViolation)) {// there are errors , but this has less serious
																			// violation
			...
			if (bestNode != null) {// there are no rule violations
				...
				return CollectionAdminRequest.addReplicaToShard(shard.first(), shard.second()).setType(type)
