...
public class HttpClientUtil {
	 ...
	 * {@link SocketFactoryRegistryProvider} will require hostname validation of SSL
	 * Certificates. The default behavior is to enforce peer name validation.
	 * <p>
	 * This property will have no effect if
	 * {@link #setSocketFactoryRegistryProvider} is used to override the default
	 * {@link SocketFactoryRegistryProvider}
	 * </p>
	 */
	...
	/**
	 * A Java system property to select the {@linkplain HttpClientBuilderFactory}
	 * used for configuring the {@linkplain HttpClientBuilder} instance by default.
	 */
	 ...
	 * {@linkplain SocketFactoryRegistryProvider} used for configuring the Apache
	 * HTTP clients.
	 */
	...
	static {
		...
		// Configure the SocketFactoryRegistryProvider if user has specified the
		// provider type.
		...
		if (socketFactoryRegistryProviderClassName != null) {
			...
			try {
				socketFactoryRegistryProvider = (SocketFactoryRegistryProvider) Class
						.forName(socketFactoryRegistryProviderClassName).getConstructor().newInstance();
		...
		if (factoryClassName != null) {
			...
			try {
				HttpClientBuilderFactory factory = (HttpClientBuilderFactory) Class.forName(factoryClassName)
						.newInstance();
	...
	private static class DynamicInterceptor implements HttpRequestInterceptor {
		...
		public void process(HttpRequest request, HttpContext context) throws HttpException, IOException {
			// don't synchronize traversal - can lead to deadlock - CopyOnWriteArrayList is
			// critical
			// we also do not want to have to acquire the mutex when the list is empty or
			// put a global
	...
	private static final class DefaultSocketFactoryRegistryProvider extends SocketFactoryRegistryProvider {
		...
		public Registry<ConnectionSocketFactory> getSocketFactoryRegistry() {
			...
			// except that we explicitly use
			// SSLConnectionSocketFactory.getSystemSocketFactory()
			// to pick up the system level default SSLContext (where javax.net.ssl.*
			// properties
	 ...
	 *
	 * @param params http client configuration, if null a client with default
	...
	public static CloseableHttpClient createClient(final SolrParams params, PoolingHttpClientConnectionManager cm,
			boolean sharedConnectionManager, HttpRequestExecutor httpRequestExecutor) {
	...
	private static HttpClientBuilder setupBuilder(HttpClientBuilder builder, SolrParams config) {
		...
		if (config.getBool(HttpClientUtil.PROP_USE_RETRY, true)) {
			retBuilder = retBuilder
					.setRetryHandler(new SolrHttpRequestRetryHandler(Integer.getInteger("solr.httpclient.retries", 3)));
	...
	protected static class GzipDecompressingEntity extends HttpEntityWrapper {
		...
		/**
		 * Return a InputStream of uncompressed data. If there is an issue with the
		 * compression of the data, a null InputStream will be returned, and the
		 * underlying compressed InputStream will be closed.
		 *
		 * The same input stream will be returned if the underlying entity is not
		 * repeatable. If the underlying entity is repeatable, then a new input stream
		 * will be created.
		 */
		@Override
	 ...
	 * {@link HttpClientContext#setUserToken(Object)} to an internal singleton. It
	 * allows to reuse underneath {@link HttpClient} in connection pools if client
	...
	public static Builder createDefaultRequestConfigBuilder() {
		...
		builder.setSocketTimeout(DEFAULT_SO_TIMEOUT).setConnectTimeout(DEFAULT_CONNECT_TIMEOUT)
				.setRedirectsEnabled(false).setDecompressionEnabled(false); // we do our own compression / decompression
		if (cpolicy != null) {
