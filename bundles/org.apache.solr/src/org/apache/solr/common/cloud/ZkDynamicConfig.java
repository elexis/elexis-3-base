...
/**
 * Class holding the dynamic config of a Zookeeper ensemble as fetched from
 * znode <code>/zookeeper/config</code>.
 */
public class ZkDynamicConfig {
	// server.<positive id> =
	// <address1>:<port1>:<port2>[:role][|<address2>:<port2>...];[<client port
	// address>:]<client port>
	// TODO: Add support for handling multiple address specs per server line, how we
	// simply ignore all but the first
	public static final Pattern linePattern = Pattern.compile(
			"server\\.(?<serverId>\\d+) ?= ?(?<address>[^:]+):(?<leaderPort>\\d+):(?<leaderElectionPort>\\d+)(:(?<role>.*?))?(\\|.*?)?(;((?<clientPortAddress>.*?):)?(?<clientPort>\\d+))?");
	...
	/**
	 * Parse a raw multi line config string with the full content of znode
	 * /zookeeper/config.
	 *
	 * @param lines the multi line config string. If empty or null, this will return
	 *              an empty list
	 ...
	 * <code>host:port,host:port[/chroot]</code>
	 *
	...
	public static ZkDynamicConfig fromZkConnectString(String zkHost) {
		ZkDynamicConfig zkDynamicConfig = new ZkDynamicConfig();
		zkDynamicConfig.servers = Arrays
				.stream(zkHost.split("/")[0].split(",")).map(h -> new ZkDynamicConfig.Server(null, null, null, null,
						null, h.split(":")[0], h.contains(":") ? Integer.parseInt(h.split(":")[1]) : 2181))
	...
	public static class Server {
		...
		Server(Integer serverId, String address, Integer leaderPort, Integer leaderElectionPort, String role,
				String clientPortAddress, Integer clientPort) {
		...
		/**
		 * Resolve the most likely address, first trying 'clientPortAddress', falling
		 * back to 'address'
		 *
		...
		public static Server parseLine(String line) {
			...
			if (!m.matches()) {
				throw new SolrException(SolrException.ErrorCode.BAD_REQUEST,
						"Could not parse dynamic zk config line: " + line);
			}
			String clientPortStr = m.group("clientPort");
			return new Server(Integer.parseInt(m.group("serverId")), m.group("address"),
					Integer.parseInt(m.group("leaderPort")), Integer.parseInt(m.group("leaderElectionPort")),
					m.group("role"), m.group("clientPortAddress"),
