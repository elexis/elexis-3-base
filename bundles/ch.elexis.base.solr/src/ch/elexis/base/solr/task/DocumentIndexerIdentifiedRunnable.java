...
public class DocumentIndexerIdentifiedRunnable extends AbstractIDocumentIndexerIdentifiedRunnable {
	...
	private final String QUERY =
		"SELECT ID FROM CH_ELEXIS_OMNIVORE_DATA WHERE !(DOCUMENT_STATUS & 2) AND (DOCUMENT_STATUS & 1) AND PatID IS NOT NULL ORDER BY lastUpdate DESC LIMIT 1000";
	...
	protected List<?> getDocuments(){
		return omnivoreModelService.getNativeQuery(QUERY)
			.executeWithParameters(Collections.emptyMap()).collect(Collectors.toList());
	}
