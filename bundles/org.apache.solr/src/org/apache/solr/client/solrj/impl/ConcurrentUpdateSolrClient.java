...
/**
 * ConcurrentUpdateSolrClient buffers all added documents and writes them into
 * open HTTP connections. This class is thread safe.
 *
 * Params from {@link UpdateRequest} are converted to http request parameters.
 * When params change between UpdateRequests a new HTTP request is started.
 *
 * Although any SolrClient request can be made with this implementation, it is
 * only recommended to use ConcurrentUpdateSolrClient with /update requests. The
 * class {@link HttpSolrClient} is better suited for the query interface.
 */
public class ConcurrentUpdateSolrClient extends SolrClient {
	 ...
	 *
	 * @deprecated use
	 *             {@link ConcurrentUpdateSolrClient#ConcurrentUpdateSolrClient(Builder)}
	 *             instead, as it is a more extension/subclassing-friendly
	 *             alternative
	 */
	@Deprecated
	protected ConcurrentUpdateSolrClient(String solrServerUrl, HttpClient client, int queueSize, int threadCount,
			ExecutorService es, boolean streamDeletes) {
		this((streamDeletes)
				? new Builder(solrServerUrl).withHttpClient(client).withQueueSize(queueSize)
						.withThreadCount(threadCount).withExecutorService(es).alwaysStreamDeletes()
				: new Builder(solrServerUrl).withHttpClient(client).withQueueSize(queueSize)
						.withThreadCount(threadCount).withExecutorService(es).neverStreamDeletes());
	}

	protected ConcurrentUpdateSolrClient(Builder builder) {
		this.internalHttpClient = (builder.httpClient == null);
		this.client = new HttpSolrClient.Builder(builder.baseSolrUrl).withHttpClient(builder.httpClient)
				.withConnectionTimeout(builder.connectionTimeoutMillis).withSocketTimeout(builder.socketTimeoutMillis)
				.build();
		...
		if (builder.executorService != null) {
			...
			this.scheduler = ExecutorUtil
					.newMDCAwareCachedThreadPool(new SolrNamedThreadFactory("concurrentUpdateScheduler"));
	...
	class Runner implements Runnable {
		...
		public void run() {
			...
			// This loop is so we can continue if an element was added to the queue after
			// the last runner exited.
			for (;;) {
				try {
					...
					synchronized (runners) {
						...
						if (runners.size() == 1 && !queue.isEmpty() && !scheduler.isShutdown()) {
							// If there is something else to process, keep last runner alive by staying in
							// the loop.
						} else {
							runners.remove(this);
							if (runners.isEmpty()) {
			...
			while (!queue.isEmpty()) {
				...
				try {
					...
					try {
						...
						if (log.isDebugEnabled())
							pollInterrupts.incrementAndGet();
						continue;
					} finally {
						inPoll = false;
					}
						...
						public void writeTo(OutputStream out) throws IOException {
							...
							while (upd != null) {
								...
								if (!origParams.toNamedList().equals(currentParams.toNamedList())
									...
									queue.add(upd); // Request has different params or destination core/collection,
													// return to queue
									break;
								}
					...
					if (statusCode != HttpStatus.SC_OK) {
						...
						try {
							...
							if (response.getEntity().getContentType().getElements().length > 0) {
								NameValuePair param = response.getEntity().getContentType().getElements()[0]
							...
							solrExc = new HttpSolrClient.RemoteSolrException(client.getBaseURL(), statusCode,
									msg.toString(), null);
							if (metadata != null) {
	...
	private void notifyQueueAndRunnersIfEmptyQueue() {
		if (queue.size() == 0) {
			...
			synchronized (runners) {
				// we notify runners too - if there is a high queue poll time and this is the
				// update
	...
	// *must* be called with runners monitor held, e.g. synchronized(runners){
	// addRunner() }
	private void addRunner() {
		...
		try {
			...
			runners.add(r);
			try {
				scheduler.execute(r); // this can throw an exception if the scheduler has been shutdown, but that
										// should be fine.
					...
					if (currentQueueSize != lastQueueSize) {
						...
						if (lastStallTime == -1) {
							...
							if (currentStallTime > stallTime) {
								throw new IOException("Request processing has stalled for " + currentStallTime
										+ "ms with " + queue.size() + " remaining elements in the queue.");
							}
						}
					}
				}
	...
	public synchronized void blockUntilFinished() throws IOException {
		...
		try {
			...
			synchronized (runners) {
				...
				// NOTE: if the executor is shut down, runners may never become empty (a
				// scheduled task may never be run,
				// which means it would never remove itself from the runners list. This is why
				...
				while (!runners.isEmpty()) {
					...
					// Need to check if the queue is empty before really considering this is
					// finished (SOLR-4260)
					...
					if (lastQueueSize != queueSize) {
						...
						if (lastStallTime == -1) {
							...
							long currentStallTime = TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - lastStallTime);
							if (currentStallTime > stallTime) {
								throw new IOException("Task queue processing has stalled for " + currentStallTime
										+ " ms with " + queueSize + " remaining elements to process.");
...
//                break;
	...
	private void waitForEmptyQueue() throws IOException {
		...
		while (!queue.isEmpty()) {
			if (log.isDebugEnabled())
				emptyQueueLoops.incrementAndGet();
			...
			synchronized (queue) {
				try {
					...
					// If we set the thread as interrupted again, the next time the wait it's called
			...
			if (currentQueueSize != lastQueueSize) {
				...
				if (lastStallTime == -1) {
					...
					long currentStallTime = TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - lastStallTime);
					if (currentStallTime > stallTime) {
						throw new IOException("Task queue processing has stalled for " + currentStallTime + " ms with "
								+ currentQueueSize + " remaining elements to process.");
//            threadInterrupted = true;
//            break;
					}
				}
	...
	/**
	 * Intended to be used as an extension point for doing post processing after a
	 * request completes.
	 */
	...
	public synchronized void close() {
		...
		try {
			...
			if (log.isDebugEnabled()) {
				log.debug("STATS pollInteruppts={} pollExists={} blockLoops={} emptyQueueLoops={}",
						pollInterrupts.get(), pollExits.get(), blockLoops.get(), emptyQueueLoops.get());
			}
		}
	}
	...
	/**
	 * set soTimeout (read timeout) on the underlying HttpConnectionManager. This is
	...
	/**
	 * @param pollQueueTime time for an open connection to wait for updates when the
	 *                      queue is empty.
	 */
	...
	/**
	 * Constructs {@link ConcurrentUpdateSolrClient} instances from provided
	 * configuration.
	 */
	public static class Builder extends SolrClientBuilder<Builder> {
		 ...
		 * </pre>
		 *
		 * Note that when a core is provided in the base URL, queries and other requests
		 * can be made without mentioning the core explicitly. However, the client can
		 * only send requests to that core.
		 *
		 ...
		 * </pre>
		 *
		 * In this case the client is more flexible and can be used to send requests to
		 * any cores. This flexibility though requires that the core be specified on all
		 * requests.
		 */
		...
		/**
		 * The maximum number of requests buffered by the SolrClient's internal queue
		 * before being processed by background threads.
		 *
		 * This value should be carefully paired with the number of queue-consumer
		 * threads. A queue with a maximum size set too high may require more memory. A
		 * queue with a maximum size set too low may suffer decreased throughput as
		 * {@link ConcurrentUpdateSolrClient#request(SolrRequest)} calls block waiting
		 ...
		 * {@link ConcurrentUpdateSolrClient}s queue.
		 *
		 * Threads are created when documents are added to the client's internal queue
		 * and exit when no updates remain in the queue.
		 * <p>
		 * This value should be carefully paired with the maximum queue capacity. A
		 * client with too few threads may suffer decreased throughput as the queue
		 * fills up and {@link ConcurrentUpdateSolrClient#request(SolrRequest)} calls
		...
		/**
		 * Provides the {@link ExecutorService} for the created client to use when
		 * servicing the update-request queue.
		 */
		 ...
		 *
		 * Streamed deletes are put into the update-queue and executed like any other
		 * update request.
		 */
		 ...
		 *
		 * With this option set when the created ConcurrentUpdateSolrClient sents a
		 * delete request it will first will lock the queue and block until all queued
		 * updates have been sent, and then send the delete request.
		 */
		...
		/**
		 * Create a {@link ConcurrentUpdateSolrClient} based on the provided
