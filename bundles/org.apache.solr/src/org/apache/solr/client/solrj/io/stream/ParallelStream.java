...
/**
 * The ParallelStream decorates a TupleStream implementation and pushes it to N
 * workers for parallel execution. Workers are chosen from a SolrCloud
 * collection. Tuples that are streamed back from the workers are ordered by a
 * Comparator.
 *
 * @since 5.1.0
 **/
public class ParallelStream extends CloudSolrStream implements Expressible {
	...
	public ParallelStream(StreamExpression expression, StreamFactory factory) throws IOException {
		...
		// Workers
		if (null == workersParam || null == workersParam.getParameter()
				|| !(workersParam.getParameter() instanceof StreamExpressionValue)) {
			throw new IOException(String.format(Locale.ROOT,
					"Invalid expression %s - expecting a single 'workers' parameter of type positive integer but didn't find one",
					expression));
		}
		...
		try {
			...
			if (workersInt <= 0) {
				throw new IOException(String.format(Locale.ROOT,
						"invalid expression %s - workers '%s' must be greater than 0.", expression, workersStr));
			}
		} catch (NumberFormatException e) {
			throw new IOException(String.format(Locale.ROOT,
					"invalid expression %s - workers '%s' is not a valid integer.", expression, workersStr));
		}
		...
		if (null == sortExpression || !(sortExpression.getParameter() instanceof StreamExpressionValue)) {
			throw new IOException(String.format(Locale.ROOT,
					"Invalid expression %s - expecting single 'sort' parameter telling us how to join the parallel streams but didn't find one",
					expression));
		}
		...
		StreamComparator comp = factory.constructComparator(
	...
	private void init(String zkHost, String collection, TupleStream tupleStream, int workers, StreamComparator comp)
			throws IOException {
		this.zkHost = zkHost;
	...
	public Tuple read() throws IOException {
		...
		if (tuple.EOF) {
			/*
			 * Map<String, Map> metrics = new HashMap(); Iterator<Entry<String,Tuple>> it =
			 * this.eofTuples.entrySet().iterator(); while(it.hasNext()) { Map.Entry<String,
			 * Tuple> entry = it.next(); if(entry.getValue().fields.size() > 1) {
			 * metrics.put(entry.getKey(), entry.getValue().fields); } }
			 *
			 * if(metrics.size() > 0) { t.setMetrics(metrics); }
			 */
			return Tuple.EOF();
		}
