...
public class Value {
	...
	Value(final String paramName, final String unit) throws PackageException{
		...
		if (CoreHub.localCfg != null
			&& CoreHub.localCfg.get(Preferences.APPLY_SENT_UNITS, "n").equalsIgnoreCase("y")) {
			_unit = unit;
		}
		...
		if (_labItem == null) {
			_labItem =
				LabImportUtilHolder.get().createLabItem(_shortName, _longName, _labor, _refMann,
					_refFrau, _unit, LabItemTyp.NUMERIC, Messages.Afinion_Value_LabName, "50");
		}
	}
	...
	public TransientLabResult fetchValue(Patient patient, String value, String flags,
		...
		IPatient iPatient = CoreModelServiceHolder.get()
			.load(patient.getId(), ch.elexis.core.model.IPatient.class).orElse(null);
