...
public class CovidHandlerUtil {
	...
	public static BillingLaw[] KK_LAWS = {
		BillingLaw.KVG
	};
	public static BillingLaw[] SZ_LAWS = {
		BillingLaw.privat, BillingLaw.VVG
	};
	
	/**
	 * Search for the first open {@link ICoverage} of the {@link IPatient} with a matching law.
	 * 
	 * @param patient
	 * @param law
	 * @return
	...
	/**
	 * Get all configured covid {@link ICodeElementBlock} instances as map with id as key.
	 * 
	 * @return
	 */
	public static Map<String, ICodeElementBlock> getConfiguredBlocks(){
		...
		if (ConfigServiceHolder.get().get(CFG_KK_PCR_BLOCKID, null) != null) {
			CoreModelServiceHolder.get()
				.load(ConfigServiceHolder.get().get(CFG_KK_PCR_BLOCKID, null),
		...
		if (ConfigServiceHolder.get().get(CFG_SZ_PCR_BLOCKID, null) != null) {
			CoreModelServiceHolder.get()
				.load(ConfigServiceHolder.get().get(CFG_SZ_PCR_BLOCKID, null),
					ICodeElementBlock.class)
	 ...
	 * @return
	 */
	public static Optional<CertificateInfo> getCertificateAtWithType(IPatient patient,
		LocalDate at, Type type){
		List<CertificateInfo> certificates = CertificateInfo.of(patient);
		if (!certificates.isEmpty()) {
			return certificates.stream()
				.filter(ci -> ci.getType() == type && ci.getTimestamp().toLocalDate().equals(at))
				.findFirst();
		}
	...
	public static void openLetter(IDocumentLetter letter,
		ILocalDocumentService localDocumentService){
		if (letter != null) {
	 ...
	 * @return
	 */
	public static List<IEncounter> getEncountersAt(IPatient patient, LocalDate localDate,
		BillingLaw... laws){
		if (patient.getCoverages() != null) {
			...
			return coverages.stream()
				.filter(coverage -> coverage.isOpen() && (laws != null
						? Arrays.asList(laws).contains(coverage.getBillingSystem().getLaw())
						: true))
	...
	public static boolean isAntigenBilled(IEncounter encounter){
		Optional<IBilled> found = encounter.getBilled().stream()
			.filter(billed -> isAntigenBilled(billed)).findFirst();
	...
	public static boolean isPcrBilled(IEncounter encounter){
		Optional<IBilled> found =
			encounter.getBilled().stream().filter(billed -> isPandemieBilled(billed)).findFirst();
		if (found.isPresent()) {
	...
	public static Optional<ICoverage> createSzCoverage(IPatient patient){
		return Optional.of(new ICoverageBuilder(CoreModelServiceHolder.get(), patient,
			"Selbstzahler", ICoverageBuilder.getDefaultCoverageReason(ConfigServiceHolder.get()),
	...
	public static TestModel getTestModel(IPatient patient, CertificatesService service,
		...
		TestModel ret =
			new TestModel().initDefault(patient, service.getOtp());
	...
	public static List<IDocumentLetter> getLettersAt(IPatient patient, LocalDate at,
		String... subjects){
		...
		if(at != null) {
			return query.execute().stream().filter(
				d -> LocalDate.ofInstant(d.getCreated().toInstant(), ZoneId.systemDefault()).equals(at))
				.collect(Collectors.toList());			
		} else {
