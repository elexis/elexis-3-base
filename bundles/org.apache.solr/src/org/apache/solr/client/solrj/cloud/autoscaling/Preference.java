...
public class Preference implements MapWriter {
	...
	public Preference(Map<String, Object> m, int idx) {
		...
		if (precision < name.min || precision > name.max) {
			throw new RuntimeException(StrUtils.formatString(
					"invalid precision value {0} , must lie between {1} and {2}", precision, name.min, name.max));
		}

	}

	// there are 2 modes of compare.
	// recursive, it uses the precision to tie & when there is a tie use the next
	// preference to compare
	// in non-recursive mode, precision is not taken into consideration and sort is
	// done on actual value
	int compare(Row r1, Row r2, boolean useApprox) {
		...
		if (o1 instanceof Long && o2 instanceof Long)
			...
			throw new RuntimeException("Unable to compare " + o1 + " of type: " + o1.getClass().getName() + " from "
					+ r1.cells[idx].toString() + " and " + o2 + " of type: " + o2.getClass().getName() + " from "
					+ r2.cells[idx].toString());
		}
	...
	/**
	 * @return an unmodifiable copy of the original map from which this object was
	 *         constructed
	 */
