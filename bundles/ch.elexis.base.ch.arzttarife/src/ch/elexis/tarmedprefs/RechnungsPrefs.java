...
public class RechnungsPrefs extends PreferencePage implements IWorkbenchPreferencePage {
	...
	protected Control createContents(Composite parent){
		...
		fixProvider.addHyperlinkListener(new HyperlinkAdapter() {
			...
			public void linkActivated(HyperlinkEvent e){
				KontaktSelektor ks =
					new KontaktSelektor(UiDesk.getTopShell(), Kontakt.class, "Kontakt auswählen",
						"Bitte selektieren Sie den fixen Leistungserbringer", new String[] {});
				int ret = ks.open();
				if (ret == Window.OK) {
					Kontakt k = (Kontakt) ks.getSelection();
					CoreHub.globalCfg.set(PreferenceConstants.TARMEDBIL_FIX_PROVIDER,
						(k != null) ? k.getId() : null);
					String label = (k != null) ? k.getLabel() : "";
					lblFixProvider.setText(label);
				} else {
					CoreHub.globalCfg.remove(PreferenceConstants.TARMEDBIL_FIX_PROVIDER);
		...
		if (StringUtils
			.isNotBlank(CoreHub.globalCfg.get(PreferenceConstants.TARMEDBIL_FIX_PROVIDER, null))) {
			Kontakt k = Kontakt
