...
public class TimeSeriesStream extends TupleStream implements Expressible {
	...
	public TimeSeriesStream(String zkHost, String collection, SolrParams params, Metric[] metrics, String field,
			String start, String end, String gap, String format) throws IOException {
	...
	public TimeSeriesStream(StreamExpression expression, StreamFactory factory) throws IOException {
		...
		if (startExpression != null) {
			...
			throw new IOException(
					String.format(Locale.ROOT, "invalid expression %s - start parameter is required", expression));
		}
		...
		if (endExpression != null) {
			...
			throw new IOException(
					String.format(Locale.ROOT, "invalid expression %s - end parameter is required", expression));
		}
		...
		if (gapExpression != null) {
			...
			throw new IOException(
					String.format(Locale.ROOT, "invalid expression %s - gap parameter is required", expression));
		}
		...
		if (fieldExpression != null) {
			...
			throw new IOException(
					String.format(Locale.ROOT, "invalid expression %s - field parameter is required", expression));
		}
		...
		for (StreamExpressionNamedParameter namedParam : namedParams) {
			if (!namedParam.getName().equals("zkHost") && !namedParam.getName().equals("start")
					&& !namedParam.getName().equals("end") && !namedParam.getName().equals("gap")) {
	...
	private void init(String collection, SolrParams params, String field, Metric[] metrics, String start, String end,
			String gap, String format, String zkHost) throws IOException {
		this.zkHost = zkHost;
	...
	private void getTuples(@SuppressWarnings({ "rawtypes" }) NamedList response, String field, Metric[] metrics) {
	...
	private void fillTuples(List<Tuple> tuples, Tuple currentTuple, @SuppressWarnings({ "rawtypes" }) NamedList facets,
			String field, Metric[] _metrics) {
		...
		for (int b = 0; b < allBuckets.size(); b++) {
			...
			if (formatter != null) {
				LocalDateTime localDateTime = LocalDateTime.ofInstant(((java.util.Date) val).toInstant(),
						ZoneOffset.UTC);
