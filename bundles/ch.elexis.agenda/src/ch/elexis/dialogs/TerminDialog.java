...
public class TerminDialog extends TitleAreaDialog {
	...
	/**
	 * Use this constructor for creating a new {@link Termin} from a date and for a
	 * resource.
	 *
	 * @param date
	 * @param resource
	...
	protected Control createDialogArea(final Composite parent) {
		 ...
		 * bPrev.setToolTipText(Messages.TerminDialog_earlier); bNext=new
		 * Button(topRight,SWT.PUSH); bNext.setText("-->"); //$NON-NLS-1$
		 * bNext.setToolTipText(Messages.TerminDialog_later);
		 */
		...
		if (AccessControlServiceHolder.get().evaluate(EvACE.of(IAppointment.class, Right.UPDATE)) == false) {
	...
	class DayOverview extends Composite implements PaintListener {
		...
		void recalc() {
			if (useGlobalData) {
				...
				list = Plannables.loadTermine(cbMandant.getText(), new TimeTool(dp.getDate().getTime()));
			}
	...
	private void createTermin(final boolean bMulti) {
		...
		actTermin.set(Termin.FLD_CREATOR, ContextServiceHolder.get().getActiveUser().get().getLabel());
	...
	/**
	 * checks if the given appointment overlaps with any other appointment of this
	 * patient
	 *
	 * @param actDate  date of the appointment
	 * @param terminTS timespan of the appointment
	 * @return true if there is a collision
	 */
	private boolean terminOverlaps(TimeTool actDate, TimeSpan terminTS) {
		boolean avoidPatientDoubleBooking = CoreHub.localCfg.get(PreferenceConstants.AG_AVOID_PATIENT_DOUBLE_BOOKING,
