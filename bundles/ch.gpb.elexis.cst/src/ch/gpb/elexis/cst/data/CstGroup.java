...
public class CstGroup extends PersistentObject {
	...
	static final String create = "CREATE TABLE `cstgroups` (" + "	`ID` VARCHAR(25) NOT NULL,"
			...
			+ "	`Name` VARCHAR(30) NULL DEFAULT NULL," + "	`Description` VARCHAR(256) NULL DEFAULT NULL,"
			+ "	`itemsRanking` BLOB NULL, " + "	`Icon` VARCHAR(25) NULL, " + "	PRIMARY KEY (`ID`)" + ")"
			+ " COlLATE='utf8_general_ci' " + " ENGINE=InnoDB;" + " INSERT INTO " + TABLENAME + " (ID, name) VALUES ("
			+ JdbcLink.wrap(VERSIONID) + "," + JdbcLink.wrap(VERSION) + ");";

	static final String create2 = "CREATE TABLE `cstgroup_labitem_joint` (" + "	`ID` VARCHAR(25) NULL DEFAULT NULL,"
			+ "	`deleted` CHAR(1) NULL DEFAULT '0'," + "	`lastupdate` BIGINT(20) NULL DEFAULT NULL,"
			+ "	`GroupID` VARCHAR(25) NULL DEFAULT NULL," + "	`ItemID` VARCHAR(25) NULL DEFAULT NULL,"
			+ "	`DisplayOnce` CHAR(1) NOT NULL DEFAULT '0'," + "	`Comment` TEXT NULL,"
			+ "	UNIQUE INDEX `GroupID` (`GroupID`, `ItemID`)" + ")" + " COLlATE='utf8_general_ci' ENGINE=InnoDB;";

	static {
		addMapping(TABLENAME, "name=Name", "description=Description", "icon=Icon", "kontaktId=KontaktID",
				"mandantId=MandantID", "itemsRanking=ItemsRanking",
	...
	public int setDisplayOnce(LabItemWrapper labItem, String displayOnce) {
		...
		sql.append("UPDATE " + GROUP_ITEM_TABLENAME + " SET DisplayOnce = '").append(displayOnce)
				.append("' WHERE GroupID = ").append(getWrappedId()).append(" AND ItemId = ")
				.append(labItem.getLabItem().getWrappedId());
	...
	public void addItem(LabItemWrapper item) {
		if (item != null && (item.getLabItem().state() == EXISTS)) {
			...
			String exists = j.queryString("SELECT ItemID FROM " + GROUP_ITEM_TABLENAME + " WHERE GroupID = "
			...
			if (StringTool.isNothing(exists)) {
				...
				throw new IllegalArgumentException(
						String.format("There is already a category of name [%s] - [%s]", item.getLabItem().getName()));
	 ...
	 */
	/*
	 * public void addLabitem(final LabItem a){ for (LabItem labItem: getLabitems())
	 * { if (labItem.getId().equalsIgnoreCase(a.getId())) return; }
	...
	public boolean delete() {
		getConnection().exec("DELETE FROM  " + GROUP_ITEM_TABLENAME + " WHERE GroupID =" + getWrappedId());
		return super.delete();
	}
