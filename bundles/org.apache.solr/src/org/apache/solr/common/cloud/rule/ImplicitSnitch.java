...
public class ImplicitSnitch extends Snitch {
	...
	public static final List<String> IP_SNITCHES = Collections
			.unmodifiableList(Arrays.asList("ip_1", "ip_2", "ip_3", "ip_4"));
	public static final Set<String> tags = Collections.unmodifiableSet(
			new HashSet<>(Arrays.asList(NODE, PORT, HOST, CORES, DISK, ROLE, "ip_1", "ip_2", "ip_3", "ip_4")));
	...
	private void fillRole(String solrNode, SnitchContext ctx, String key) throws KeeperException, InterruptedException {
		...
		try {
			if (roles == null)
				roles = ctx.getZkJson(ZkStateReader.ROLES);
	...
	private String[] getIpFragments(String solrNode) {
		...
		log.warn("Failed to match host IP address from node URL [{}] using regex [{}]", solrNode,
