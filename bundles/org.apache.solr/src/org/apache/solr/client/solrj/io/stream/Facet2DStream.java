...
public class Facet2DStream extends TupleStream implements Expressible {
	...
	public Facet2DStream(String zkHost, String collection, ModifiableSolrParams params, Bucket x, Bucket y,
			String dimensions, Metric metric) throws IOException {
		if (dimensions != null) {
	...
	public Facet2DStream(StreamExpression expression, StreamFactory factory) throws IOException {
		...
		List<StreamExpression> metricExpression = factory.getExpressionOperandsRepresentingTypes(expression,
		...
		if (x == null || y == null) {
			throw new IOException(String.format(Locale.ROOT,
					"invalid expression %s - x and y buckets expected. eg. 'x=\"name\"'", expression, collectionName));
		}
		...
		if (dimensionsExpression != null) {
			if (dimensionsExpression.getParameter() instanceof StreamExpressionValue) {
				String[] strDimensions = ((StreamExpressionValue) dimensionsExpression.getParameter()).getValue()
						.split(",");
				if (strDimensions.length != 2) {
					throw new IOException(
							String.format(Locale.ROOT, "invalid expression %s - two dimension values expected"));
	...
	private void init(String collection, SolrParams params, Bucket x, Bucket y, FieldComparator bucketSort,
			int dimensionX, int dimensionY, Metric metric, String zkHost) {
	...
	public StreamExpressionParameter toExpression(StreamFactory factory) throws IOException {
		...
		// dimensions
		expression.addParameter(new StreamExpressionNamedParameter("dimensions",
				Integer.toString(dimensionX) + "," + Integer.toString(dimensionY)));
	...
	private String getJsonFacetString(Bucket x, Bucket y, Metric metric, FieldComparator[] adjustedSorts,
	...
	private void appendJson(StringBuilder buf, Bucket x, Bucket y, Metric metric, FieldComparator[] adjustedSorts,
			int dimensionX, int dimensionY) {
