...
public class ZkNodeProps implements JSONWriter.Writable {
	...
	/**
	 * Feature flag to enable storing the 'base_url' property; base_url will not be
	 * stored as of Solr 9.x. Installations that use an older (pre-8.8) SolrJ
	 * against a 8.8.0 or newer server will need to set this system property to true
	 * to avoid NPEs when reading cluster state from Zookeeper, see SOLR-15145.
	 */
	...
	public ZkNodeProps(Map<String, Object> propMap) {
		...
		// don't store base_url if we have a node_name to recompute from when we read
		// back from ZK
		// sub-classes that know they need a base_url (Replica) can eagerly compute in
		// their ctor
		if (!STORE_BASE_URL && this.propMap.containsKey(ZkStateReader.NODE_NAME_PROP)) {
			this.propMap.remove(ZkStateReader.BASE_URL_PROP);
		}

		// TODO: store an unmodifiable map, but in a way that guarantees not to wrap
		// more than once.
	...
	public void write(JSONWriter jsonWriter) {
		...
		if (!STORE_BASE_URL && propMap.containsKey(ZkStateReader.BASE_URL_PROP)
				&& propMap.get(ZkStateReader.NODE_NAME_PROP) != null) {
			final Map<String, Object> filtered = new HashMap<>(propMap);
			filtered.remove(ZkStateReader.BASE_URL_PROP);
			jsonWriter.write(filtered);
		} else if (STORE_BASE_URL && propMap.get(ZkStateReader.BASE_URL_PROP) == null
				&& propMap.get(ZkStateReader.NODE_NAME_PROP) != null) {
			// this is for back-compat with older SolrJ
			final Map<String, Object> addBaseUrl = new HashMap<>(propMap);
			addBaseUrl.put(ZkStateReader.BASE_URL_PROP,
					UrlScheme.INSTANCE.getBaseUrlForNodeName((String) propMap.get(ZkStateReader.NODE_NAME_PROP)));
	...
	public String getStr(String key, String def) {
		...
		// TODO: This "hack" should not be needed but keeping it here b/c we removed the
		// base_url from the map in the ctor
