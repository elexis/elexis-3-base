...
public class Row implements MapWriter {
	...
	public Row(String node, List<Pair<String, Variable.Type>> params, List<String> perReplicaAttributes,
			Policy.Session session) {
		this(node, params, perReplicaAttributes, session, session.nodeStateProvider, session.cloudManager);
	}

	/**
	 * Constructor that allows explicitly passing a {@link NodeStateProvider} and a
	 * {@link SolrCloudManager} in order not to use those obtained through the
	 * passed <code>session</code>.
	 * <p>
	 * Note the resulting row has a {@link Policy.Session} that may not be
	 * consistent with the rest of the Row's state. When rows are copied as part of
	 * a {@link Policy.Session} copy, the copied rows' sessions are eventually
	 * updated in
	 ...
	 * once the new {@link Policy.Session} instance is available.
	 * </p>
	 */
	@SuppressWarnings({ "rawtypes" })
	Row(String node, List<Pair<String, Variable.Type>> params, List<String> perReplicaAttributes,
			Policy.Session session, NodeStateProvider nsp, SolrCloudManager cloudManager) {
		this.session = session;
		collectionVsShardVsReplicas = nsp.getReplicaInfo(node, perReplicaAttributes);
		if (collectionVsShardVsReplicas == null)
			collectionVsShardVsReplicas = new HashMap<>();
		this.node = node;
		...
		for (int i = 0; i < params.size(); i++) {
			...
			cells[i] = new Cell(i, pair.first(), Clause.validate(pair.first(), vals.get(pair.first()), false), null,
					pair.second(), this);
...
//      getCacheEntry(cacheName).hits.incrementAndGet();
	...
	public void forEachShard(String collection, BiConsumer<String, List<ReplicaInfo>> consumer) {
		collectionVsShardVsReplicas.getOrDefault(collection, Collections.emptyMap()).forEach(consumer);
	}
	...
	public <R> R computeCacheIfAbsent(String coll, String shard, String cacheName, Object key,
			Function<Object, R> supplier) {
		Map collMap = (Map) this.perCollCache.get(coll);
		if (collMap == null)
			this.perCollCache.put(coll, collMap = new HashMap());
		Map shardMap = (Map) collMap.get(shard);
		if (shardMap == null)
			collMap.put(shard, shardMap = new HashMap());
		Map cacheNameMap = (Map) shardMap.get(cacheName);
		if (cacheNameMap == null)
			shardMap.put(cacheName, cacheNameMap = new HashMap());
	...
	public Row(String node, Cell[] cells, boolean anyValueMissing,
			Map<String, Map<String, List<ReplicaInfo>>> collectionVsShardVsReplicas, boolean isLive,
			Policy.Session session, @SuppressWarnings({ "rawtypes" }) Map perRowCache,
	...
	Row copy() {
		return new Row(node, cells, anyValueMissing, collectionVsShardVsReplicas, isLive, session, this.globalCache,
				this.perCollCache);
	}
	...
	/**
	 * this simulates adding a replica of a certain coll+shard to node. as a result
	 * of adding a replica , values of certain attributes will be modified, in this
	 * node as well as other nodes. Please note that the state of the current
	 * session is kept intact while this operation is being performed
	 *
	 * @param coll           collection name
	 * @param shard          shard name
	 * @param type           replica type
	 * @param recursionCount the number of times we have recursed to add more
	 *                       replicas
	...
	Row addReplica(String coll, String shard, Replica.Type type, int recursionCount, boolean strictMode) {
		...
		ReplicaInfo ri = new ReplicaInfo(replicaname, replicaname, coll, shard, type, this.node, Utils
				.makeMap(ZkStateReader.REPLICA_TYPE, type != null ? type.toString() : Replica.Type.NRT.toString()));
		replicas.add(ri);
	...
	private void lazyCopyReplicas(String coll, String shard) {
		...
		if (isAlreadyCopied)
			return;// caches need to be invalidated but the rest can remain as is
	...
	public void createCollShard(Pair<String, String> collShard) {
		Map<String, List<ReplicaInfo>> shardInfo = collectionVsShardVsReplicas.computeIfAbsent(collShard.first(),
				o -> new HashMap<String, List<ReplicaInfo>>());
		if (collShard.second() != null)
			shardInfo.computeIfAbsent(collShard.second(), o -> new ArrayList<ReplicaInfo>());
	}
	...
	public static void forEachReplica(Map<String, Map<String, List<ReplicaInfo>>> collectionVsShardVsReplicas,
			Consumer<ReplicaInfo> consumer) {
		collectionVsShardVsReplicas
				.forEach((coll, shardVsReplicas) -> shardVsReplicas.forEach((shard, replicaInfos) -> {
