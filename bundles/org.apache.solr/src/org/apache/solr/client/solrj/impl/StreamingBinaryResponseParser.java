...
/**
 * A BinaryResponseParser that sends callback events rather then build a large
 * response
 *
 *
 * @since solr 4.0
...
public class StreamingBinaryResponseParser extends BinaryResponseParser {
	...
	private NamedList<Object> fastStreamDocs(InputStream body, FastStreamingDocsCallback fastCallback)
			throws IOException {
		...
		new FastJavaBinDecoder().withInputStream(body).decode(new EntryListener() {
			@Override
			public void entry(DataEntry e) {
				...
				if (e.isKeyValEntry() && entry.getTag() == Tag._SOLRDOCLST) {
					...
					e.listenContainer(fastCallback.initDocList((Long) l.get(0), (Long) l.get(1), (Float) l.get(2)),
							docListener);
				} else {
