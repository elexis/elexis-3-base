...
public class CommitStream extends TupleStream implements Expressible {
	...
	public CommitStream(StreamExpression expression, StreamFactory factory) throws IOException {
		...
		if (batchSize < 0) {
			throw new IOException(String.format(Locale.ROOT,
					"invalid expression %s - batchSize cannot be less than 0 but is '%d'", expression, batchSize));
		}
		...
		init(collectionName, factory.constructStream(sourceStreamExpression), zkHost, batchSize, waitFlush,
				waitSearcher, softCommit);
	}

	public CommitStream(String collectionName, TupleStream tupleSource, String zkHost, int batchSize, boolean waitFlush,
			boolean waitSearcher, boolean softCommit) throws IOException {
		if (batchSize < 0) {
	...
	private void init(String collectionName, TupleStream tupleSource, String zkHost, int batchSize, boolean waitFlush,
	...
	public Tuple read() throws IOException {
		...
		if (tuple.EOF) {
			...
			if (commitBatchSize > 0 && docsSinceCommit >= commitBatchSize) {
				// if commitBatchSize == 0 then the tuple.EOF above will end up calling
				// sendCommit()
				sendCommit();
			}
		}
	...
	private StreamExpression toExpression(StreamFactory factory, boolean includeStreams) throws IOException {
		...
		if (includeStreams) {
			if (tupleSource instanceof Expressible) {
				...
				throw new IOException(
						"This CommitStream contains a non-expressible TupleStream - it cannot be converted to an expression");
			}
		} else {
	...
	public Explanation toExplanation(StreamFactory factory) throws IOException {
		...
		// A commit stream is backward wrt the order in the explanation. This stream is
		// the "child"
	...
	private void sendCommit() throws IOException {
		...
		try {
			...
			log.warn(String.format(Locale.ROOT,
					"Unable to commit documents to collection '%s' due to unexpected error.", collection), e);
			String className = e.getClass().getName();
			String message = e.getMessage();
			throw new IOException(
					String.format(Locale.ROOT, "Unexpected error when committing documents to collection %s- %s:%s",
							collection, className, message));
		}
	}
}
