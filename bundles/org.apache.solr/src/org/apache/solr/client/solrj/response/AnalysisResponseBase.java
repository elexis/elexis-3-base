...
public class AnalysisResponseBase extends SolrResponseBase {
	 ...
	 * Expects a named list of the form: <br>
	 *
	 * <pre>
	 * <code>
	 *  &lt;lst name="index"&gt;
	 ...
	 * </pre>
	 *
	 * The special case is a CharacterFilter that just returns a string, which we
	 * then map to a single token without type.
	 *
	...
	/**
	 * Convert a string value (from CharacterFilter) into a TokenInfo for its value
	 * full span.
	 *
	...
	/**
	 * Parses the given named list and builds a token infoform it. Expects a named
	 * list of the form: <br>
	 *
	 * <pre>
	 * <code>
	...
	/**
	 * A phase in the analysis process. The phase holds the tokens produced in this
	 * phase and the name of the class that produced them.
	 */
	public static class AnalysisPhase {
		...
		/**
		 * The name of the class (analyzer, tokenzier, or filter) that produced the
		 * token stream for this phase.
		 *
		...
		/**
		 * Returns a list of tokens which represent the token stream produced in this
		 * phase.
		 *
		 * @return A list of tokens which represent the token stream produced in this
		 *         phase.
		 */
	...
	public static class TokenInfo {
		 ...
		 * @param rawText  The raw text of the token. If the token is stored in the
		 *                 index in a special format (e.g. dates or padded numbers) this
		 *                 argument should hold this value. If the token is stored as
		 *                 is, then this value should be {@code null}.
		 * @param type     The type fo the token (typically either {@code word} or
		 *                 {@code <ALPHANUM>} though it depends on the tokenizer/filter
		 *                 used).
		 * @param start    The start position of the token in the original text where it
		 *                 was extracted from.
		 * @param end      The end position of the token in the original text where it
		 *                 was extracted from.
		 * @param position The position of the token within the token stream.
		 * @param match    Indicates whether this token matches one of the the query
		 *                 tokens.
		 */
		...
		/**
		 * Returns the raw text of the token. If the token is index in a special format
		 * (e.g. date or paddded numbers) it will be returned as the raw text. Returns
		 * {@code null} if the token is indexed as is.
		 *
		...
		/**
		 * Returns the type of the token. Typically this will be {@code word} or
		 * {@code <ALPHANUM>}, but it really depends on the tokenizer and filters that
		 * are used.
		 *
		...
		/**
		 * Returns the start position of this token within the text it was originally
		 * extracted from.
		 *
		 * @return The start position of this token within the text it was originally
		 *         extracted from.
		 */
		public int getStart() {
			return start;
		...
		/**
		 * Returns the end position of this token within the text it was originally
		 * extracted from.
		 *
		 * @return The end position of this token within the text it was originally
		 *         extracted from.
		 */
		public int getEnd() {
			return end;
		...
		/**
		 * Returns whether this token matches one of the query tokens (if query analysis
		 * is performed).
		 *
		 * @return Whether this token matches one of the query tokens (if query analysis
		 *         is performed).
		 */
