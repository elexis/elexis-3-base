...
public class PsychoLeistungCodeElementService implements ICodeElementServiceContribution, IStoreToStringContribution {
	...
	public String getSystem() {
		return PsychoLeistung.CODESYSTEM_NAME;
	}
	...
	public Optional<ICodeElement> loadFromCode(String code, Map<Object, Object> context) {
		...
		TypedQuery<PsychoLeistung> gtinQuery = em.createNamedQuery("PsychoLeistung.code", PsychoLeistung.class);
		gtinQuery.setParameter("code", code);
		List<PsychoLeistung> resultList = gtinQuery.getResultList();
		if (resultList.size() > 0) {
			Optional<Identifiable> element = ArzttarifeModelAdapterFactory.getInstance()
					.getModelAdapter(resultList.get(0), IPsychoLeistung.class, false);
	...
	public Optional<Identifiable> loadFromString(String storeToString) {
		if (storeToString.startsWith(
			...
			String id = split[1];
			EntityManager em = (EntityManager) entityManager.getEntityManager();
			EntityWithId dbObject = em.find(PsychoLeistung.class, id);
	...
	public List<Identifiable> loadFromStringWithIdPart(String partialStoreToString) {
		...
		if (split != null && split.length == 2) {
			String id = split[1];
			Class<? extends EntityWithId> clazz = ch.elexis.core.jpa.entities.PsychoLeistung.class;
			EntityManager em = (EntityManager) entityManager.getEntityManager();
	...
	public Class<?> getEntityForType(String type) {
		if (ch.elexis.base.ch.arzttarife.psycho.model.PsychoLeistung.STS_CLASS.equals(type)) {
			return ch.elexis.core.jpa.entities.PsychoLeistung.class;
		}
		return null;
	}
	...
	public String getTypeForEntity(Object entityInstance) {
		if (entityInstance instanceof ch.elexis.core.jpa.entities.PsychoLeistung) {
			return ch.elexis.base.ch.arzttarife.psycho.model.PsychoLeistung.STS_CLASS;
		}
		return null;
	}
