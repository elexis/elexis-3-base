...
public class TextLogitStream extends TupleStream implements Expressible {
	...
	public TextLogitStream(String zkHost, String collectionName, @SuppressWarnings({ "rawtypes" }) Map params,
			String name, String field, TupleStream termsStream, List<Double> weights, String outcome, int positiveLabel,
			double threshold, int maxIterations) throws IOException {

		init(collectionName, zkHost, params, name, field, termsStream, weights, outcome, positiveLabel, threshold,
	...
	public TextLogitStream(StreamExpression expression, StreamFactory factory) throws IOException {
		...
		init(collectionName, zkHost, params, name, feature, stream, weights, outcomeParam, positiveLabel, threshold,
				maxIterations, iteration);
	}
	...
	private void init(String collectionName, String zkHost, @SuppressWarnings({ "rawtypes" }) Map params, String name,
			String feature, TupleStream termsStream, List<Double> weights, String outcome, int positiveLabel,
			double threshold, int maxIterations, int iteration) throws IOException {
		this.zkHost = zkHost;
	...
	public void open() throws IOException {
		...
		this.executorService = ExecutorUtil
				.newMDCAwareCachedThreadPool(new SolrNamedThreadFactory("TextLogitSolrStream"));
	}
	...
	private List<Future<Tuple>> callShards(List<String> baseUrls) throws IOException {
		...
		for (String baseUrl : baseUrls) {
			LogitCall lc = new LogitCall(baseUrl, this.params, this.field, this.terms, this.weights, this.outcome,
					this.positiveLabel, this.learningRate, this.iteration);
			...
			if (++iteration > maxIterations) {
				...
				if (this.idfs == null) {
					...
					if (weights != null && terms.size() + 1 != weights.size()) {
						throw new IOException(String.format(Locale.ROOT,
								"invalid expression %s - the number of weights must be %d, found %d", terms.size() + 1,
								weights.size()));
					}
				}
	...
	protected class LogitCall implements Callable<Tuple> {
		...
		public LogitCall(String baseUrl, Map<String, String> paramsMap, String feature, List<String> terms,
				List<Double> weights, String outcome, int positiveLabel, double learningRate, int iteration) {
