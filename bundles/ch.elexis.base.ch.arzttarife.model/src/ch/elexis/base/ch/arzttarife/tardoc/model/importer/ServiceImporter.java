	...
	public IStatus doImport(IProgressMonitor ipm) throws SQLException, IOException {
		...
		try {
			...
			while (res.next()) {
				...
				TardocLeistung tl = EntityUtil.load(id, TardocLeistung.class);
				if (tl != null) {
					...
					extensionMap.put(TardocLeistung.EXT_FLD_HIERARCHY_SLAVES, slaves);

					// get LEISTUNG_GRUPPEN
					String groups = getGroups();
					extensionMap.put(TardocLeistung.EXT_FLD_SERVICE_GROUPS, groups);

					// no get LEISTUNG_BLOECKE in TARDOC

					// get LEISTUNG_ALTER
					String age = getAge();
					extensionMap.put(TardocLeistung.EXT_FLD_SERVICE_AGE, age);
		...
		try {
			try (ResultSet res = subStm
					.query(String.format("SELECT * FROM %sLEISTUNG_KUMULATION WHERE LNR_MASTER='%s' AND ART_MASTER='L'",
							TardocReferenceDataImporter.ImportPrefix, code))) {
	...
	private String[] getTexts() throws SQLException, IOException {
		...
		try {
			ResultSet rsub = subStm.query(String.format("SELECT * FROM %sLEISTUNG_TEXT WHERE SPRACHE='%s' AND LNR='%s'",
					TardocReferenceDataImporter.ImportPrefix, lang, code)); // $NON-NLS-1$
			List<Map<String, String>> validResults = ImporterUtil.getValidValueMaps(rsub, validFrom);
			if (!validResults.isEmpty()) {
