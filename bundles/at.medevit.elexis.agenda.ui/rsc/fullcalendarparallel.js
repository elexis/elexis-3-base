...
ParallelView = View.extend({
  ...
  renderDates: function () {
    ...
    // var resourceGridAllDayEl = $('<div class="fc-day-grid" />');
    // var resourceGridAllDayEl2 = $('<div class="fc-row fc-week fc-widget-content" />');
    // this.el.find('.fc-body > tr > td').append(resourceGridAllDayEl);
    // this.el.find('.fc-day-grid').append(resourceGridAllDayEl2);
    ...
    this.bottomRuleEl = $('<hr class="fc-divider ' + this.widgetHeaderClass + '"/>').appendTo(this.resourceGrid.el); // inject it into the time-grid
  ...
  registerRightclickListener: function () {
    ...
    this.el.on('contextmenu', function (ev) {
      var fcContainer = $(ev.target).closest(
        '.fc-bg, .fc-slats, .fc-content-skeleton, ' + '.fc-bgevent-skeleton, .fc-highlight-skeleton'
      );
      ...
      if (cell) {
        that.triggerRightClick(that.getHitSpan(hit), that.getHitEl(hit), ev);
      }
    });
  },
  ...
  renderHead: function () {
    this.headContainerEl = this.el.find('.fc-head-container').html(this.resourceGrid.renderHeadHtml());
  },
