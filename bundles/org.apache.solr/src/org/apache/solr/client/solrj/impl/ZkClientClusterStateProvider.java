...
public class ZkClientClusterStateProvider implements ClusterStateProvider {
	 ...
	 *
	 * NOTE: You should only allow trusted users to upload configs. If you are
	 * allowing client access to zookeeper, you should protect the /configs node
	 * against unauthorised write access.
	 *
	...
	@Override
	public void connect() {
		// Esentially a No-Op, but force a check that we're not closed and the
		// ZkStateReader is available...
			...
			synchronized (this) {
				...
				if (zkStateReader == null) {
					...
					try {
						...
						if (zk != null)
							zk.close();
						// do not wrap because clients may be relying on the underlying exception being
						// thrown
						throw e;
					}
				}
	...
	public void close() throws IOException {
		synchronized (this) {
			if (false == isClosed && zkStateReader != null) {
				...
				// force zkStateReader to null first so that any parallel calls drop into the
				// synch block
