...
public abstract class RecursiveTemporalEvaluator extends RecursiveEvaluator implements OneValueWorker {
	...
	public RecursiveTemporalEvaluator(StreamExpression expression, StreamFactory factory, String functionName)
			throws IOException {
		super(expression, factory);
		this.functionName = functionName;

		if (1 != containedEvaluators.size()) {
			throw new IOException(String.format(Locale.ROOT, "Invalid expression %s - expecting one value but found %d",
	...
	public Object normalizeInputType(Object value) throws StreamEvaluatorException {
		...
		// There is potential for various types to be coming in which are valid temporal
		// values.
		// The order in which we check these types is actually somewhat critical in how
		// we handle them.
			...
			if (!valueStr.isEmpty()) {
				try {
					...
					throw new UncheckedIOException(new IOException(String.format(Locale.ROOT,
							"Invalid parameter %s - The String must be formatted in the ISO_INSTANT date format.",
							valueStr)));
				}
			}
		} else if (value instanceof List) {
			// for each list value, recurse in
			return ((List<?>) value).stream().map(innerValue -> normalizeInputType(innerValue))
	...
	public Object doWork(Object value) {
		if (null == value) {
			...
			try {
				...
				throw new UncheckedIOException(new IOException(String.format(Locale.ROOT,
						"It is not possible to call '%s' function on %s", getFunction(), value.getClass().getName())));
			}
		}
	}
}
