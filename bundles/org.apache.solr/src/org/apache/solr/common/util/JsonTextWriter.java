...
public interface JsonTextWriter extends TextWriter {
	...
	/**
	 * Represents a NamedList directly as a JSON Object (essentially a Map) Map null
	 * to "" and name mangle any repeated keys to avoid repeats in the output.
	 */
	default void writeNamedListAsMapMangled(String name, @SuppressWarnings({ "rawtypes" }) NamedList val)
			throws IOException {
		int sz = val.size();
		writeMapOpener(sz);
		incLevel();
	 ...
	 * repeating any keys if they are repeated in the NamedList. null key is mapped
	 * to "".
	 */
	// NamedList("a"=1,"bar"="foo",null=3,null=null) =>
	// {"a":1,"bar":"foo","":3,"":null}
	default void writeNamedListAsMapWithDups(String name, @SuppressWarnings({ "rawtypes" }) NamedList val)
			throws IOException {
		int sz = val.size();
		writeMapOpener(sz);
		incLevel();
	...
	// Represents a NamedList directly as an array of JSON objects...
	// NamedList("a"=1,"b"=2,null=3,null=null) =>
	// [["a",1],["b",2],[null,3],[null,null]]
