...
public class Replica extends ZkNodeProps {
	...
	/**
	 * The replica's state. In general, if the node the replica is hosted on is not
	 * under {@code /live_nodes} in ZK, the replica's state should be discarded.
	 */
	public enum State {
		 ...
		 * <p>
		 * <b>NOTE</b>: when the node the replica is hosted on crashes, the replica's
		 * state may remain ACTIVE in ZK. To determine if the replica is truly active,
		 * you must also verify that its {@link Replica#getNodeName() node} is under
		 * {@code /live_nodes} in ZK (or use
		...
		/**
		 * The first state before {@link State#RECOVERING}. A node in this state should
		 * be actively trying to move to {@link State#RECOVERING}.
		 * <p>
		 * <b>NOTE</b>: a replica's state may appear DOWN in ZK also when the node it's
		 * hosted on gracefully shuts down. This is a best effort though, and should not
		 * be relied on.
		 * </p>
		 */
		DOWN("D"),
		...
		/**
		 * The node is recovering from the leader. This might involve peer-sync, full
		 ...
		 * {@code /live_nodes} in ZK; in that case the node is not part of the cluster
		 * and it's state should be discarded.
		 * </p>
		 */
		RECOVERY_FAILED("F");
	...
	public enum Type {
		/**
		 * Writes updates to transaction log and indexes locally. Replicas of type
		 * {@link Type#NRT} support NRT (soft commits) and RTG. Any {@link Type#NRT}
		 * replica can become a leader. A shard leader will forward updates to all
		 * active {@link Type#NRT} and {@link Type#TLOG} replicas.
		 */
		NRT,
		/**
		 * Writes to transaction log, but not to index, uses replication. Any
		 * {@link Type#TLOG} replica can become leader (by first applying all local
		 * transaction log elements). If a replica is of type {@link Type#TLOG} but is
		 * also the leader, it will behave as a {@link Type#NRT}. A shard leader will
		 * forward updates to all active {@link Type#NRT} and {@link Type#TLOG}
		 * replicas.
		 */
		TLOG,
		/**
		 * Doesn’t index or writes to transaction log. Just replicates from
		 * {@link Type#NRT} or {@link Type#TLOG} replicas. {@link Type#PULL} replicas
		 * can’t become shard leaders (i.e., if there are only pull replicas in the
		 * collection at some point, updates will fail same as if there is no leaders,
		 * queries continue to work), so they don’t even participate in elections.
		 */
		PULL;
