...
public class LabOrderActionHl7 extends Action {
	...
	public void run() {
		...
		if (patient != null && costBearer != null) {
			...
			filenamePath = writeHL7File(patient, rechnungsempfaenger, costBearer, plan, beginDate, vnr, orderNr);
		...
		if (ok) {
			...
			MessageDialog.openInformation(Hub.getActiveShell(), Messages.LabOrderAction_infoTitleLabOrderFinshed,
					MessageFormat.format(Messages.LabOrderAction_infoMessageLabOrderFinshed, orderNrText, patLabel,
							filenamePath));
		}
	}
	...
	private String writeHL7File(final Patient patient, final Kontakt rechnungsempfaenger, final Kontakt kostentraeger,
			final String plan, final Date beginDate, final String vnr, final long orderNr) {
