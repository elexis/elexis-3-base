 ...
 * Instances of this class communicate with Zookeeper to discover Solr endpoints
 * for SolrCloud collections, and then use the {@link LBHttp2SolrClient} to
 * issue requests.
 *
...
public class CloudHttp2SolrClient extends BaseCloudSolrClient {
	...
	public static class Builder {
		 ...
		 * {@link CloudHttp2SolrClient} instances. The solr client will use these urls
		 * to understand the cluster topology, which solr nodes are active etc.
		 *
		 ...
		 * {@link CloudHttp2SolrClient} instances.
		 *
		...
		/**
		 * Tells {@link CloudHttp2SolrClient.Builder} that created clients should send
		 * direct updates to shard leaders only.
		 *
		...
		/**
		 * Tells {@link CloudHttp2SolrClient.Builder} that created clients can send
		 * updates to any shard replica (shard leaders and non-leaders).
		 *
		...
		/**
		 * Tells {@link CloudHttp2SolrClient.Builder} whether created clients should
		 * send shard updates serially or in parallel
		 *
		 * When an {@link UpdateRequest} affects multiple shards,
		 * {@link CloudHttp2SolrClient} splits it up and sends a request to each
		 * affected shard. This setting chooses whether those sub-requests are sent
		 ...
		 * Http2SolrClient using this builder (instead of the empty default one).
		 * Providing this builder allows users to configure the internal clients
		 * (authentication, timeouts, etc).
		 *
		 * @param internalClientBuilder the builder to use for creating the internal
		 *                              http client.
		 * @return this
		 */
