...
public class ArtikelstammImporterTest {
	...
	public static void beforeClass(){
		...
		artikelstammCodeElements =
			codeElementService.getContribution(CodeElementTyp.ARTICLE, "Artikelstamm").get();
	}
	...
	private void runImport(String baseName) throws ParseException{
		...
		Optional<ICodeElement> nonPharma =
			artikelstammCodeElements.loadFromCode(gtinNonPharma);
		
		assertFalse(nonPharma.isPresent());
		IStatus success = importer.performImport(new NullProgressMonitor(),
			ArtikelstammImporterTest.class.getResourceAsStream(baseName + "_first_v5.xml"), true,
			false, null);
		if (!success.isOK()) {
		...
		Optional<ICodeElement> onlyInSecond =
			artikelstammCodeElements.loadFromCode(gtinOnlyInSecond);
		...
		assertTrue(
			artikelstammCodeElements.loadFromCode(gtinNonPharmaInactiveInSecond).isPresent());
		assertFalse(((IArtikelstammItem) artikelstammCodeElements
			.loadFromCode(gtinNonPharmaInactiveInSecond).get()).isBlackBoxed());
		...
		Optional<ICodeElement> item7digitPhar =
			artikelstammCodeElements.loadFromCode(gtinWithPriceOverridden);
		...
		success = importer.performImport(new NullProgressMonitor(),
			ArtikelstammImporterTest.class.getResourceAsStream(baseName + "_second_v5.xml"), true,
			true, null);
		if (!success.isOK()) {
			String msg =
		...
		Optional<ICodeElement> overridden =
			artikelstammCodeElements.loadFromCode(gtinWithPriceOverridden);
		...
		Optional<ICodeElement> itemWithLeadingZero =
			artikelstammCodeElements.loadFromCode(pharWithLeadingZero);
		assertTrue(itemWithLeadingZero.isPresent());
		assertEquals(pharWithLeadingZero,
			((IArtikelstammItem) itemWithLeadingZero.get()).getPHAR());
		...
		Optional<ICodeElement> pharmaOnlyInFirst =
			artikelstammCodeElements.loadFromCode(gtinPharmaOnlyInFirst);
		...
		Optional<ICodeElement> withPkgOverride =
			artikelstammCodeElements.loadFromCode(gtinWithPkgSizeOverride);
	...
	private IArtikelstammItem setPkgOverride(String gtin){
		IArtikelstammItem overridePkgSize =
			(IArtikelstammItem) artikelstammCodeElements.loadFromCode(gtin).get();
		assertFalse(overridePkgSize.isUserDefinedPrice());
		// Now Override PkgSize
		int old_pkg_size = overridePkgSize.getPackageSize();
		log.debug(
			"VerpackungsEinheit after first import {} getVerpackungsEinheit {} via FLD_PKG_SIZE '{}'",
			gtinWithPkgSizeOverride, old_pkg_size,
			overridePkgSize.getPackageSize());
		assertFalse(overridePkgSize.isUserDefinedPrice());
	...
	private static void checkResettingPrice(String gtin, Money expectedPrice){
		IArtikelstammItem item =
			(IArtikelstammItem) artikelstammCodeElements.loadFromCode(gtin).get();
		log.debug("checkResettingPrice: phar {} gtin {} user {} old {} before restoring {}",
			item.getPHAR(), item.getGtin(), expectedPrice, item.getSellingPrice());
		item.restoreOriginalSellingPrice();
		log.debug("checkResettingPrice: phar {} gtin {} old {} restored {}", item.getPHAR(),
	...
	private static void checkResettingVerpackungsEinheit(String gtin, int expectedPkgSize){
		IArtikelstammItem item =
			(IArtikelstammItem) artikelstammCodeElements.loadFromCode(gtin).get();
		log.debug("checkResettingVerpackungsEinheit: phar {} gtin {} old {} before restoring {}",
			item.getPHAR(), item.getGtin(), expectedPkgSize, item.getSellingPrice());
		item.restoreOriginalPackageSize();
		int newEinheit = item.getPackageSize();
		log.debug("checkResettingVerpackungsEinheit: phar {} gtin {} old {} restored {}",
			item.getPHAR(), item.getGtin(), expectedPkgSize, newEinheit);
