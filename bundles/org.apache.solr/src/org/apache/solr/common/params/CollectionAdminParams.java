...
public interface CollectionAdminParams {
	 ...
	 * creation or restore operation).
	 */
	String CREATE_NODE_SET_PARAM = "createNodeSet";

	/**
	 * A parameter which specifies if the provided list of Solr nodes (via
	 * {@linkplain #CREATE_NODE_SET_PARAM}) should be shuffled before being used.
	 */
	...
	/**
	 * This constant defines the index backup strategy based on copying index files
	 * to desired location.
	 */
	String COPY_FILES_STRATEGY = "copy-files";

	/**
	 * This constant defines the strategy to not copy index files (useful for
	 * meta-data only backup).
	 */
	...
	/**
	 * The reverse-link to WITH_COLLECTION flag. It is stored in the cluster state
	 * of the `withCollection` and points to the collection on which the
	 * `withCollection` was specified.
	 */
	String COLOCATED_WITH = "COLOCATED_WITH";

	/**
	 * Used by cluster properties API as a wrapper key to provide defaults for
	 * collection, cluster etc.
	 *
	...
	/**
	 * Cluster wide defaults can be nested under this key e.g. {defaults:
	 * {cluster:{useLegacyReplicaAssignment:false}}}
	 */
	String CLUSTER = "cluster";

	/**
	 * This cluster property decides whether Solr should use the legacy round-robin
	 * replica placement strategy or the autoscaling policy based strategy to assign
	 * replicas to nodes. The default is false.
	 */
