...
public class MinimaxCanvas extends CstCanvas {
	...
	void paintControl(PaintEvent e) {
		...
		// int xOff3 = 520;
		/*
		 * int xOff2 = 200; int xOff3 = 400; int xOff4 = 600;
		 */
		int xOff2 = 170;
		int xOff3 = 340;
		int xOff4 = 510;
		...
		/*
		 * gc.drawLine(260, 0, 260, iPixY); gc.drawLine(500, 0, 500, iPixY);
		 */
		...
		gc.drawText("Zeitspanne \r\n" + "von\t" + CstService.getGermanFromDate(finding.getDateStartOfSpan3())
				+ "\r\nbis\t" + CstService.getGermanFromDate(finding.getDateEndOfSpan3()) + ": ", xOff1 + 10, 30);

		if (finding.getMaxOfSpan3() == -1) {
			gc.drawText("Maximum:\t keine Werte", xOff1 + 10, 80, true);

		} else {
			gc.drawText("Maximum:\t " + finding.getMaxOfSpan3(), xOff1 + 10, 80, true);
		}
		...
		if (finding.getMinOfSpan3() == -1) {
			...
			gc.drawText("Minimum:\t " + finding.getMinOfSpan3(), xOff1 + 10, 96, true);
		}

		gc.setForeground(ORANGE);

		gc.drawText("Zeitspanne \r\n" + "von\t" + CstService.getGermanFromDate(finding.getDateStartOfSpan2())
				+ "\r\nbis\t" + CstService.getGermanFromDate(finding.getDateEndOfSpan2()) + ": ", xOff2 + 10, 30);

		if (finding.getMaxOfSpan2() == -1) {
			gc.drawText("Maximum:\t keine Werte", xOff2 + 10, 80, true);
		} else {
			gc.drawText("Maximum:\t " + finding.getMaxOfSpan2(), xOff2 + 10, 80, true);
		}

		if (finding.getMinOfSpan2() == -1) {
			gc.drawText("Minimum:\t keine Werte", xOff2 + 10, 96, true);
		} else {
			gc.drawText("Minimum:\t " + finding.getMinOfSpan2(), xOff2 + 10, 96, true);
		}

		gc.setForeground(BLACK);

		gc.drawText("Zeitspanne \r\n" + "von\t" + CstService.getGermanFromDate(finding.getDateStartOfSpan1())
				+ "\r\nbis\t" + CstService.getGermanFromDate(finding.getDateEndOfSpan1()) + ": ", xOff3 + 10, 30);

		if (finding.getMaxOfSpan1() == -1) {
			gc.drawText("Maximum:\tkeine Werte", xOff3 + 10, 80, true);

		} else {
			gc.drawText("Maximum:\t " + finding.getMaxOfSpan1(), xOff3 + 10, 80, true);
		}
		...
		if (finding.getMinOfSpan1() == -1) {
			...
			gc.drawText("Minimum:\t " + finding.getMinOfSpan1(), xOff3 + 10, 96, true);
		}

		/*
		 * StyledText styledText = new StyledText(this, SWT.NONE);
		 * styledText.setText(finding.getAbstract()); styledText.setWordWrap(true);
		 * styledText.setSize(200, 300); gc.drawText(styledText.getText(), xOff3 + 210,
		 * 20, true);
		 */
		...
		// textStyle.
		/*
		 * gc.drawText(finding.getAbstract(), xOff3 + 210, 20, true);
		 */
