 ...
 * external source must be an CSV or Excel(tm) file with the fields:
 * parentCode,codeID,codeName,cost,price,time,validFrom,validUntil,factor
 * <ul>
 * <li>parentCode: If this code system is organized as tree: codeID of the
 * parent or 'NIL' if this is a top level code. If this code system is a flat
 * list, parentCode is always 'NIL'</li>
 * <li>codeID: The (within this code system unique) identification of the code.
 * e.g. 10.00.01 for al Tarmed-style system</li>
 * <li>codeName: the human understandable name of the code</li>
 * <li>cost: the internal cost of this service (what do we have to pay for it),
 * in cents/Rp</li>
 * <li>price: the external cost of this service (what have cliebnts to pay us
 * for it), in cents/Rp</li>
 * <li>time: the average time needed (in minutes) for this service
 * <li>validFrom: the date as YYYYMMDD when this code with this price starts
 * being valid</li>
 * <li>validUntil: the date as YYYYMMDD when the validity of this entry
 * expires</li>
 * <li>factor: a multiplicator to apply to the price before calculating the
 * end-user-price. Sometimes referred to as "Taxpunktwert"</li>
 * </ul>
 *
 * Please note: the codeID needs not to be a unique key. instead we might have
 * several entries with the same codeID, but different validity-dates and
 * prices/factors. At a given date however, only one ellement of each code
 * should be valid.
 *
 *
...
public class Importer extends ImporterPage {
	...
	public IStatus doImport(final IProgressMonitor monitor) throws Exception {
		...
		if (!file.canRead()) {
			...
			return new Status(Status.ERROR, PreferenceConstants.PLUGIN_ID, "Can't read " + results[0]);
		}
		Result<String> res;
		if (results[0].endsWith(".xls")) {
			...
			return new Status(Status.ERROR, PreferenceConstants.PLUGIN_ID, "Unsupported file format");
		}
		if (res.isOK()) {
	...
	private Result<String> importExcel(final String file, final IProgressMonitor mon) {
		...
		xl.setFieldTypes(new Class[] { String.class, String.class, String.class, Integer.class, Integer.class,
				Integer.class, TimeTool.class, TimeTool.class, Double.class });
	...
	private Result<String> importCSV(final String file, final IProgressMonitor mon) {
		try {
			...
			ExHandler.handle(ex);
			return new Result<String>(Result.SEVERITY.ERROR, 1, "Could not read " + file, ex.getMessage(), true);
		}
	...
	private void importLine(final String[] line) {
		if (line.length == 8) {
			...
			if (res.size() > 0) {
				...
				// new Leistung(null, line[0], line[2], line[1], line[3], line[4], line[5],
				// line[6],
				// line[7]);
			}
