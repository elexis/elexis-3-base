...
/**
 * Universell einsetzbare Klasse für alle Laborplugins, welche die HL7 Dateien via MedTransfer von
 * OpenMedical abholen. Die Klasse bietet eine statische Methode für den Download an. Das Ini File
 * wird automatisch ins Temp Directory extrahiert und von dort angewendet. Die Laborimporter Plugins
 * müssen das Ini damit nicht mehr zwingend in den Einstellungen verlangen. Damit wird der
...
public class MedTransfer {
	...
	/**
	 * Ruft den JMedTransferO von OpenMedical auf. Damit werden HL7 Dateien abgeholt.
	 * 
	 * @param jarFilePath
	 *            Voller Pfad und Dateiname auf das JMedTransferO.jar
	 * @param downloadDir
	 *            Verzeichnis, in welches die Dateien heruntergeladen werden sollen
	 * @param params
	 *            Benutzerspezifische Parameter für den JMedTransfer (z.B. -allInOne bei
	 *            Gruppenpraxen)
	...
	public static int doDownload(String jarFilePath, String downloadDir, String params){
			...
			if (jar.canRead()) {
				...
				try {
					URLClassLoader urlLoader =
						getURLClassLoader(new URL("file", null, jar.getAbsolutePath())); //$NON-NLS-1$
		...
		try {
			...
			String[] systemParams = new String[] {
				"--download", downloadDir, "--logPath", downloadDir, "--ini", iniFilename, //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				"--verbose", "INF", "-#OpenMedicalKey#" //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			};
	...
	/**
	 * Extrahiert das Ini File aus dem Jar ins temp. Directory des Systems. Getestet auf Windows.
	 * Linux und Mac noch ohne Gewähr.
	 * 
	 * @param jarFilePath
	 *            Voller Pfad und Dateiname des JMedTransferO.jar
