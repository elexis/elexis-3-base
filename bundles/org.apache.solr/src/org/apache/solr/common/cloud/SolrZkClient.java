...
public class SolrZkClient implements Closeable {
	...
	private final ExecutorService zkCallbackExecutor = ExecutorUtil
			.newMDCAwareCachedThreadPool(new SolrNamedThreadFactory("zkCallback"));
	private final ExecutorService zkConnManagerCallbackExecutor = ExecutorUtil
			.newMDCAwareSingleThreadExecutor(new SolrNamedThreadFactory("zkConnectionManagerCallback"));
	...
	public SolrZkClient(String zkServerAddress, int zkClientTimeout, ZkClientConnectionStrategy strat,
	...
	public SolrZkClient(String zkServerAddress, int zkClientTimeout, int clientConnectTimeout,
			ZkClientConnectionStrategy strat, final OnReconnect onReconnect, BeforeReconnect beforeReconnect,
			ZkACLProvider zkACLProvider, IsClosed higherLevelIsClosed) {
		...
		connManager = new ConnectionManager("ZooKeeperConnection Watcher:" + zkServerAddress, this, zkServerAddress,
				strat, onReconnect, beforeReconnect, new IsClosed() {
		...
		try {
			strat.connect(zkServerAddress, zkClientTimeout, wrapWatcher(connManager), zooKeeper -> {
	...
	protected ZkCredentialsProvider createZkCredentialsToAddAutomatically() {
		...
		if (!StringUtils.isEmpty(zkCredentialsProviderClassName)) {
			try {
				...
				return (ZkCredentialsProvider) Class.forName(zkCredentialsProviderClassName).getConstructor()
						.newInstance();
			} catch (Throwable t) {
	...
	/**
	 * Wraps the watcher so that it doesn't fire off ZK's event queue. In order to
	 * guarantee that a watch object will only be triggered once for a given
	 * notification, users need to wrap their watcher using this method before
	...
	public String create(final String path, final byte[] data, final CreateMode createMode, boolean retryOnConnLoss)
			throws KeeperException, InterruptedException {
		if (retryOnConnLoss) {
			return zkCmdExecutor
					.retryOperation(() -> keeper.create(path, data, zkACLProvider.getACLsToAdd(path), createMode));
		} else {
	...
	public void makePath(String path, File file, boolean failOnExists, boolean retryOnConnLoss)
		...
		makePath(path, FileUtils.readFileToByteArray(file), CreateMode.PERSISTENT, null, failOnExists, retryOnConnLoss,
				0);
	}
	 ...
	 * group, node exist, each will be created.
	 *
	 * skipPathParts will force the call to fail if the first skipPathParts do not
	 * exist already.
	 *
	 * Note: retryOnConnLoss is only respected for the final node - nodes before
	 * that are always retried on connection loss.
	 */
	public void makePath(String path, byte[] data, CreateMode createMode, Watcher watcher, boolean failOnExists,
			boolean retryOnConnLoss, int skipPathParts) throws KeeperException, InterruptedException {
		...
		for (int i = 0; i < paths.length; i++) {
			...
			try {
				...
				// in auth cases, we may not have permission for an earlier part of a path,
				// which is fine
	...
	/**
	 * Check to see if a Throwable is an InterruptedException, and if it is, set the
	...
	/**
	 * Gets the raw config node /zookeeper/config as returned by server. Response
	 * may look like
	 *
	 * <pre>
	 ...
	 * </pre>
	 *
	 * @return Multi line string representing the config. For standalone ZK this
	...
	/**
	 * Set the ACL on a single node in ZooKeeper. This will replace all existing ACL
	 * on that node.
	 *
	 * @param path            path to set ACL on e.g. /solr/conf/solrconfig.xml
	 * @param acls            a list of {@link ACL}s to be applied
	 * @param retryOnConnLoss true if the command should be retried on connection
	 *                        loss
	...
	// Some pass-throughs to allow less code disruption to other classes that use
	// SolrZkClient.
	...
	/**
	 * Watcher wrapper that ensures that heavy implementations of process do not
	 * interfere with our ability to react to other watches, but also ensures that
	 * two wrappers containing equal watches are considered equal (and thus we won't
	 * accumulate multiple wrappers of the same watch).
	 */
	private final class ProcessWatchWithExecutor implements Watcher { // see below for why final.
		...
		// These overrides of hashcode/equals ensure that we don't store the same exact
		// watch
		...
		// cases with sub-classes, if you need different behavior, find a new class or
		// make
		// sure you account for the case where two diff sub-classes with different
		// behavior
