...
public class XMLExporterProcessing {
	...
	public static XMLExporterProcessing buildProcessing(IInvoice invoice, XMLExporter xmlExporter){
		...
		transport.setAttribute(ATTR_TRANSPORT_TO, getRecipientEAN(invoice, xmlExporter));

		logger.debug("Using intermediate EAN [" + getIntermediateEAN(invoice, xmlExporter) + "]");
		Element via = new Element(ELEMENT_TRANSPORT_VIA, XMLExporter.nsinvoice);
		via.setAttribute(ATTR_TRANSPORT_VIA_VIA, getIntermediateEAN(invoice, xmlExporter));
		...
		if (Tiers.GARANT == tiersType && (TarmedRequirements.hasTCContract(actMandant))) {
			...
			demand.setAttribute("tc_token", //$NON-NLS-1$
				xmlExporter.getBesr().createCodeline(
					XMLTool.moneyToXmlDouble(xmlExporter.getDueMoney()).replaceFirst("[.,]", ""), //$NON-NLS-1$//$NON-NLS-2$
					tcCode));
