 ...
 * ch.elexis.data.PersistentObject, and thereby the management of the object persistence is
 ...
 * Einige Methoden _müssen_ vorhanden sein, damit eine von PersistentObject abgeleitete Klasse
...
public class Messwert extends PersistentObject {
	 ...
	 * avoid naming conflicts. Unfortunately, this plugin was created before that convention was
	 * declared and therefore the tablename ist not quite correct. We leave it for compatibility
	 * reasons. If this plugin was made today, the name of the table ought to be something like
	 * CH_ELEXIS_BEFUNDE_MESSWERTE
	 * 
	 * Name der Tabelle. Die Konvention sieht jetzt vor, dass jede Tabelle mit der Präfix ihres
	 * Plugins benannt werden muss, um Namenskonflikte zu vermeiden. Wir belassen hier jetzt dennoch
	 * diesen eigentlich unerwünschten Namen, um existierende Datenbanken nicht zu zerstören. Würde
	 * dieses Plugin jetzt neu erstellt, müsste seine Tabelle heissen: CH_ELEXIS_BEFUNDE_MESSWERTE
	 * (oder so ähnlich)
	 */
	private static final String TABLENAME = "ELEXISBEFUNDE"; //$NON-NLS-1$
	
	/**
	 * -required- Here we define the mapping of table fields to class members. The first String has
	 * always to be the name of the table, the other Strings are table fields, that we need as
	 * members. This mapping makes ist possible to access every declared field simply by e.g.
	 * get("Name") and set("Name","blabla") instead of having to fiddle around with SQL-statements.
	 * Elexis convention encourrages every plugin developer strongly to use this mechanism, because
	 * this ensures also the ability of elexis to run without change on different database engines.
	 * 
	 * The declaration of a mapping can be a simple String (as below in "Name"). In that case, the
	 * name of the field is identical to the name of the member. It could as well be a modification
	 * of the form "Name=TBL_USR" In that case, the member "Name" would be mapped to the table field
	 * "TBL_USR". And the declaration can also be of the form "Datum=S:D:Datum". In that case,
	 * PersistentObject modifies the field before passing it to the database. In this case, every
	 * Date will be converted to the form yyyymmdd (TimeTool.DATE_COMPACT). Other modifyers include
	 * compression of fields etc. See documentation of PersistentObject.
	 * 
	 * -Zwingend- Hier wird die Zuordnung von Tabellenfeldern zu Klassenmembern definiert. Der erste
	 * String ist der Name der Tabelle, die weiteren Strings sind Tabellenfelder, die als Member
	 * benötigt werden. Diese können entweder als einfaches Wort aufgeführt werden, dann wird der
	 * Member genauso heissen, wie das Tabellenfeld, und die Daten aus der Tabelle werden als
	 * einfacher Text interpretiert. Oder es kann ein String der Form Name=Feld eingesetzt werden,
	 * dann kann der Member anders heissen, als das Feld, und der Feldinhalt kann beim Lesen udn
	 * Schreiben in bestimmter Weise umgeformt werden (Als Datum interpretiert, Komprimiert etc).
	 ...
	 * 
	 * @param pat
	 *            Der Patient, dem dieser Messwert zugeordnet werden soll
	 * @param name
	 *            Name des Messwerts
	 * @param date
	 *            Datum des Messwerts
	...
	/**
	 * -required- Name of the table, where objects of this class should be persisted. See remarks
	 * above near the field TABLENAME
	 * 
	 * -Zwingend- Name der Tabelle, in der Objekte dieser Klasse persistiert werden sollen
	 */
	@Override
