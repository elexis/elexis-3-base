...
public class SolrClientCache implements Serializable {
	...
	private static final int conTimeout = Math
			.max(Integer.parseInt(System.getProperty(HttpClientUtil.PROP_CONNECTION_TIMEOUT, "60000")), 60000);
	private static final int socketTimeout = Math
			.max(Integer.parseInt(System.getProperty(HttpClientUtil.PROP_SO_TIMEOUT, "60000")), 60000);
	...
	public synchronized CloudSolrClient getCloudSolrClient(String zkHost) {
		...
		if (solrClients.containsKey(zkHost)) {
			...
			CloudSolrClient.Builder builder = new CloudSolrClient.Builder(hosts, Optional.empty())
					.withSocketTimeout(socketTimeout).withConnectionTimeout(conTimeout);
			if (httpClient != null) {
	...
	public synchronized HttpSolrClient getHttpSolrClient(String host) {
		...
		if (solrClients.containsKey(host)) {
			...
			HttpSolrClient.Builder builder = new HttpSolrClient.Builder(host).withSocketTimeout(socketTimeout)
