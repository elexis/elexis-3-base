...
public class CloudSolrStream extends TupleStream implements Expressible {
	...
	void init(String collectionName, String zkHost, SolrParams params) throws IOException {
		...
		// fieldA but fieldA is aliased to alias.fieldA then the comparater will be
	...
	public static Slice[] getSlices(String collectionName, ZkStateReader zkStateReader, boolean checkAlias)
		...
		for (String col : collectionNames) {
			List<String> collections = checkAlias ? zkStateReader.getAliases().resolveAliases(col) // if not an alias,
																									// returns
																									// collectionName
		...
		List<Slice> slices = allCollections.stream().map(collectionsMap::get).filter(Objects::nonNull)
				.flatMap(docCol -> Arrays.stream(docCol.getActiveSlicesArr())).collect(Collectors.toList());
		if (!slices.isEmpty()) {
