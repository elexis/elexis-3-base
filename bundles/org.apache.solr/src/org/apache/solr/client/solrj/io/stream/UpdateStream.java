...
/**
 * Sends tuples emitted by a wrapped {@link TupleStream} as updates to a
...
public class UpdateStream extends TupleStream implements Expressible {
	...
	public static String BATCH_INDEXED_FIELD_NAME = "batchIndexed"; // field name in summary tuple for #docs updated in
																	// batch
	 ...
	 * tuples when converting to Solr Documents. May be set per expression using the
	...
	public UpdateStream(String collectionName, TupleStream tupleSource, String zkHost, int updateBatchSize)
			throws IOException {
	 ...
	 * <code>"pruneVersionField"</code> option. {@link UpdateStream} returns
	 ...
	 * converting each tuple in that batch to a Solr Input Document.
	 */
	protected void uploadBatchToCollection(List<SolrInputDocument> documentBatch) throws IOException {
		...
		try {
			...
			throw new IOException(String.format(Locale.ROOT,
					"Unexpected error when adding documents to collection %s- %s:%s", collection, className, message));
		}
	}
