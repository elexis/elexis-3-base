...
public class StammdatenUebermittelnDialog extends TitleAreaDialog {
	 ...
	 * @param parentShell
	 */
	public StammdatenUebermittelnDialog(Shell parentShell, GDTSatzNachricht6301 gdtSatz, String preSelCommPartner) {
		super(parentShell);
	...
	protected void initDataBindings() {
		...
		int[] property = { GDTConstants.FELDKENNUNG_PATIENT_KENNUNG, GDTConstants.FELDKENNUNG_PATIENT_NAME,
				...
				GDTConstants.FELDKENNUNG_PATIENT_GROESSE, GDTConstants.FELDKENNUNG_PATIENT_MUTTERSPRACHE,
				GDTConstants.FELDKENNUNG_PATIENT_WOHNORT, GDTConstants.FELDKENNUNG_PATIENT_STRASSE,
				GDTConstants.FELDKENNUNG_PATIENT_TITEL, GDTConstants.FELDKENNUNG_PATIENT_VERSICHERTENNUMMER };

		for (int i = 0; i < control.length; i++) {
			bindMapValue(control[i], property[i], bindingContext, gdt6301ValuesObserveMap);
		}

		IObservableValue widgetValueGeschlecht = ViewerProperties.singleSelection().observe(comboViewerGeschlecht);
		IObservableValue observableMapValueGeschlecht = Observables.observeMapEntry(gdt6301ValuesObserveMap,
		...
		IObservableValue observableMapValueVersichertenart = Observables.observeMapEntry(gdt6301ValuesObserveMap,
		...
		IObservableValue observableValueGeburtstag = Observables.observeMapEntry(gdt6301ValuesObserveMap,
