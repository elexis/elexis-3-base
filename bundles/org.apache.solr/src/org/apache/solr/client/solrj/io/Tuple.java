...
/**
 * A simple abstraction of a record containing key/value pairs. Convenience
 * methods are provided for returning single and multiValue String, Long and
 * Double values. Note that ints and floats are treated as longs and doubles
 * respectively.
 *
 **/
...
public class Tuple implements Cloneable, MapWriter {
	...
	/**
	 * When EOF field is true the Tuple marks the end of the stream. The EOF Tuple
	 * will not contain a record from the stream, but it may contain
	 * metrics/aggregates gathered by underlying streams.
	 */
	public boolean EOF;
	/**
	 * When EXCEPTION field is true the Tuple marks an exception in the stream and
	 * the corresponding "EXCEPTION" field contains a related message.
	 */
	...
	/**
	 * This represents the mapping of external field labels to the tuple's internal
	 * field names if they are different from field names.
	 *
	...
	/**
	 * A list of field names to serialize. This list (together with the mapping in
	 * {@link #getFieldLabels()} determines what tuple values are serialized and
	 * their external (serialized) names.
	 *
