	...
	SolrRequest tryEachNode(boolean strict) {
		...
		for (int i1 = 0; i1 < validReplicas.size(); i1++) {
			...
			for (int j = session.matrix.size() - 1; j >= stopAt; j--) {
				...
				if (targetRow.node.equals(fromRow.node))
					continue;
				if (!isNodeSuitableForReplicaAddition(targetRow, fromRow))
					continue;
				targetRow = targetRow.addReplica(ri.getCollection(), ri.getShard(), ri.getType(), strict); // add
																											// replica
																											// to target
																											// first
				Row srcRowModified = targetRow.session.getNode(fromRow.node).removeReplica(ri.getCollection(),
						ri.getShard(), ri.getType());// then remove replica from source node
				...
				if (!containsNewErrors(errs) && isLessSerious(errs, leastSeriousViolation)
					...
					if (!force && srcRowModified.isLive && targetRow.isLive) {
						result = tmpSession.getPolicy().getClusterPreferences().get(0).compare(srcRowModified,
								tmpSession.getNode(targetRow.node), true);
						if (result == 0)
							result = tmpSession.getPolicy().getClusterPreferences().get(0).compare(srcRowModified,
									tmpSession.getNode(targetRow.node), false);
					}
		...
		if (bestSrcRow != null) {
			...
			return new CollectionAdminRequest.MoveReplica(sourceReplicaInfo.getCollection(),
					sourceReplicaInfo.getName(), bestTargetRow.node);
		}
		return null;
	}
