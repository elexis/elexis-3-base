...
public class JSONTupleStream implements TupleStreamParser {
	...
	public Map<String, Object> next() throws IOException {
		...
		// advance past ARRAY_START (in the case that we just advanced to docs, or
		// OBJECT_END left over from the last call.
		int event = parser.nextEvent();
		if (event == JSONParser.ARRAY_END)
			return null;
	...
	private void expect(int parserEventType) throws IOException {
		...
		if (event != parserEventType) {
			throw new IOException("JSONTupleStream: expected " + JSONParser.getEventString(parserEventType)
					+ " but got " + JSONParser.getEventString(event));
		}
	}
