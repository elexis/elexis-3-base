...
public class XChangeContributor implements IExchangeContributor {
	...
	public void importHook(XChangeContainer container, PersistentObject context) {
		...
		for (Element re : records) {
			/*
			 * List<Element> xrefs=re.getChildren("xref", container.getNamespace()); Samdas
			 * smd=new Samdas(k.getEintrag().getHead()); if(xrefs!=null){ for(Element
			 * e:xrefs){ String type=e.getAttributeValue("type");
			 * if(type.equalsIgnoreCase("image/jpeg") ||
			 * type.equalsIgnoreCase("image/png")){ String
			 * content=e.getAttributeValue("content"); String id=e.getAttributeValue("id");
			 * byte[] data=null; if(content.equals("inline")){ BASE64Decoder b64=new
			 * BASE64Decoder(); try { data=b64.decodeBuffer(e.getText()); } catch
			 * (IOException e1) { ExHandler.handle(e1); continue; } }else{
			 * data=container.getBinary(id); } Patient pat=k.getFall().getPatient(); Bild
			 * bild=new Bild(pat,"type",data); String spos=e.getAttributeValue("pos"); int
			 * pos=(spos==null) ? 0 : Integer.parseInt(spos); String
			 * slen=e.getAttributeValue("len"); int len=(slen==null) ? 0 :
			 * Integer.parseInt(slen); Samdas.Record rec=smd.getRecord(); Samdas.XRef xr=new
			 * Samdas.XRef("bildanzeige",bild.getId(),pos,len); rec.add(xr);
