 ...
 * upconfig/downconfig/cp/ls/mv commands in bin/solr it made sense to keep the
 * individual transfer methods in a central place, so here it is.
 */
public class ZkMaintenanceUtils {
	...
	} // don't let it be instantiated, all methods are static.

	/**
	 * Lists a ZNode child and (optionally) the znodes of all the children. No data
	 * is dumped.
	 *
	 ...
	 * @throws SolrServerException  zookeeper node has children and recurse not
	 *                              specified.
	...
	/**
	 * Copy between local file system and Zookeeper, or from one Zookeeper node to
	 * another, optionally copying recursively.
	 *
	 * @param src     Source to copy from. Both src and dst may be Znodes. However,
	 *                both may NOT be local
	 * @param dst     The place to copy the files too. Both src and dst may be
	 *                Znodes. However both may NOT be local
	 * @param recurse if the source is a directory, reccursively copy the contents
	 *                iff this is true.
	 * @throws SolrServerException  Explanatory exception due to bad params, failed
	 *                              operation, etc.
	 * @throws KeeperException      Could not perform the Zookeeper operation.
	 * @throws InterruptedException Thread interrupted
	 */
	public static void zkTransfer(SolrZkClient zkClient, String src, Boolean srcIsZk, String dst, Boolean dstIsZk,
		...
		if (recurse == false) {
			if (srcIsZk) {
				...
				throw new SolrServerException(
						"Local path " + Paths.get(src).toAbsolutePath() + " is a directory and recurse is false");
			}
		}
		...
		// Copying individual files from ZK requires special handling since
		// downloadFromZK assumes the node has children.
	...
	// If the dest ends with a separator, it's a directory or non-leaf znode, so
	// return the
	...
	private static String normalizeDest(String srcName, String dstName, boolean srcIsZk, boolean dstIsZk) {
		...
		if (dstName.endsWith(dstSeparator)) { // Dest is a directory or non-leaf znode, append last element of the src
												// path.
	...
	public static void moveZnode(SolrZkClient zkClient, String src, String dst)
		...
		// Special handling if the source has no children, i.e. copying just a single
		// file.
		if (zkClient.getChildren(src, null, true).size() == 0) {
		...
		// throws error if not all there so the source is left intact. Throws error if
		// source and dest don't match.
		checkAllZnodesThere(zkClient, src, destName);

		clean(zkClient, src);
	}

	// Insure that all the nodes in one path match the nodes in the other as a
	...
	// This not just a copy operation since the config manager takes care of
	// construction the znode path to configsets
	...
	public static void clean(SolrZkClient zkClient, String path, Predicate<String> filter)
		...
		// sort the list in descending order to ensure that child entries are deleted
		// first
	...
	public static void uploadToZK(SolrZkClient zkClient, final Path fromPath, final String zkPath,
		...
		Files.walkFileTree(rootPath, new SimpleFileVisitor<Path>() {
			...
			public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
				...
				try {
					// if the path exists (and presumably we're uploading data to it) just set its
					// data
	...
	private static int copyDataDown(SolrZkClient zkClient, String zkPath, File file)
	...
	public static void downloadFromZK(SolrZkClient zkClient, String zkPath, Path file) throws IOException {
		try {
			...
			// If it has no children, it's a leaf node, write the associated data from the
			// ZNode.
			// Otherwise, continue recursing, but write the associated data to a special
			// file if any
			if (children.size() == 0) {
				// If we didn't copy data down, then we also didn't create the file. But we
				// still need a marker on the local
	...
	public enum VISIT_ORDER {
		VISIT_PRE, VISIT_POST
	}

	/**
	 * Recursively visit a zk tree rooted at path and apply the given visitor to
	 * each path. Exists as a separate method because some of the logic can get
	 * nuanced.
	 *
	 * @param path       the path to start from
	 * @param visitOrder whether to call the visitor at the at the ending or
	 *                   beginning of the run.
	 * @param visitor    the operation to perform on each path
	 */
	public static void traverseZkTree(SolrZkClient zkClient, final String path, final VISIT_ORDER visitOrder,
			final ZkVisitor visitor) throws InterruptedException, KeeperException {
	...
	public static String createZkNodeName(String zkRoot, Path root, Path file) {
		...
		if (relativePath.length() == 0 && isNonLeafData == false)
			return zkRoot;

		// Important to have this check if the source is file:whatever/ and the
		// destination is just zk:/
		if (zkRoot.endsWith("/") == false)
			zkRoot += "/";
