...
public class OccupationalLeistungCodeSelectorFactory extends CodeSelectorFactory {
	...
	public ViewerConfigurer createViewerConfigurer(CommonViewer cv) {
		cv.setSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (!ss.isEmpty()) {
					IOccupationalLeistung selected = (IOccupationalLeistung) ss.getFirstElement();
					ContextServiceHolder.get().getRootContext()
							.setNamed("ch.elexis.views.codeselector.occupational.selection", selected);
				} else {
					ContextServiceHolder.get().getRootContext()
							.setNamed("ch.elexis.views.codeselector.occupational.selection", null);
				}
			}
		});
		FieldDescriptor<?>[] fd = new FieldDescriptor<?>[] {
				new FieldDescriptor<IOccupationalLeistung>("Ziffer", "code", null),
				new FieldDescriptor<IOccupationalLeistung>("Text", "codeText", null) };
		SelectorPanelProvider slp = new SelectorPanelProvider(fd, true);
		vc = new ViewerConfigurer(new OccupationalContentProvider(cv, slp), new DefaultLabelProvider(), slp,
	...
	private class OccupationalContentProvider extends CommonViewerContentProvider {

		private ControlFieldProvider controlFieldProvider;

		public OccupationalContentProvider(CommonViewer commonViewer, ControlFieldProvider controlFieldProvider) {
			super(commonViewer);
		...
		protected IQuery<?> getBaseQuery() {
			IQuery<IOccupationalLeistung> query = ArzttarifeModelServiceHolder.get()
					.getQuery(IOccupationalLeistung.class);
