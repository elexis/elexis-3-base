...
import ch.elexis.views.TardocRefcodesDialog;

public class TardocRefcodesHandler extends AbstractHandler {

	public static String CMDID = "ch.elexis.base.ch.arzttarife.tardoc.refcodes";
		...
		if (shell != null) {
			...
			if (billed.isPresent()) {
				if (billed.get().getBillable() instanceof ITardocLeistung) {
					new TardocRefcodesDialog(shell, billed.get()).open();
				}
			}
		}
		return shell;
