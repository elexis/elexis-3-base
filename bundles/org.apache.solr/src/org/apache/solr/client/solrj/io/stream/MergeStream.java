...
/**
 * Merges two or more streams together ordering the Tuples based on a
 * Comparator. All streams must be sorted by the fields being compared - this
 * will be validated on construction.
 *
 * @since 5.1.0
 **/
public class MergeStream extends TupleStream implements Expressible {
	...
	public MergeStream(StreamExpression expression, StreamFactory factory) throws IOException {
		...
		init(factory.constructComparator(((StreamExpressionValue) onExpression.getParameter()).getValue(),
				FieldComparator.class), streams);
	}
	...
	private void init(StreamComparator comp, TupleStream... streams) throws IOException {
		...
		for (TupleStream stream : streams) {
			if (!comp.isDerivedFrom(stream.getStreamSort())) {
				throw new IOException(
						"Invalid MergeStream - all substream comparators (sort) must be a superset of this stream's comparator.");
			}
		}
	...
	public Tuple read() throws IOException {
		...
		// might be able to optimize this by sorting the streams based on the next to
		// read tuple from each.
		// if we can ensure the sort of the streams and update it in less than linear
		// time then there would
		// be some performance gain. But, assuming the # of streams is kinda small then
		// this might not be
		// worth it
