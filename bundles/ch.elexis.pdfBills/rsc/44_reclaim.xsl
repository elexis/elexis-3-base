...
<xsl:stylesheet
		...
		<fo:root>
			<fo:layout-master-set>
				<fo:simple-page-master master-name="firstPage"
					...
					<fo:region-body margin-top="13.1cm"
						margin-bottom="1.2cm" />
					<fo:region-before extent="13.1cm" />
					<fo:region-after extent="1.2cm"
						display-align="after" />
				</fo:simple-page-master>
				<fo:simple-page-master
					master-name="middlePage" page-height="29.7cm" page-width="21cm"
					margin-top="1cm" margin-bottom="1.3cm" margin-left="1.5cm"
					margin-right="1.4cm">
			...
			<fo:page-sequence master-reference="allPages">
				...
				<fo:static-content flow-name="last-footer">
					<fo:block text-align="right" font-size="7px">
						<fo:block>
							<xsl:call-template name="reclaim_summary_total">
							</xsl:call-template>
						</fo:block>
					</fo:block>
					<fo:block margin-top="2mm">
								...
								<fo:table-row>
									...
									<fo:table-cell>
										<fo:block white-space-treatment="preserve">
											<xsl:choose>
												<xsl:when
													test="/invoice:request/invoice:payload/@copy = '0'">
										...
										<fo:block white-space-treatment="preserve">
											<xsl:choose>
												<xsl:when
													test="count(/invoice:request/invoice:payload/invoice:body/invoice:kvg) > 0">
													<xsl:value-of
														select="/invoice:request/invoice:payload/invoice:body/invoice:kvg/@contract_number" />
												</xsl:when>
												<xsl:when
													test="count(/invoice:request/invoice:payload/invoice:body/invoice:mvg) > 0">
													<xsl:value-of
														select="/invoice:request/invoice:payload/invoice:body/invoice:mvg/@contract_number" />
												</xsl:when>
												<xsl:when
													test="count(/invoice:request/invoice:payload/invoice:body/invoice:ivg) > 0">
													<xsl:value-of
														select="/invoice:request/invoice:payload/invoice:body/invoice:ivg/@contract_number" />
												</xsl:when>
												<xsl:otherwise>
													<xsl:value-of
														select="/invoice:request/invoice:payload/invoice:body/invoice:uvg/@contract_number" />
												</xsl:otherwise>
											</xsl:choose>
											<xsl:value-of select="' '" />
										</fo:block>
	...
	<xsl:template name="reclaim_summary_total">
		<fo:table table-layout="fixed" width="100%">
				...
				<fo:table-row>
					...
					<fo:table-cell>
						<fo:block text-align="left" font-size="7px"
							...
							<xsl:value-of select="' '" />
							<xsl:value-of
								select="/invoice:request/invoice:payload/invoice:body/invoice:balance/invoice:vat/@vat_number" />
							<xsl:value-of select="' '" />
							MWST
						</fo:block>
					</fo:table-cell>
					...
					<fo:table-cell>
						<fo:block text-align="right" font-size="7px"
							...
							<xsl:choose>
								<xsl:when
									test="count(/invoice:request/invoice:payload/invoice:body/invoice:balance/@amount_prepaid) > 0">
									<xsl:value-of
				...
				<fo:table-row>
					...
					<fo:table-cell>
						<fo:block text-align="left" font-size="7px"
							...
							<xsl:value-of select="' '" />
							<xsl:value-of
								select="/invoice:request/invoice:payload/invoice:body/invoice:balance/@currency" />
						</fo:block>
					</fo:table-cell>
					<fo:table-cell>
						<fo:block text-align="left" font-size="7px"
					...
					<fo:table-cell>
						<fo:block text-align="right" font-size="7px"
							font-family="tahoma,arial,helvetica,sans-serif">
							<xsl:value-of
								select="/invoice:request/invoice:payload/invoice:body/invoice:balance/@amount_obligations" />
						</fo:block>
					</fo:table-cell>
				</fo:table-row>
	...
	<xsl:template name="VatAmount">
		...
		<xsl:choose>
			<xsl:when
				test="count(/invoice:request/invoice:payload/invoice:body/invoice:balance/invoice:vat/invoice:vat_rate[@vat_rate=$Rate]) = 0">
				<xsl:variable name="formattedRate">
					<xsl:value-of select="format-number($Rate,'##0.00')" />
				</xsl:variable>
				<xsl:choose>
					<xsl:when
						test="count(/invoice:request/invoice:payload/invoice:body/invoice:balance/invoice:vat/invoice:vat_rate[@vat_rate=$formattedRate]) > 0">
						<xsl:call-template name="FormatNumber">
							<xsl:with-param name="Number"
								select="/invoice:request/invoice:payload/invoice:body/invoice:balance/invoice:vat/invoice:vat_rate[@vat_rate=$formattedRate]/@amount" />
						</xsl:call-template>
					</xsl:when>
					<xsl:otherwise>
			...
			<xsl:otherwise>
				<xsl:call-template name="FormatNumber">
					<xsl:with-param name="Number"
						select="/invoice:request/invoice:payload/invoice:body/invoice:balance/invoice:vat/invoice:vat_rate[@vat_rate=$Rate]/@amount" />
				</xsl:call-template>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template name="VatVat">
		...
		<xsl:choose>
			<xsl:when
				...
				<xsl:choose>
					<xsl:when
						...
						<xsl:call-template name="FormatNumber">
							<xsl:with-param name="Number"
								select="/invoice:request/invoice:payload/invoice:body/invoice:balance/invoice:vat/invoice:vat_rate[@vat_rate=$formattedRate]/@vat" />
						</xsl:call-template>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="'0.00'" />
			...
			<xsl:otherwise>
				<xsl:call-template name="FormatNumber">
					<xsl:with-param name="Number"
						select="/invoice:request/invoice:payload/invoice:body/invoice:balance/invoice:vat/invoice:vat_rate[@vat_rate=$Rate]/@vat" />
				</xsl:call-template>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
