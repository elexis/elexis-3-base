...
public class OOTextPlugin implements ITextPlugin {
		...
		public void run(final IProgressMonitor monitor) {
			...
			try {
				...
				if (this.url == null) { // Empty
					...
					document = (ITextDocument) officeApplication.getDocumentService().loadDocument(officeFrame, url,
	 ...
	 *
	 * @param frame   office frame to be configured
	...
	public synchronized Composite createContainer(Composite parent, ICallback handler) {
		...
		try {
			officeApplication = OOActivator.getDefault().getLocalOfficeApplication(parent.getShell());
		} catch (Exception ex) {
			handleError(ex, true);
			dispose();
		} catch (Error err) {
	...
	public void activateLocalOfficeApplication() throws CoreException, OfficeApplicationException {
		...
		synchronized (this.officeApplication) {
			if (!this.officeApplication.isActive()) {
				...
				IStatus status = NOAUIPlugin.startLocalOfficeApplication(officeParent.getShell(),
						this.officeApplication);
				if (status.getSeverity() == IStatus.ERROR) {
					throw new CoreException(new Status(IStatus.ERROR, "openoffice.prototyp", IStatus.ERROR,
							"Local OpenOffice.org application cannot be activated.", null));
				}
			}
	 ...
	 * Insert a table.
	 *
	 * @param place       A string to search for and replace with the table
	 * @param properties  Properties for the table
	 * @param contents    An Array of String[]s describing each line of the table
	 * @param columnsizes Int-array describing the relative width of each column
	 *                    (all columns together are taken as 100%). May be null, in
	 *                    that case the columns will be spread evenly
	 */
	public synchronized boolean insertTable(String place, int properties, String[][] contents, int[] columnSizes) {
		...
		if (!searchResult.isEmpty()) {
			...
			try {
				ITextTable textTable = this.document.getTextTableService()
						.constructTextTable(contents.length /*
															 * + offset
															 */, contents[0].length);
				this.document.getTextService().getTextContentService().insertTextContent(r, textTable);
				r.setText("");
				...
				for (int i = 0; i < contents[0].length; i++) {
					textTable.getCell(i, 0).setCellParagraphStyle(ITextTableCell.STYLE_TABLE_CONTENT);
				}
				return true;
			} catch (Exception ex) {
				handleError(ex, false);
	...
	public synchronized Object insertTextAt(int x, int y, int w, int h, String text, int adjust) {
		...
		try {
			...
			com.sun.star.lang.XMultiServiceFactory documentFactory = (com.sun.star.lang.XMultiServiceFactory) UnoRuntime
					.queryInterface(com.sun.star.lang.XMultiServiceFactory.class, myDoc);
	...
	/**
	 * Stores current document. If asFilenamePath is null, the document is stored in
	 * the appropiate file (if exists).
	 */
	 ...
	 * NOA does no allow us to select printer and tray, so we do it directly with
	 * UNO.
	 */
	public synchronized boolean print(String toPrinter, String toTray, boolean waitUntilFinished) {
		...
		com.sun.star.view.XPrintable xPrintable = (com.sun.star.view.XPrintable) UnoRuntime
				.queryInterface(com.sun.star.view.XPrintable.class, this.document.getXComponent());
		...
		if (!StringTool.isNothing(toTray)) {
			...
			try {
				...
				String printer = manuallyHookIntoPrintJob(printSettings.getPrinter(), printSettings.getTray());
				if (printer == null) {
					handleError(MessageFormat.format("Fehler beim Auswählen des Druckerfaches {0}", toTray));
					return false;
				}
				toPrinter = printer;
			}
		...
		com.sun.star.view.XPrintJobBroadcaster selection = (com.sun.star.view.XPrintJobBroadcaster) UnoRuntime
				.queryInterface(com.sun.star.view.XPrintJobBroadcaster.class, xPrintable);
	...
	private String manuallyHookIntoPrintJob(String printer, String tray) {
		...
		for (PrintService ps : printers) {
			...
			if (ps.getName().equals(printer)) {
				...
				Object attributes = ps.getSupportedAttributeValues(Media.class, DocFlavor.SERVICE_FORMATTED.PRINTABLE,
						null);
				if (attributes != null && attributes.getClass().isArray()) {
					...
					if (printSrv != null) {
						try {
							...
							if (mediaTray != null) {
								PrintRequestAttributeSet pReqAS = new HashPrintRequestAttributeSet();
	 ...
	 *
	 * @param visible if true then show
	 */
	...
	public void initTemplatePrintSettings(String template) {
		...
		if (document != null && printSettings.getPrinter() != null) {
			try {
				IPrinter printer = document.getPrintService().createPrinter(printSettings.getPrinter());
