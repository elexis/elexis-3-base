...
public class HttpSolrClient extends BaseHttpSolrClient {
	 ...
	 * <p>
	 * Parameters that are added to every request regardless. This may be a place to
	 * add something like an authentication token.
	 */
	...
	/**
	 * @deprecated use {@link HttpSolrClient#HttpSolrClient(Builder)} instead, as it
	 *             is a more extension/subclassing-friendly alternative
	 */
	@Deprecated
	protected HttpSolrClient(String baseURL, HttpClient client, ResponseParser parser, boolean allowCompression) {
		this(new Builder(baseURL).withHttpClient(client).withResponseParser(parser).allowCompression(allowCompression));
	}
	 ...
	 * @param invariantParams  The parameters which should be included with every
	 *                         request.
	 *
	 * @deprecated use {@link HttpSolrClient#HttpSolrClient(Builder)} instead, as it
	 *             is a more extension/subclassing-friendly alternative
	 ...
	 * Expert Method
	 *
	 * @param queryParams set of param keys to only send via the query string Note
	 *                    that the param will be sent as a query string if the key
	 *                    is part of this Set or the SolrRequest's query params.
	 ...
	 * {@link org.apache.solr.client.solrj.SolrRequest#getResponseParser()} is null,
	 * then use {@link #getParser()}
	 *
	 * @param request The {@link org.apache.solr.client.solrj.SolrRequest} to
	 *                process
	...
	public NamedList<Object> request(@SuppressWarnings({ "rawtypes" }) final SolrRequest request,
			final ResponseParser processor) throws SolrServerException, IOException {
		return request(request, processor, null);
	}

	public NamedList<Object> request(@SuppressWarnings({ "rawtypes" }) final SolrRequest request,
			final ResponseParser processor, String collection) throws SolrServerException, IOException {
	...
	private void setBasicAuthHeader(@SuppressWarnings({ "rawtypes" }) SolrRequest request, HttpRequestBase method)
	...
	@SuppressWarnings({ "unchecked" })
	protected HttpRequestBase createMethod(@SuppressWarnings({ "rawtypes" }) SolrRequest request, String collection)
		...
		if (SolrRequest.METHOD.POST == request.getMethod() || SolrRequest.METHOD.PUT == request.getMethod()) {
			...
			if (contentWriter != null) {
				...
				HttpEntityEnclosingRequestBase postOrPut = fillContentStream(request, streams, wparams, isMultipart,
						postOrPutParams, fullQueryUrl);
				return postOrPut;
			}
	...
	private void fillSingleContentStream(Collection<ContentStream> streams, HttpEntityEnclosingRequestBase postOrPut)
			throws IOException {
	...
	private HttpEntityEnclosingRequestBase fillContentStream(@SuppressWarnings({ "rawtypes" }) SolrRequest request,
		...
		if (isMultipart && streams != null) {
			for (ContentStream content : streams) {
				...
				parts.add(new FormBodyPart(name,
						new InputStreamBody(content.getStream(), ContentType.parse(contentType), content.getName())));
			}
		}
	...
	protected NamedList<Object> executeMethod(HttpRequestBase method, Principal userPrincipal,
		...
		try {
			...
			if (userPrincipal != null) {
				...
				// However, if a personal Principal object exists, we use that instead, also as
				// a means
				// to transfer authentication information to Auth plugins that wish to intercept
				// the request later
			...
			default:
				if (processor == null || contentType == null) {
					throw new RemoteSolrException(baseUrl, httpStatus,
							"non ok status: " + httpStatus + ", message:" + response.getStatusLine().getReasonPhrase(),
							null);
				}
			}
			...
			if (procCt != null) {
				...
				if (!procMimeType.equals(mimeType)) {
					...
					try {
						...
						throw new RemoteSolrException(baseUrl, httpStatus,
								"Could not parse response with encoding " + exceptionCharset, e);
					}
			...
			if (error != null && (isV2Api
					|| String.valueOf(getObjectByPath(error, true, errPath)).endsWith("ExceptionWithErrObject"))) {
			...
			throw new SolrServerException("Timeout occurred while waiting response from server at: " + getBaseURL(), e);
		} catch (IOException e) {
			throw new SolrServerException("IOException occurred when talking to server at: " + getBaseURL(), e);
		} finally {
			if (shouldClose) {
	 ...
	 * any cores. The cost of this is that the core must be specified on each
	 * request.
	 */
	...
	/**
	 * Set SoTimeout (read timeout). This is desirable for queries, but probably not
	 * for indexing.
	 *
	 * @param timeout Timeout in milliseconds
	 *
	 ...
	 * redirect to get to a Solr installation, something is misconfigured somewhere.
	 * </p>
	 */
	...
	/**
	 * @deprecated since 8.0, catch {@link BaseHttpSolrClient.RemoteSolrException}
	 *             instead
	 */
	@Deprecated
	...
	/**
	 * @deprecated since 8.0, catch
	 *             {@link BaseHttpSolrClient.RemoteExecutionException} instead
	 */
	@Deprecated
	...
	public static class Builder extends SolrClientBuilder<Builder> {
		...
		/**
		 * Specify the base-url for the created client to use when sending requests to
		 * Solr.
		 *
		 * Two different paths can be specified as a part of this URL:
		 *
		 ...
		 *
		 * In this case the client is more flexible and can be used to send requests to
		 * any cores. This flexibility though requires that the core is specified on all
		 * requests.
		 */
		...
		/**
		 * Adds to the set of params that the created {@link HttpSolrClient} will add on
		 * all requests
		 *
		 * @param params a set of parameters to add to the invariant-params list. These
		 *               params must be unique and may not duplicate a param already in
		 *               the invariant list.
		 */
