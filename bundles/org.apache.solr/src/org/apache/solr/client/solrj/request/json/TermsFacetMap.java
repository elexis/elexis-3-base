...
public class TermsFacetMap extends JsonFacetMap<TermsFacetMap> {
	 ...
	 *
	 * @param numToSkip the number of buckets to skip over before selecting the
	 *                  buckets to return
	 */
	 ...
	 *
	 * Values can be based on 'count' (the number of results in each bucket),
	 * 'index' (the natural order of bucket values), or on any stat facet that
	 * occurs in the bucket. Defaults to "count desc" if not specified.
	 *
	 * By default, {@code sort} is calculated for all buckets generated by all
	 * shards. If {@code sort} is expensive a cheaper approximation can be provided
	 * using {@link #setPreliminarySort(String)} that will be run instead during
	 * bucket collection.
	 *
	...
	/**
	 * Indicates an approximate sort calculation to be performed during initial
	 * bucket generation and collection.
	 *
	 * Values can be based on 'count' (the number of results in each bucket),
	 * 'index' (the natural order of bucket values), or on any stat facet that
	 * occurs in the bucket. Defaults to "count desc" if not specified.
	 *
	 * When no {@code prelim_sort} is provided, {@code sort} is calculated on all
	 * buckets generated by all shards. If {@code sort} is expensive,
	 * {@code prelim_sort} can be used to provide a cheaper approximation
	 * calculation that is run instead on initial bucket collection. {@code sort} is
	 * still used when assembling the final list of buckets.
	 *
	...
	/**
	 * Indicates the number of additional buckets to request internally beyond those
	 * required by {@link #setLimit(int)}.
	 *
	 * Defaults to -1 if not specified, which triggers some heuristic guessing based
	 * on other settings.
	 */
	 ...
	 *
	 * "Distributed facet refining" is a second, optional stage in the facet process
	 * that ensures that counts for the returned buckets are exact. Enabling it is a
	 * tradeoff between precision and speed/performance. Defaults to false if not
	 * specified.
	 *
	 * @param useRefining true if distributed facet refining should be used; false
	 *                    otherwise
	 */
	...
	/**
	 * Indicates how many extra buckets to request during distributed-facet-refining
	 * beyond those required by {@link #setLimit(int)}
	 *
	 * Defaults to -1 if not specified, which triggers some heuristic guessing based
	 * on other settings.
	 */
	...
	/**
	 * Indicates that the facet results should not include any buckets with a count
	 * less than {@code minCount}.
	 *
	...
	/**
	 * Indicates that Solr should create a bucket corresponding to documents missing
	 * the field used by this facet.
	 *
	 * Defaults to false if not specified.
	 *
	 * @param missingBucket true if the special "missing" bucket should be created;
	 *                      false otherwise
	 */
	...
	/**
	 * Indicates that Solr should include the total number of buckets for this
	 * facet.
	 *
	 * Note that this is different than the number of buckets returned. Defaults to
	 * false if not specified
	 *
	 * @param numBuckets true if the "numBuckets" field should be computed; false
	 *                   otherwise
	 */
	 ...
	 *
	 * For multi-valued fields this is different than a bucket for the entire
	 * domain, since documents can belong to multiple buckets. Defaults to false if
	 * not specified.
	 *
	 * @param shouldInclude true if the union bucket "allBuckets" should be
	...
	/**
	 * Indicates that the facet should only produce buckets for terms that start
	 ...
	 *
	 * Defaults to "smart" if not specified, which has Solr guess which computation
