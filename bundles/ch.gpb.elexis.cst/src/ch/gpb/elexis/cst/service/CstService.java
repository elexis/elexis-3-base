 ...
 * 27.06.2015
 * TODO: there may be unused methods
 */
public class CstService {
	...
	for (Patient patient : target) {
	    ...
	    CstProfile newProfile = new CstProfile(Messages.Cst_Text_Copy_of + source.getName() + " ("
		    + CstService.getGermanFromDate(new Date()) + ")",
		    source.getDescription(), null, patient.getId(), mandant.getId(),
		    getCompactFromDate(new Date()), getCompactFromDate(new Date()), "1");
    ...
    /**
     * Copies all data and all records from related tables from source to target profile
     * @param source
     * @param target
     */
	...
	/* not needed - maybe configurable?
	CstGastroColo dbObj = CstGastroColo.getByProfileId(source.getId());

	    if (dbObj != null) {

	    CstGastroColo newColo = new CstGastroColo(target.getId(), dbObj.getDatumGastro(),
			dbObj.getDatumColo());
		...
		newColo.setText1(dbObj.getText1());
		newColo.setText2(dbObj.getText2());
		newColo.setText3(dbObj.getText3());
		newColo.setText4(dbObj.getText4());
	    }*/
    ...
    /**
     * get the separator that can be stored with a Befund selection in the options panel
     * and is for splitting findings that contain double values like n.n/nn.n
     * @param mapAuswahl
     * @param befundArt
     * @return
     */
	...
	// mapAuswahl may contain the key, but with a "" value (and not null), so we return null explicitly on empty strings
	Iterator<Object> itKeys = mapAuswahl.keySet().iterator();
	while (itKeys.hasNext()) {
	    Object key = (Object) itKeys.next();
	...
	for (String sKey : keys1) {
		...
		for (String sKey3 : keys3) {
			...
			// Labitems seem to be intermixed (ie HGB / H�moglobin and LDH / LDH Elektrophorese)
	...
	for (String sKey : keys1) {
		...
		for (String sKey3 : keys3) {
			...
			/*
						if (currentResult.getItem().getName().equals(name) && dateResult.after(dateStart)
			...
			*/
			...
			if ((currentResult.getItem().getName().equals(name) ||
				...
				currentResult.getItem().getKuerzel().equals(name))
				&& dateResult.after(dateStart)
				&& dateResult.before(dateEnd)) {
			    if (result == null) {
				result = currentResult;
				//log.debug("HIT 1: " + currentResult.getItem().getName() + " \tDate: " + currentResult.getDate());
			    }
			    else {
	...
	try {
	    ...
	    //log.error("Formatting Result: " + sResult + "/" + sRes + " formatted: " + result, Log.ERRORS);
