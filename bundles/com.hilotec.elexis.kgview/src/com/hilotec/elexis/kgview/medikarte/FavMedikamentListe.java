 ...
 * suchen, neue Eintraege zu erstellen indem ein neues Medikament per Drag&Drop
 * auf die Tabelle gezogen wird, Eintraege zu veraendern und zu loeschen.
 *
		...
		for (FavMedikament fm : FavMedikament.getAll()) {
			...
			for (Artikel na : al) {
				if (SWTHelper.askYesNo("Fav. Medikamente",
						"Soll '" + fm.getBezeichnung() + "' neu mit '" + na.getName() + "' verknüpft werden?")) {
					fm.relinkTo(na);
					rl = true;
					break;
				}
		...
		actEdit = new Action("Bearbeiten", Action.AS_PUSH_BUTTON) {
			...
			public void run() {
				...
				if (tis == null || tis.length != 1)
					return;
				new FavMedikamentDialog(getSite().getShell(), (FavMedikament) tis[0].getData()).open();
				refresh();
			}
		};
		...
		actDelete = new Action("Löschen", Action.AS_PUSH_BUTTON) {
			...
			public void run() {
				...
				if (!SWTHelper.askYesNo("Medikament(e) aus Liste entfernen",
						"Sollen das/die ausgewählte(n) Medikament(e) aus der" + " Liste entfernt werden?"))
					return;
