...
public class AscEvaluator extends RecursiveObjectEvaluator implements OneValueWorker {
	...
	public Object doWork(Object value) throws IOException {
		...
		for (int idx = 0; idx < list.size(); ++idx) {
			...
			if (null == item) {
				throw new IOException(String.format(Locale.ROOT, "Invalid expression %s - found null value",
						toExpression(constructingFactory)));
			} else if (!(item instanceof Comparable<?>)) {
				throw new IOException(
						String.format(Locale.ROOT, "Invalid expression %s - found non-comparable value %s of type %s",
								toExpression(constructingFactory), item.toString(), item.getClass().getSimpleName()));
		...
		return list.stream().sorted((left, right) -> ((Comparable) left).compareTo((Comparable) right))
