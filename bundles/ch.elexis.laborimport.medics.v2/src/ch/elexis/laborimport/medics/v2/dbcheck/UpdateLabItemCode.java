...
public class UpdateLabItemCode extends ExternalMaintenance {
	...
	public String executeMaintenance(IProgressMonitor pm, String DBVersion){
		...
		pm.beginTask(getMaintenanceDescription() + " (" + items.size() + " Medics Parameter) ("
			+ nameToCodeMap.size() + " Medics Methodenstamm)",
			items.size());
		int updated = 0;
		int already = 0;
		int notfound = 0;
		for (LabItem item : items) {
			...
			if (code != null) {
				...
				LoggerFactory.getLogger(getClass())
					.warn("No code found for medics item " + item.getLabel());
				notfound++;
			}
		}
	...
	private Map<String, String> getNameToCodeMap(){
		...
		if (exw.load(getClass().getResourceAsStream("/rsc/codes/methodenstamm_20200131.xlsx"), 0)) {
			exw.setFieldTypes(new Class[] {
				String.class /* methode */, String.class /* langname */
			});
