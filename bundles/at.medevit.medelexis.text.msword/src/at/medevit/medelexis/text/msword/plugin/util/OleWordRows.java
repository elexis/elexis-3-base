...
public class OleWordRows extends OleWrapper {
	public OleWordRows(OleAutomation oleAuto, Display display, OleWrapperManager manager){
		super(oleAuto, display, manager);
	}
	
	public OleWordRow add(OleWrapperManager manager){
		Variant oleVar = runInvoke("Add"); //$NON-NLS-1$
		OleAutomation oleAuto = OleUtil.getOleAutomationFromVariant(oleVar);
		return new OleWordRow(oleAuto, display, manager);
	}
	
	public OleWordRow getItem(long index, OleWrapperManager manager){
