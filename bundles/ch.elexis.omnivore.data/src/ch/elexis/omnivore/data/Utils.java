...
public class Utils {
			...
			for (Integer i = 0; i < Preferences.getOmnivorenRulesForAutoArchiving(); i++) {
				...
				if ((SrcPattern != null) && (DestDir != null)
					...
					if (file.getAbsolutePath().contains(SrcPattern)) {
						...
						if (newFile.isDirectory()) {
							log.debug("new File {} is a directory ; archiveFile not attempted",newFile.getAbsolutePath());
							...
							if (newFile.isFile()) {
								log.debug("new File {} already exits ; archiveFile not attempted",newFile.getAbsolutePath());
								SWTHelper.showError(Messages.DocHandle_MoveErrorCaption,
								...
								if (Files.move(file.toPath(), newFile.toPath(),
									...
									log.debug(
										"Archived incoming file {} to: {}", file.getAbsolutePath(), newFile.getAbsolutePath());
								} else {
									log.debug(
	...
	static private String processFileElement(IPreferenceStore preferenceStore,
		String element_key, String element_data){
		
		log.debug("processFileElement: element_key=<{}> data <{}>", element_key,
			element_data);
		...
		for (int i = 0; i < nCotfRules; i++) {
			if (Preferences.PREFERENCE_cotf_elements[i].equals(element_key)) {
				if (element_key.contains("constant")) {
					String search = PREFBASE + Preferences.PREFERENCE_COTF + Preferences.PREFERENCE_cotf_elements[i] + "_"
						+ Preferences.PREFERENCE_cotf_parameters[1];
					String constant = preferenceStore.getString(search).trim();
					log.debug("processFileElement: {} returning constant=<{}>",
						search, constant);
					...
					String element_data_processed5 = (element_data.replaceAll(
						java.util.regex.Matcher.quoteReplacement(Preferences.cotf_unwanted_chars), "").toString().trim());
					...
					log.debug("{} {} {} {} <{}> {} <{>}", i, snumId, snum_digits, leadId, lead_fill_char, trailId, add_trail_char);
				}
	...
	/**
	 * Generate a nice file name for the docHandle in the temp directory
	 * Used when double-clicking or dragging the file
	 * <br><br>Can be configured via the setting document exchange..Omnivore
	 * <br>Returns a formatted
	 ...
	 * 
	 * 
	 * @param dh The docHandle (containing title, patient, etc for which we want to generate the name
