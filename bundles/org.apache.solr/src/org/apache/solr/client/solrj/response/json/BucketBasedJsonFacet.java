...
/**
 * Represents the top-level response for a bucket-based JSON facet (i.e. "terms"
 * or "range")
 *
 ...
 * {@code allBuckets}, {@code numBuckets}, {@code before}, etc.)
 */
public class BucketBasedJsonFacet {
	...
	public BucketBasedJsonFacet(NamedList<Object> bucketBasedFacet) {
		for (Map.Entry<String, Object> entry : bucketBasedFacet) {
			...
			if ("buckets".equals(key)) {
				...
				// We don't recognize the key. Possible JSON faceting schema has changed without
				// updating client.
				// Silently ignore for now, though we may want to consider throwing an error if
	...
	/**
	 * The total number of buckets found in the domain (of which the returned
	 * buckets are only a part).
	 *
	 * This value can only be computed on "terms" facets where the user has
	 * specifically requested it with the {@code numBuckets} option.
	 * {@link #UNSET_FLAG} is returned if this is a "range" facet or
	 * {@code numBuckets} computation was not requested in the intiial request.
	 *
	 * @deprecated this method can trigger ClassCastException's if the returned
	 *             value is a long. In the future it will return a 'long' instead to
	 *             avoid this problem. Until then, use {@link #getNumBucketsCount()}
	 *             instead.
	 */
	 ...
	 *
	 * Note that for facets on multi-valued fields, documents may belong to multiple
	 * buckets, making {@link #getAllBuckets()} return a result greater than the
	 * number of documents in the domain.
	 * <p>
	 * This value is only present if the user has specifically requested it with the
	 * {@code allBuckets} option. {@link #UNSET_FLAG} is returned if this is not the
	 * case.
	 */
	...
	/**
	 * The count of all records whose field value precedes the {@code start} of this
	 * "range" facet
	 *
	 * This value is only present if the user has specifically requested it with the
	 * {@code other} option. {@link #UNSET_FLAG} is returned if this is not the
	 * case.
	 *
	 * @deprecated this method can trigger ClassCastException's if the returned
	 *             value is a long. In the future it will return a 'long' instead to
	 *             avoid this problem. Until then, use {@link #getBeforeCount()}
	 *             instead.
	 */
	...
	/**
	 * The count of all records whose field value follows the {@code end} of this
	 * "range" facet
	 *
	 * This value is only present if the user has specifically requested it with the
	 * {@code other} option. {@link #UNSET_FLAG} is returned if this is not the
	 ...
	 * @deprecated this method can trigger ClassCastException's if the returned
	 *             value is a long. In the future it will return a 'long' instead to
	 *             avoid this problem. Until then, use {@link #getAfterCount()}
	 *             instead.
	 */
	public int getAfter() {
	...
	/**
	 * The count of all records whose field value falls between {@code start} and
	 * {@code end}.
	 *
	 * This value is only present if the user has specifically requested it with the
	 * {@code other} option. {@link #UNSET_FLAG} is returned if this is not the
	 ...
	 * @deprecated this method can trigger ClassCastException's if the returned
	 *             value is a long. In the future it will return a 'long' instead to
	 *             avoid this problem. Until then, use {@link #getBetweenCount()}
	 *             instead.
	 */
