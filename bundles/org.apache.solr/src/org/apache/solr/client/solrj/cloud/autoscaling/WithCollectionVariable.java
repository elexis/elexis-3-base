...
public class WithCollectionVariable extends VariableBase {
	...
	public void projectAddReplica(Cell cell, ReplicaInfo ri, Consumer<Row.OperationInfo> opCollector,
		...
		for (Map.Entry<String, String> e : withCollectionMap.entrySet()) {
			if (uniqueColls.contains(e.getKey()) && !uniqueColls.contains(e.getValue())) {
				...
				opCollector.accept(
						new Row.OperationInfo(withCollection, "shard1", row.node, cell.name, true, Replica.Type.NRT));
			}
		}
	}
				...
				if (withCollectionMap != null) {
					row.forEachReplica(r -> {
						...
						if (withCollection != null) {
							// test whether this row has at least 1 replica of withCollection, else there is
							// a violation
					...
					ctx.currentViolation.replicaCountDelta = (double) ctx.currentViolation.getViolatingReplicas()
							.size();
				}
			}
		}
	...
	public void getSuggestions(Suggestion.Ctx ctx) {
		...
		Map<String, Object> nodeValues = ctx.session.nodeStateProvider.getNodeValues(ctx.violation.node,
		...
		collectionLoop: for (String collection : uniqueCollections) {
			...
			for (Row row : ctx.session.matrix) {
				if (ctx.violation.node.equals(row.node))
					continue; // filter the violating node
				...
				if (hostedCollections.contains(withCollection) && !hostedCollections.contains(collection)) {
					...
					for (ReplicaInfo toMove : movableReplicas) {
						...
						Suggester suggester = ctx.session.getSuggester(MOVEREPLICA).forceOperation(true)
								...
								.hint(Suggester.Hint.SRC_NODE, row.node).hint(Suggester.Hint.REPLICA, toMove.getName())
			...
			Suggester suggester = ctx.session.getSuggester(ADDREPLICA).forceOperation(true)
					.hint(Suggester.Hint.COLL_SHARD, new Pair<>(withCollection, "shard1"))
					.hint(Suggester.Hint.TARGET_NODE, ctx.violation.node);
