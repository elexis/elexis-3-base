...
/**
 * This represents a data entry in the payload/stream. There are multiple ways
 * to consume the data entry a) listen to it, if it's a container object, and
 * get callbacks for each sub-entry b) read as an object using the
 * {{@link #val()}} method. Please note that it creates objects and expect more
 * memory usage c) read the corresponding primitive value Do not keep a
 * reference of this Object beyond the scope where it is called. Read the
 * relevant data out.
 */
public interface DataEntry {
	 ...
	 *
	 * @param ctx      This is any object that should be shared with the child entry
	 *                 callbacks
	 ...
	 * {{@link #listenContainer(Object, EntryListener)}} method
	 */
	Object ctx();
	 ...
	 *
	 * if it's a {{@link CharSequence}} or byte[] , it's the no:of bytes in the
	 * stream
	 *
	 * @return a number greater than or equal to zero if the size is known, -1 if
	 *         unknown
	 */
	int length();
	 ...
	 */
	enum Type {
		NULL(true), LONG(true), INT(true), BOOL(true), FLOAT(true), DOUBLE(true), DATE(true),
		/**
		 ...
		 */
		ENTRY_ITER(false, true), STR(false), BYTEARR(false),
		/**
		 * don't know how to stream it. read as an object using
		 * {{@link DataEntry#val()}} method
		 */
		JAVA_OBJ(false);
	...
	interface EntryListener {
		...
		/**
		 * Callback for each entry in this container. once the method call returns, the
		 * entry object is not valid anymore It is usually reused. If the object value
		 * is a {{@link Utf8CharSequence}} do a {{@link Object#clone()}} because the
