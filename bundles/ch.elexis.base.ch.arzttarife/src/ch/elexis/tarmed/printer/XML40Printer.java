...
public class XML40Printer {
	...
	private static Namespace namespace = Namespace.getNamespace("http://www.xmlData.ch/xmlInvoice/XSD"); //$NON-NLS-1$
	...
	private EZPrinter.EZPrinterData getEZPrintData(Element balance, Element invoice) {
		...
		try {
			ret.amountTarmed = XMLTool.xmlDoubleToMoney(balance.getAttributeValue(XMLExporter.ATTR_AMOUNT_TARMED));
			ret.amountDrug = XMLTool.xmlDoubleToMoney(balance.getAttributeValue(XMLExporter.ATTR_AMOUNT_DRUG));
			ret.amountLab = XMLTool.xmlDoubleToMoney(balance.getAttributeValue(XMLExporter.ATTR_AMOUNT_LAB));
			ret.amountMigel = XMLTool.xmlDoubleToMoney(balance.getAttributeValue(XMLExporter.ATTR_AMOUNT_MIGEL));
			ret.amountPhysio = XMLTool.xmlDoubleToMoney(balance.getAttributeValue(XMLExporter.ATTR_AMOUNT_PHYSIO));
			ret.amountUnclassified = XMLTool
					.xmlDoubleToMoney(balance.getAttributeValue(XMLExporter.ATTR_AMOUNT_UNCLASSIFIED));

			ret.due = XMLTool.xmlDoubleToMoney(balance.getAttributeValue(XMLExporter.ATTR_AMOUNT_DUE));
			ret.paid = XMLTool.xmlDoubleToMoney(balance.getAttributeValue(XMLExporter.ATTR_AMOUNT_PREPAID));
	...
	public boolean doPrint(Rechnung rn, Document xmlRn, TYPE rnType, String saveFile, boolean withESR,
			boolean withForms, boolean doVerify, IProgressMonitor monitor) {
		...
		String tcCode = null;
		IContact rsContact = CoreModelServiceHolder.get().load(rs.getId(), IContact.class).orElse(null);
		if (TarmedRequirements.hasTCContract(rsContact) && ezData.paymentMode.equals(XMLExporter.TIERS_GARANT)) {
		...
		ESR esr = new ESR(rs.getInfoString(TarmedACL.getInstance().ESRNUMBER),
				rs.getInfoString(TarmedACL.getInstance().ESRSUB), rn.getRnId(), ESR.ESR27);
		...
		IContact zuweiserContact = CoreModelServiceHolder.get().load(zuweiser.getId(), IContact.class).orElse(null);
		if (zuweiser != null) {
		...
		SortedList<Element> ls = new SortedList<Element>(services.getChildren(), new RnComparator());
		...
		for (Element s : ls) {
			...
			sb.append(Integer.toString(XMLPrinterUtil.guessVatCode(XMLPrinterUtil.getValue(s, "vat_rate")))) //$NON-NLS-1$
					.append("\t");
		...
		cursor = XMLPrinterUtil.print(cursor, tp, false, XMLPrinterUtil.getValue(balance, "amount_physio")); //$NON-NLS-1$
		cursor = XMLPrinterUtil.print(cursor, tp, true, "\tMiGeL \t"); //$NON-NLS-1$
		cursor = XMLPrinterUtil.print(cursor, tp, false, XMLPrinterUtil.getValue(balance, "amount_migel")); //$NON-NLS-1$
		cursor = XMLPrinterUtil.print(cursor, tp, true, "\tÜbrige \t"); //$NON-NLS-1$
		cursor = XMLPrinterUtil.print(cursor, tp, false, XMLPrinterUtil.getValue(balance, "amount_unclassified")); //$NON-NLS-1$
		...
		cursor = XMLPrinterUtil.print(cursor, tp, false, XMLPrinterUtil.getValue(balance, "amount_lab")); //$NON-NLS-1$
		cursor = XMLPrinterUtil.print(cursor, tp, true, "\tMedi \t"); //$NON-NLS-1$
		cursor = XMLPrinterUtil.print(cursor, tp, false, XMLPrinterUtil.getValue(balance, "amount_drug")); //$NON-NLS-1$
		cursor = XMLPrinterUtil.print(cursor, tp, true, "\tKantonal \t"); //$NON-NLS-1$
		cursor = XMLPrinterUtil.print(cursor, tp, false, XMLPrinterUtil.getValue(balance, "amount_cantonal")); //$NON-NLS-1$
		...
		footer.append("\n\n").append("■ Gesamtbetrag\t\tCHF\t\t").append(df.format(sumTotal)) //$NON-NLS-1$ //$NON-NLS-2$
				...
				.append(ezData.paid.getAmountAsString()).append("\tFälliger Betrag \t") //$NON-NLS-1$
				.append(ezData.due.getAmountAsString());
		...
		Boolean isVat = (Boolean) mnd.getRechnungssteller().getInfoElement(XMLExporter.VAT_ISMANDANTVAT);
		if (isVat != null && isVat) {
