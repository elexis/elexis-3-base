...
/**
 * An equality field Comparator which compares a field of two Tuples and
...
public class FieldComparator implements StreamComparator {
	...
	public Explanation toExplanation(StreamFactory factory) throws IOException {
		return new Explanation(comparatorNodeId.toString()).withExpressionType(ExpressionType.SORTER)
	 ...
	 * We want the compare(...) function to run as fast as possible because it will
	 * be called many many times over the lifetime of this object. For that reason
	 * we want to limit the number of comparisons taking place in the compare(...)
	 * function. Because this class supports both ascending and descending
	 * comparisons and the logic for each is slightly different, we want to do the
	 * if(ascending){ compare like this } else { compare like this } check only once
	 * - we can do that in the constructor of this class, create a lambda, and then
	 * execute that lambda in the compare function. A little bit of branch
	 * prediction savings right here.
	 */
	...
	public boolean isDerivedFrom(StreamComparator base) {
		...
		if (base instanceof FieldComparator) {
			...
			return (leftFieldName.equals(baseComp.leftFieldName) || rightFieldName.equals(baseComp.rightFieldName))
	...
	public FieldComparator copyAliased(Map<String, String> aliases) {
		return new FieldComparator(aliases.containsKey(leftFieldName) ? aliases.get(leftFieldName) : leftFieldName,
				aliases.containsKey(rightFieldName) ? aliases.get(rightFieldName) : rightFieldName, order);
	}
	...
	public boolean equals(Object o) {
		...
		return leftFieldName.equals(that.leftFieldName) && rightFieldName.equals(that.rightFieldName)
