...
/**
 * CompositeIdRouter partitions ids based on a {@link #SEPARATOR}, hashes each
 * partition and merges the hashes together to map the id to a slice. This
 * allows bucketing of like groups of ids.
 *
 * Allows basic separation split between 32 bits as example given below, or
 * using {@link #bitsSeparator} can specify exact bits
 *
 * <pre>
 * Example inputs:
 * user!uniqueid
 ...
 * </pre>
 *
 * Lets say you had a set of records you want to index together such as a
 * contact in a database, using a prefix of contact!contactid would allow all
 * contact ids to be bucketed together.
 *
 * <pre>
 * An Example:
...
public class CompositeIdRouter extends HashBasedRouter {
	...
	// separator used to optionally specify number of bits to allocate toward first
	// part.
	...
	public Collection<Slice> getSearchSlicesSingle(String shardKey, SolrParams params, DocCollection collection) {
		if (shardKey == null) {
			...
			// TODO: this may need modification in the future when shard splitting could
			// cause an overlap
	...
	public List<Range> partitionRange(int partitions, Range range) {
		...
		// Round to avoid splitting hash domains across ranges if such rounding is not
		// significant.
		// With default bits==16, one would need to create more than 4000 shards before
		// this
	...
	static class KeyParser {
		...
		Range getRange() {
			...
			if ((masks[0] == 0 && !triLevel) || (masks[0] == 0 && masks[1] == 0 && triLevel)) {
				// no bits used from first part of key.. the code above will produce
				// 0x000000000->0xffffffff
