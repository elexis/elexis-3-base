...
public class KonsData extends PersistentObject {
	...
	// duerfen.
	private static final String[] KGFIELDS = { FLD_JETZLEIDEN, FLD_JETZLEIDEN_ICPC, FLD_ALLGSTATUS, FLD_LOKSTATUS,
			FLD_PROZEDERE, FLD_PROZEDERE_ICPC, FLD_DIAGNOSE, FLD_DIAGNOSE_ICPC, FLD_THERAPIE, FLD_VERLAUF, FLD_ROENTGEN,
			FLD_EKG, };
	private static final HashSet<String> KGFIELD_SET = new HashSet<String>(Arrays.asList(KGFIELDS));

	static {
		addMapping(TABLENAME, "JetzigesLeiden=JetzLeiden", "JetzigesLeidenICPC=JetzLeidenICPC", "AllgStatus",
				"LokStatus", "Prozedere", "ProzedereICPC", "Diagnose", "DiagnoseICPC", "Therapie", "Verlauf",
				"Roentgen", "EKG", "KonsZeit", "KonsBeginn", "KonsTyp", "Autor");
		checkTable();
	}
	...
	private static final String create = "CREATE TABLE " + TABLENAME + " ("
			...
			+ "  Therapie		TEXT, " + "  Verlauf		TEXT, " + "  Roentgen		TEXT, "
			+ "  EKG			TEXT, " + "  KonsZeit 		BIGINT DEFAULT 0, " + "  KonsBeginn     BIGINT,  "
			+ "  KonsTyp		SMALLINT DEFAULT 0, " + "  Autor          VARCHAR(25)  " + ");" + "INSERT INTO "
			+ TABLENAME + " (ID, JetzLeiden) VALUES " + "	('VERSION', '" + VERSION + "');";

	private static final String up_1to2 = "ALTER TABLE " + TABLENAME + "  ADD JetzLeidenICPC	TEXT AFTER JetzLeiden,"
			+ "  ADD ProzedereICPC 	TEXT AFTER Prozedere," + "  ADD DiagnoseICPC 	TEXT AFTER Diagnose;" + "UPDATE "
			+ TABLENAME + " SET JetzLeiden = '2' WHERE" + "  ID LIKE 'VERSION';";

	private static final String up_2to3 = "ALTER TABLE " + TABLENAME + "  ADD Therapie	TEXT AFTER DiagnoseICPC,"
			+ "  ADD Verlauf 	TEXT AFTER Therapie;" + "UPDATE " + TABLENAME + " SET JetzLeiden = '3' WHERE"
			+ "  ID LIKE 'VERSION';";

	private static final String up_3to4 = "ALTER TABLE " + TABLENAME + "  ADD Roentgen	TEXT AFTER Verlauf,"
			+ "  ADD EKG		TEXT AFTER Roentgen;" + "UPDATE " + TABLENAME + " SET JetzLeiden = '4' WHERE"
			+ "  ID LIKE 'VERSION';";

	private static final String up_4to5 = "ALTER TABLE " + TABLENAME + "  ADD KonsBeginn	BIGINT AFTER KonsZeit;"
			+ "UPDATE " + TABLENAME + " SET JetzLeiden = '5' WHERE" + "  ID LIKE 'VERSION';";

	private static final String up_5to6 = "ALTER TABLE " + TABLENAME + "  CHANGE KonsZeit KonsZeit BIGINT DEFAULT 0;"
			+ "UPDATE " + TABLENAME + " SET KonsZeit = 0 WHERE" + "  KonsZeit IS NULL;" + "UPDATE " + TABLENAME
	...
	private static final String up_7to8 = "ALTER TABLE " + TABLENAME + "  ADD Autor		VARCHAR(25) AFTER IstTelefon;"
			+ "UPDATE " + TABLENAME + " SET JetzLeiden = '8' WHERE" + "  ID LIKE 'VERSION';";

	private static final String up_8to9 = "ALTER TABLE " + TABLENAME
			+ "  ADD KonsTyp	SMALLINT DEFAULT 0 AFTER IstTelefon;" + "UPDATE " + TABLENAME + " SET KonsTyp = 1 WHERE"
			+ "  IstTelefon = 1;" + "ALTER TABLE " + TABLENAME + "  DROP IstTelefon; " + "UPDATE " + TABLENAME
			+ " SET JetzLeiden = '9' WHERE" + "  ID LIKE 'VERSION';";
	...
	/**
	 * Wir ueberschreiben hier set() um sicherzustellen dass nur der Autor einen
	 * KG-Eintrag anpassen kann, und um den Autor festzuhalten falls das noch nicht
	 * geschehen ist.
	 */
	@Override
	public boolean set(final String field, String value) {
		if (KGFIELD_SET.contains(field)) {
			...
			if (au == null) {
				...
				// aktuellen User
				setAutor(CoreHub.getLoggedInContact());
			} else if (!au.equals(CoreHub.getLoggedInContact())) {
				// Ungueltiger User
	...
	/**
	 * Prueft ob die KG dieser Konsultation vom aktuellen Benutzer bearbeitet werden
	 * darf
	 */
	public boolean isEditOK() {
		Anwender au = getAutor();
		return au == null || au.equals(CoreHub.getLoggedInContact());
	}
