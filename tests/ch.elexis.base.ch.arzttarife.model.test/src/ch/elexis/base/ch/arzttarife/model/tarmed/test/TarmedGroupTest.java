...
public class TarmedGroupTest {
	...
	public void getExclusions(){
		ITarmedGroup grp01 =
			AllTestsSuite.getModelService().load("GRP01-20010101", ITarmedGroup.class).get();
	...
	public void getLimitations(){
		...
		ITarmedGroup grp01 =
			AllTestsSuite.getModelService().load("GRP04-20180101-KVG", ITarmedGroup.class).get();
		assertTrue(grp01 instanceof ITarmedGroup);
		List<TarmedLimitation> limitations = grp01.getLimitations();
		assertEquals(1, limitations.size());
		assertEquals("Codes der Gruppe 04 maximal 30 Mal pro 3 Monat(e)",
			limitations.get(0).toString());
	}
