...
/**
 * This class represents the Word UI inside the Workbench. It also provides
 * access to the created Word Document OLE/COM object.
 *
 * @author thomashu
 *
 */
public class OleWordSite {
		...
		synchronized (this) {
			OleRunnable runnable = new OleRunnable() {
				...
				public void run() {
					...
					try {
						if (file == null) {
							oleClientSite = new DbgOleControlSite(oleFrame, SWT.NONE, "Word.Document"); //$NON-NLS-1$
							oleClientSiteAuto = new OleAutomation(oleClientSite);
						} else {
							oleClientSite = new DbgOleControlSite(oleFrame, SWT.NONE, "Word.Document", file); //$NON-NLS-1$
							oleClientSiteAuto = new OleAutomation(oleClientSite);
						}
	...
	public OleWordApplication getApplication(OleWrapperManager manager) {
		synchronized (this) {
			OleAutomation returnAutomation = OleUtil.getOleAutomationProperty(oleClientSiteAuto, "Application"); //$NON-NLS-1$
			return (OleWordApplication) new OleWordApplication(this, returnAutomation, oleFrame.getDisplay(), manager);
		}
	}
	...
	protected synchronized void run(OleRunnable runnable) {
		try {
			// IMPORTANT: trigger event loop, else OLE/COM does not work correct
			// (unpredictable
