...
public class EncounterIndexerIdentifiedRunnable extends AbstractIndexerIdentifiedRunnable {
	...
	public Map<String, Serializable> run(Map<String, Serializable> runContext, IProgressMonitor progressMonitor,
		...
		try (HttpSolrClient solr = solrClientBuilder.build()) {
			...
			configService.set(SolrConstants.CONFIG_KEY_LASTINDEXRUN_ENCOUNTER, Long.toString(newestLastUpdate), false);
	...
	private long indexEncounters(HttpSolrClient solr, Long lastIndexRunLastUpdate, IProgressMonitor progressMonitor,
			Logger logger, ArrayList<SingleIdentifiableTaskResult> failures) throws SolrServerException, IOException {
		...
		query.and(ModelPackage.Literals.IDENTIFIABLE__LASTUPDATE, COMPARATOR.GREATER, lastIndexRunLastUpdate);
			...
			while (encounterCursor.hasNext()) {
				...
				try {
					...
					if (encounter.isDeleted()) {
						...
						if (document != null) {
							checkResponse(solr.deleteById(SolrConstants.CORE_ENCOUNTERS, encounter.getId()));
							noRemovedFromIndex++;
						}
