...
public class TardocDetailDisplay implements IDetailDisplay {
	...
	private ITardocLeistung actCode;

	public TardocDetailDisplay() {

	}

	@Inject
	public void selection(@Optional @Named("ch.elexis.views.codeselector.tardoc.selection") ITardocLeistung tarmed) {
	...
	public void display(Object obj) {
		if (obj instanceof ITardocLeistung) {
			...
			if (actCode.isChapter()) {
				...
				List<ITardocKumulation> kumulations = actCode.getKumulations(TardocKumulationArt.SERVICE);
				exclusion.setText(getKumulationsString(kumulations, actCode.getCode(), TardocKumulationTyp.EXCLUSION),
						false, false);
				inclusion.setText(getKumulationsString(kumulations, actCode.getCode(), TardocKumulationTyp.INCLUSION),
	...
	private String getKumulationsString(List<ITardocKumulation> list, String code, TardocKumulationTyp typ) {
		StringBuilder sb = new StringBuilder();
		if (list != null) {
			List<ITardocKumulation> slaveServices = list.stream()
					.filter(k -> k.getTyp() == typ && k.getSlaveArt() == TardocKumulationArt.SERVICE
							&& k.getSlaveCode().equals(code) && k.getMasterArt() == TardocKumulationArt.SERVICE)
					.collect(Collectors.toList());
			List<ITardocKumulation> masterServices = list.stream()
					.filter(k -> k.getTyp() == typ && k.getMasterArt() == TardocKumulationArt.SERVICE
							&& k.getMasterCode().equals(code) && k.getSlaveArt() == TardocKumulationArt.SERVICE)
					.collect(Collectors.toList());

			if (!slaveServices.isEmpty() || !masterServices.isEmpty()) {
				sb.append("Leistungen: ");
				StringJoiner sj = new StringJoiner(", ");
				for (ITardocKumulation tarmedKumulation : slaveServices) {
				...
				for (ITardocKumulation tarmedKumulation : masterServices) {
					// dont add same exclusion multiple times
			...
			List<ITardocKumulation> slaveGroups = list.stream()
					.filter(k -> k.getTyp() == typ && k.getSlaveArt() == TardocKumulationArt.GROUP)
					.collect(Collectors.toList());
			List<ITardocKumulation> masterGroups = list.stream()
					.filter(k -> k.getTyp() == typ && k.getMasterArt() == TardocKumulationArt.GROUP)
					.collect(Collectors.toList());
			...
			if (!slaveGroups.isEmpty() || !masterGroups.isEmpty()) {
				...
				sb.append("Gruppen: ");
				StringJoiner sj = new StringJoiner(", ");
				for (ITardocKumulation tarmedKumulation : slaveGroups) {
					// dont add same exclusion multiple times
					if (!sj.toString().contains(tarmedKumulation.getSlaveCode())) {
						sj.add(tarmedKumulation.getSlaveCode());
					}
				}
				for (ITardocKumulation tarmedKumulation : masterGroups) {
					// dont add same exclusion multiple times
					if (!sj.toString().contains(tarmedKumulation.getMasterCode())) {
						sj.add(tarmedKumulation.getMasterCode());
					}
