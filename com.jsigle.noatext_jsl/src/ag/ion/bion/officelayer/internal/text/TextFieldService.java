...
public class TextFieldService implements ITextFieldService {
	 ...
	 *             if the OpenOffice.org interface is not valid
	 * 
	 * @author Andreas Bröker
	 */
	public TextFieldService(ITextDocument textDocument) {
		if (textDocument == null) {
			throw new IllegalArgumentException(
					"The submitted text document is not valid.");
		}
	 ...
	 * 
	 * @throws TextException
	 *             if the masters of the user textfields can not be constructed
	 * 
	 * @author Andreas Bröker
	 */
	public ITextFieldMaster[] getUserTextFieldMasters(String prefix)
			throws TextException {
		if (prefix == null) {
		...
		try {
			...
			for (int i = 0, n = names.length; i < n; i++) {
				...
				if (name.toLowerCase().startsWith(
						USER_TEXTFIELD_MASTER_PREFIX.toLowerCase())) {
					String fieldName = name
							.substring(USER_TEXTFIELD_MASTER_PREFIX.length());
					if (fieldName.startsWith(prefix)) {
						try {
							...
							xPropertySet = (XPropertySet) UnoRuntime
									.queryInterface(XPropertySet.class, any);
							if (xPropertySet != null) {
								arrayList.add(new TextFieldMaster(textDocument,
										xPropertySet));
							}
						} catch (NoSuchElementException noSuchElementException) {
							// do nothing
			...
			return (ITextFieldMaster[]) arrayList
					.toArray(new ITextFieldMaster[arrayList.size()]);
		} catch (Exception exception) {
			throw new TextException(exception);
		}
	}
	...
	/**
	 * Returns masters of the user textfields with the submitted name prefix and
	 * suffix.
	 * 
	 * @param prefix
	 *            name prefix to be used
	 ...
	 * 
	 * @return masters of the user textfields with the submitted name prefix and
	 *         suffix
	 * 
	 * @throws TextException
	 *             if the masters of the user textfields can not be constructed
	 ...
	 */
	public ITextFieldMaster[] getUserTextFieldMasters(String prefix,
			String suffix) throws TextException {
		if (prefix == null) {
			return EMPTY_TEXTFIELD_MASTER_ARRAY;
		}
	 ...
	 * 
	 * @throws TextException
	 *             if the placeholder textfields can not be constructed
	 * 
	 * @author Markus Krüger
	 * @date 23.01.2007
	 */
	...
	public ITextField[] getPlaceholderFields() throws TextException {
		try {
			// eingefügt von Jürgen Steinhilber
			PropertyValue[] pfs = textDocument.getXTextDocument().getArgs();
			for (int i = 0; i < pfs.length; i++) {
				if (pfs[i].Name.endsWith("Hidden")
						&& pfs[i].Value.equals(Boolean.valueOf(true))) {
					XRefreshable refresh = null;
					refresh = (XRefreshable) UnoRuntime
	 ...
	 * 
	 * @throws TextException
	 *             if the variable textfields can not be constructed
	 * 
	 * @author Markus Krüger
	 * @date 29.05.2007
	 */
