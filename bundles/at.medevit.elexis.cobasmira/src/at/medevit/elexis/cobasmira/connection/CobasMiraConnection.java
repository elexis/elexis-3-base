...
public class CobasMiraConnection implements PropertyChangeListener {
	...
	public boolean isActivated() {
		...
		if (cobasMiraReader != null) {
			...
			if (cobasMiraReaderState == Thread.State.NEW || cobasMiraReaderState == Thread.State.TERMINATED)
				return false;
		} else {
			return false;
		}
	...
	public static String[] getComPorts() {
		try {
			...
			Status status = new Status(IStatus.WARNING, Activator.PLUGIN_ID, "No class definition found", e);
			StatusManager.getManager().handle(status, StatusManager.SHOW);
			return new String[] { "No ports found" };
		}
	}
	...
	public String getStatus() {
		if (isActivated()) {
			return new String("Status: Cobas Mira verbunden auf Port "
					+ CoreHub.localCfg.get(Preferences.PORT, "/dev/cu.PL2303-0000103D") + ", Daten werden gesammelt.");
		} else {
