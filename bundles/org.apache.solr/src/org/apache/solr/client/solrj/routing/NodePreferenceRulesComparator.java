 ...
 * the given list of preferences. E.g. If all nodes prefer local cores then a
 * bad/heavily-loaded node will receive less requests from healthy nodes. This
 * will help prevent a distributed deadlock or timeouts in all the healthy nodes
 * due to one bad node.
 *
 * Optional final preferenceRule is *not* used for pairwise sorting, but instead
 * defines how "equivalent" replicas will be ordered (the base ordering).
...
public class NodePreferenceRulesComparator implements Comparator<Object> {
	...
	public NodePreferenceRulesComparator(final List<PreferenceRule> preferenceRules, final SolrParams requestParams,
		...
		if (!ShardParams.SHARDS_PREFERENCE_REPLICA_BASE.equals(lastRule.name)) {
			...
			this.baseReplicaListTransformer = defaultRltFactory.getInstance(null, requestParams,
			...
			case ShardParams.REPLICA_RANDOM:
				this.baseReplicaListTransformer = RequestReplicaListTransformerGenerator.RANDOM_RLTF
						.getInstance(parts.length == 1 ? null : parts[1], requestParams, null);
				break;
			case ShardParams.REPLICA_STABLE:
				this.baseReplicaListTransformer = stableRltFactory.getInstance(parts.length == 1 ? null : parts[1],
						requestParams, RequestReplicaListTransformerGenerator.RANDOM_RLTF);
				break;
			default:
	...
	private static final ReplicaListTransformer NOOP_RLT = (List<?> choices) -> {
		/* noop */ };
	private static final ReplicaListTransformerFactory NOOP_RLTF = (String configSpec, SolrParams requestParams,
			ReplicaListTransformerFactory fallback) -> NOOP_RLT;

	/**
	 * For compatibility with tests, which expect this constructor to have no effect
	 * on the *base* order.
	 */
