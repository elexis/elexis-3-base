 ...
 * The results of that query will be returned as tuples. An EOF tuple will
 * indicate that all have been read.
 *
 * Supported Datatypes for most types vary by JDBC driver based on the specific
 * java type as reported by
 * {@link java.sql.ResultSetMetaData#getColumnClassName(int)}. The exception are
 * {@link Types#DATE}, {@link Types#TIME} or {@link Types#TIMESTAMP} which are
 * determined by the JDBC type.
 *
 * <table rules="all" frame="box" cellpadding="3" summary="Supported Java
 * Types">
 * <tr>
...
public class JDBCStream extends TupleStream implements Expressible {
	...
	public JDBCStream(StreamExpression expression, StreamFactory factory) throws IOException {
		...
		for (StreamExpressionNamedParameter namedParam : namedParams) {
			if (!namedParam.getName().equals("driver") && !namedParam.getName().equals("connection")
		...
		if (null != definedSortExpression && definedSortExpression.getParameter() instanceof StreamExpressionValue) {
			definedSort = factory.constructComparator(
					((StreamExpressionValue) definedSortExpression.getParameter()).getValue(), FieldComparator.class);
		}
		if (null == definedSort) {
			throw new IOException(
					String.format(Locale.ROOT, "invalid expression %s - sort not found", definedSortExpression));
		}
	...
	private void init(String connectionUrl, String sqlQuery, StreamComparator definedSort,
			Properties connectionProperties, String driverClassName, int fetchSize) {
	...
	public void open() throws IOException {
		...
		try {
			...
			throw new IOException(String.format(Locale.ROOT, "Failed to load JDBC driver for '%s'", driverClassName),
					e);
		}

		// See if we can figure out the driver based on the url, if not then tell the
		// user they most likely want to provide the driverClassName.
		// Not being able to find a driver generally means the driver has not been
		// loaded.
		try {
			...
			throw new IOException(String.format(Locale.ROOT,
					"Failed to determine JDBC driver from connection url '%s'. Usually this means the driver is not loaded - "
							+ "you can have JDBCStream try to load it by providing the 'driverClassName' value",
					connectionUrl), e);
		}
		...
		try {
			...
			throw new IOException(String.format(Locale.ROOT, "Failed to open JDBC connection to '%s'", connectionUrl),
					e);
		}
		...
		try {
			...
			throw new IOException(
					String.format(Locale.ROOT, "Failed to create a statement from JDBC connection '%s'", connectionUrl),
					e);
		}
		...
		try {
			...
			throw new IOException(String.format(Locale.ROOT,
					"Failed to execute sqlQuery '%s' against JDBC connection '%s'.%nCaused by: %s", sqlQuery,
					connectionUrl, e.getMessage()), e);
		}
		...
		try {
			...
			throw new IOException(String.format(Locale.ROOT,
					"Failed to generate value selectors for sqlQuery '%s' against JDBC connection '%s'", sqlQuery,
					connectionUrl), e);
		}
	}
	...
	protected ResultSetValueSelector determineValueSelector(int columnIdx, ResultSetMetaData metadata)
		...
		// Use getColumnLabel instead of getColumnName to make sure fields renamed with
		// AS as picked up properly
		...
		// Directly supported types can be just directly returned - no conversion really
		// necessary
		...
		// We're checking the Java class names because there are lots of SQL types
		// across
		// lots of database drivers that can be mapped to standard Java types.
		// Basically,
		// this makes it easier and we don't have to worry about esoteric type names in
		// the
		...
		// Now we're going to start seeing if things are assignable from the returned
		// type
		// to a more general type - this allows us to cover cases where something we
		// weren't
	...
	public Tuple read() throws IOException {
		...
		try {
			...
			throw new IOException(
					String.format(Locale.ROOT, "Failed to read next record with error '%s'", e.getMessage()), e);
		}
	}
	...
	public StreamExpressionParameter toExpression(StreamFactory factory) throws IOException {
		...
		if (null != connectionProperties) {
			for (String propertyName : connectionProperties.stringPropertyNames()) {
				expression.addParameter(new StreamExpressionNamedParameter(propertyName,
						connectionProperties.getProperty(propertyName)));
			}
		}
	...
	public Explanation toExplanation(StreamFactory factory) throws IOException {
		...
		if (null == driverClassName) {
			try {
				...
				driverClassName = String.format(Locale.ROOT, "Failed to find driver for connectionUrl='%s'",
						connectionUrl);
			}
		}
	...
	public StreamComparator getStreamSort() {
		// TODO: Need to somehow figure out the sort applied to the incoming data. This
		// is not something you can ask a JDBC stream
		// Possibly we can ask the creator to tell us the fields the data is sorted by.
		// This would be duplicate information because
		// it's already in the sqlQuery but there's no way we can reliably determine the
		// sort from the query.
		return definedSort;
	}
