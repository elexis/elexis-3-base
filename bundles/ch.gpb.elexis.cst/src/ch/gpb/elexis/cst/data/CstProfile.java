...
public class CstProfile extends PersistentObject implements Comparable<CstProfile> {
	...
	final static String create = "CREATE TABLE " + TABLENAME + " (" + "`ID` VARCHAR(25) NOT NULL,	"
			+ "`lastupdate` BIGINT(20) NULL DEFAULT NULL,	" + "`deleted` CHAR(1) NULL DEFAULT '0',	"
			+ "`KontaktID` VARCHAR(25) NULL DEFAULT NULL,	" + "`MandantID` VARCHAR(25) NULL DEFAULT NULL,	"
			+ "`Name` VARCHAR(256) NULL DEFAULT NULL, " + "`Description` VARCHAR(256) NULL DEFAULT NULL,	"
			+ "`Icon` VARCHAR(25) NULL DEFAULT NULL,	" + "`ValidFrom` CHAR(8) NULL DEFAULT NULL,	"
			+ "`ValidTo` CHAR(8) NULL DEFAULT NULL,	" + "`Active` CHAR(1) NULL DEFAULT '1',	"
			+ "`Template` CHAR(1) NULL DEFAULT '0',	" + "`OutputHeader` VARCHAR(256) NULL DEFAULT NULL, "
			+ "`AusgabeRichtung` CHAR(1) NULL DEFAULT '0',	" + "`Auswahlbefunde` BLOB NULL,	"
			+ "`itemsRanking` BLOB NULL,	" + "`PlausibilityCheck` char(1) DEFAULT '0', "
			+ "`AnzeigeTyp` VARCHAR(50) NULL DEFAULT NULL,	" + "`CrawlBack` SMALLINT(6) NULL DEFAULT '180',	"
			+ "`DaySpan1` SMALLINT(6) NULL DEFAULT '720',	" + "`DaySpan2` SMALLINT(6) NULL DEFAULT '360',	"
			+ "`DaySpan3` SMALLINT(6) NULL DEFAULT '0',	" + "`Therapievorschlag` TEXT NULL,	"
			+ "`Period1DateStart` CHAR(8) NULL DEFAULT NULL,	" + "`Period1DateEnd` CHAR(8) NULL DEFAULT NULL,	"
			+ "`Period2DateStart` CHAR(8) NULL DEFAULT NULL,	" + "`Period2DateEnd` CHAR(8) NULL DEFAULT NULL,	"
			+ "`Period3DateStart` CHAR(8) NULL DEFAULT NULL,	" + "`Period3DateEnd` CHAR(8) NULL DEFAULT NULL,	"
			+ "PRIMARY KEY (`ID`)) COLLATe='utf8_general_ci' ENGINE=InnoDB; "

			+ "CREATE TABLE `cstgroup_profile_joint` (	" + "`ID` VARCHAR(25) NULL DEFAULT NULL,	"
			+ "`deleted` CHAR(1) NULL DEFAULT '0',	" + "`lastupdate` BIGINT(20) NULL DEFAULT NULL,	"
			+ "`CstgroupID` VARCHAR(25) NULL DEFAULT NULL,	" + "`ProfileID` VARCHAR(25) NULL DEFAULT NULL,	"
			+ "`Comment` TEXT NULL,	" + "UNIQUE INDEX `CstgroupID` (`CstgroupID`, `ProfileID`)) "
			+ "COLLAte='utf8_general_ci' ENGINE=InnoDB; " + "INSERT INTO " + TABLENAME + " (ID, name) VALUES ("
			+ JdbcLink.wrap(VERSIONID) + "," + JdbcLink.wrap(VERSION) + ");";

	static {
		addMapping(TABLENAME, "name=Name", "description=Description", "icon=Icon", "kontaktId=KontaktID",
				"mandantId=MandantID", "validFrom=ValidFrom", "validTo=ValidTo", "active=Active", "template=Template",
				"outputHeader=OutputHeader", "ausgabeRichtung=AusgabeRichtung", "itemsRanking=ItemsRanking",
				"therapievorschlag=Therapievorschlag", "diagnose=Diagnose", "anzeigeTyp=AnzeigeTyp",
				"plausibilityCheck=PlausibilityCheck", "crawlBack=CrawlBack", "daySpan1=DaySpan1", "daySpan2=DaySpan2",
				"daySpan3=DaySpan3", "period1DateStart=Period1DateStart", "period1DateEnd=Period1DateEnd",
				"period2DateStart=Period2DateStart", "period2DateEnd=Period2DateEnd",
				"period3DateStart=Period3DateStart", "period3DateEnd=Period3DateEnd",
		...
		if (!tableExists(TABLENAME)) {
			...
			if (vi.isOlder(VERSION)) {
				...
				/**/
				/*
				 * TODO: this create seems to be unnecessary in other examples of
				 * PersistenObject implementations, check this
				 */
	...
	public CstProfile(String name, String description, String icon, String mandantID) {
		// CstGroup existing = getByNameAndPatientAndMandant(name, kontaktID,
		// mandantID);
	...
	public static CstProfile getByNameAndPatientAndMandant(String name, String kontaktId, String mandantId) {
		...
		if (res.isEmpty()) {
			...
			if (res.size() > 1) {
				throw new IllegalArgumentException(
						String.format("There is already a category of name [%s] - [%s]", name));
			}
			return res.get(0);
		}
	}
	 ...
	 * Query<CstProfile> qbe = new Query<CstProfile>(CstProfile.class);
	 * qbe.add("ID", Query.NOT_EQUAL, VERSIONID); //$NON-NLS-1$ qbe.add("name",
	 * Query.EQUALS, name); qbe.add("profileId", Query.EQUALS, profileId);
	 * List<CstProfile> res = qbe.execute(); if (res.isEmpty()) { return null; }
	 * else { if (res.size() > 1) { throw new
	 * IllegalArgumentException(String.format(
	 * "There is already a category of name [%s] - [%s]", name)); } return
	 * res.get(0); } }
	 */
	 ...
	 * Query<CstProfile> qbe = new Query<CstProfile>(CstProfile.class);
	 * qbe.add("ID", Query.NOT_EQUAL, VERSIONID); qbe.add("itemId", Query.EQUALS,
	 * labItemId); return qbe.execute(); }
	 */

	/*
	 * public static List<CstProfile> getCstGroups(Patient patient) {
	 * Query<CstProfile> qbe = new Query<CstProfile>(CstProfile.class);
	 * qbe.add("ID", Query.NOT_EQUAL, VERSIONID); qbe.add("kontaktId", Query.EQUALS,
	 * patient.getId()); return qbe.execute(); }
	 */
	...
	/*
	 * public static List<CstProfile> getCstGroups() { Query<CstProfile> qbe = new
	 * Query<CstProfile>(CstProfile.class); qbe.add("ID", Query.NOT_EQUAL,
	 * VERSIONID); return qbe.execute(); }
	 */

	/*
	 * TODO: this should be renamed, on this level, the objects are CstGroup
	 * instances so getCstGroups would be adequate
	 */
	...
	/*
	 * TODO: this should be renamed, on this level, the objects are CstGroup
	 * instances so removeCstGroup would be adequate
	 */
	...
	public void addItem(CstGroup item) {
		if (item != null && (item.state() == EXISTS)) {
			...
			String exists = j.queryString("SELECT CstgroupID FROM " + GROUP_ITEM_TABLENAME + " WHERE ProfileID = "
			...
			if (StringTool.isNothing(exists)) {
				...
				throw new IllegalArgumentException(
						String.format("CSTGroup of name [%s] - [%s] already exists", item.getName()));
	...
	public boolean delete() {
		getConnection().exec("DELETE FROM " + GROUP_ITEM_TABLENAME + " WHERE ProfileID =" + getWrappedId());
		return super.delete();
	}
