...
public class CartesianProductStream extends TupleStream implements Expressible {
	...
	public CartesianProductStream(StreamExpression expression, StreamFactory factory) throws IOException {
		...
		List<StreamExpressionParameter> evaluateAsExpressions = factory.getOperandsOfType(expression,
		...
		if (expression.getParameters().size() != streamExpressions.size() + evaluateAsExpressions.size()
				+ (null == orderByExpression ? 0 : 1)) {
			throw new IOException(String.format(Locale.ROOT, "Invalid %s expression %s - unknown operands found",
					functionName, expression));
		}

		if (1 != streamExpressions.size()) {
			throw new IOException(String.format(Locale.ROOT,
					"Invalid %s expression %s - expecting single stream but found %d (must be TupleStream types)",
					functionName, expression, streamExpressions.size()));
		}

		stream = factory.constructStream(streamExpressions.get(0));
		orderBy = null == orderByExpression ? null
				: factory.constructComparator(((StreamExpressionValue) orderByExpression.getParameter()).getValue(),
		...
		for (StreamExpressionParameter evaluateAsExpression : evaluateAsExpressions) {
			...
			if (fullString.toLowerCase(Locale.ROOT).contains(" as ")) {
				...
				if (2 != parts.length) {
					throw new IOException(String.format(Locale.ROOT,
							"Invalid %s expression %s - expecting evaluator of form 'fieldA' or 'fieldA as alias' but found %s",
							functionName, expression, originalFullString));
				}
			...
			if (null == evaluator || null == asNamePart) {
				throw new IOException(
						String.format(Locale.ROOT, "Invalid %s expression %s - failed to parse evaluator '%s'",
								functionName, expression, originalFullString));
			}
	...
	private StreamExpression toExpression(StreamFactory factory, boolean includeStreams) throws IOException {
		...
		for (NamedEvaluator evaluator : evaluators) {
			expression.addParameter(String.format(Locale.ROOT, "%s as %s",
					evaluator.getEvaluator().toExpression(factory), evaluator.getName()));
		}
	...
	private boolean iterate(List<NamedEvaluator> evaluators, int[] indexes, Map<String, Object> evaluatedValues) {
		// this assumes evaluators and indexes are the same length, which is ok cause we
		// created it so we know it is
