...
public class XML45Printer {

	private static Logger logger = LoggerFactory.getLogger(XML45Printer.class);
	...
	public XML45Printer(TextContainer text) {
		this.text = text;
		tTime = new TimeTool();
	...
	private EZPrinter.EZPrinterData getEZPrintData(Balance45Adapter balance, ServicesType services, BodyType body) {
	...
	public boolean doPrint(Rechnung rn, Document xmlRn, TYPE rnType, String saveFile, boolean withESR,
		...
		Balance45Adapter balance = body.getTiersGarant() != null
				? new Balance45Adapter(body.getTiersGarant().getBalance())
				: new Balance45Adapter(body.getTiersPayant().getBalance());
		ServicesType services = body.getServices();
		EZPrinterData ezData = getEZPrintData(balance, services, body);

		String tcCode = null;
		IContact rsContact = CoreModelServiceHolder.get().load(rnSteller.getId(), IContact.class).orElse(null);
		...
		XMLPrinterUtil.createBrief(TarmedTemplateRequirement.TT_TARMED_45_S1, adressat, text);
		...
		SortedList<Object> serviceRecordsSorted = new SortedList<Object>(serviceRecords, new Rn45Comparator());
		...
		for (Object obj : serviceRecordsSorted) {
			...
			if (obj instanceof ServiceType) {
				recText = getRecordServiceString((ServiceType) obj, sb, eanMap);
				name = ((ServiceType) obj).getName();
			} else if (obj instanceof ServiceExType) {
				recText = getTarmedRecordString((ServiceExType) obj, sb, eanMap);
				name = ((ServiceExType) obj).getName();
			}
			...
			if (cmAvail <= cmPerLine) {
				...
				XMLPrinterUtil.insertPage(TarmedTemplateRequirement.TT_TARMED_45_S2, ++page, adressat, rn, xmlRn,
	...
	private void addSubTotalLine(Object cursor, ITextPlugin tp, Balance45Adapter balance, String tcCode, ESR esr) {
	...
	private void addESRCodeLine(Balance45Adapter balance, String tcCode, ESR esr) {
	...
	private boolean initBasicInvoiceValues(Rechnung rn, Document xmlRn) {
		...
		request = TarmedJaxbUtil.unmarshalInvoiceRequest450(xmlRn);
		if (request == null) {
	...
	private String[] initEanArray(List<Object> serviceRecords) {
		...
		for (Object record : serviceRecords) {
			...
			if (record instanceof ServiceType) {
				responsibleEAN = ((ServiceType) record).getResponsibleId();
				providerEAN = ((ServiceType) record).getProviderId();
			} else if (record instanceof ServiceExType) {
				responsibleEAN = ((ServiceExType) record).getResponsibleId();
				providerEAN = ((ServiceExType) record).getProviderId();
			}
	...
	private String getRecordServiceString(ServiceType rec, StringBuilder sb, HashMap<String, String> eanMap) {
		...
		sb.append(rec.getTariffType() != null ? rec.getTariffType() : StringUtils.EMPTY).append("\t");//$NON-NLS-1$
	...
	private String getTarmedRecordString(ServiceExType tarmed, StringBuilder sb, HashMap<String, String> eanMap) {
	...
	private void addBalanceLines(Object cursor, ITextPlugin tp, Balance45Adapter balance, Money paid) {
	...
	private Kontakt getAddressat(String paymentMode, Fall fall) {
		...
		default:
			IPatient patientContact = CoreModelServiceHolder.get().load(fall.getPatient().getId(), IPatient.class)
					.orElse(null);
			ICoverage coverage = CoreModelServiceHolder.get().load(fall.getId(), ICoverage.class).orElse(null);
			IContact guarantor = XMLExporterUtil.getGuarantor(XMLExporter.TIERS_GARANT, patientContact, coverage);
