...
/**
 * Defines a JoinStream which can hold N streams, all joined with the same
 * equalitor
 *
 * @since 6.0.0
 */
public abstract class JoinStream extends TupleStream implements Expressible {
	...
	public PushBackStream getStream(int idx) {
		...
		throw new IllegalArgumentException(String.format(Locale.ROOT,
				"Stream idx=%d doesn't exist. Number of streams is %d", idx, streams.size()));
	}

	protected boolean isValidTupleOrder() {
		// Validate that the equalitor is derivable from the comparator in each stream.
		// If it is, then we know all stream
		// comparators are
	...
	/**
	 * Given the stream, start from beginning and load group with all tuples that
	 * are equal to the first in stream (including the first one in the stream). All
	 * matched tuples are removed from the stream. Result is at least one tuple will
	 * be read from the stream and 0 or more tuples will exist in the group. If the
	 * first tuple is EOF then the group will have 0 items. Else it will have at
	 * least one item. The first group member is returned.
	 *
	 * @param group - should be empty
	 */
	protected Tuple loadEqualTupleGroup(PushBackStream stream, LinkedList<Tuple> group,
			StreamComparator groupComparator) throws IOException {
