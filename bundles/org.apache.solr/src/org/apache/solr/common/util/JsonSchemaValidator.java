 ...
 * This custom tool is created because a) we need to support non json inputs b)
 * to avoiding double parsing (this accepts an already parsed json as a map) It
 * validates most aspects of json schema but it is NOT A FULLY COMPLIANT JSON
 * schema parser or validator. This validator borrow some design's idea from
...
public class JsonSchemaValidator {
	...
	private static Set<String> KNOWN_FNAMES = new HashSet<>(
	...
	public JsonSchemaValidator(Map jsonSchema) {
		...
		for (Object fname : jsonSchema.keySet()) {
			if (KNOWN_FNAMES.contains(fname.toString()))
				continue;
...
enum Type {
	STRING(String.class), ARRAY(List.class), NUMBER(Number.class), INTEGER(Long.class) {
		@Override
	...
	ENUM(List.class), OBJECT(Map.class), NULL(null), UNKNOWN(Object.class);
			...
			for (String requiredProp : requiredProps) {
				if (requiredProp.contains(".")) {
					if (requiredProp.endsWith(".")) {
						errs.add("Illegal required attribute name (ends with '.': " + requiredProp
								+ ").  This is a bug.");
						return false;
					}
					...
					if (!fnames.contains(requiredProp)) {
						errs.add(
								"Missing required attribute '" + requiredProp + "' in object " + Utils.toJSONString(o));
						return false;
					}
				}
			}
...
class OneOfValidator extends Validator<List<String>> {
	...
	boolean validate(Object o, List<String> errs) {
		if (o instanceof Map) {
			...
			Map map = (Map) o;
			for (Object key : map.keySet()) {
				if (oneOfProps.contains(key.toString()))
					return true;
			}
