 ...
 * collections, and then use the {@link LBHttpSolrClient} to issue requests.
 *
 * This class assumes the id field for your documents is called 'id' - if this
 * is not the case, you must set the right name with
...
public class CloudSolrClient extends BaseCloudSolrClient {
	...
	/**
	 * Create a new client object that connects to Zookeeper and is always aware of
	 * the SolrCloud state. If there is a fully redundant Zookeeper quorum and
	...
	protected Map<String, LBHttpSolrClient.Req> createRoutes(UpdateRequest updateRequest,
	...
	public static class Builder extends SolrClientBuilder<Builder> {
		...
		/**
		 * @deprecated use other constructors instead. This constructor will be changing
		 *             visibility in an upcoming release.
		 */
		@Deprecated
		public Builder() {
		}
		 ...
		 * {@link CloudSolrClient} instances. The solr client will use these urls to
		 * understand the cluster topology, which solr nodes are active etc.
		 *
		 ...
		 * cores, or other path components.
		 *
		 * Usage example:
		 *
		 * <pre>
		 ...
		 * {@link CloudSolrClient} instances.
		 *
		 ...
		 *                 "zookeeper1:2181")
		 * @param zkChroot the path to the root ZooKeeper node containing Solr data.
		 *                 Provide {@code java.util.Optional.empty()} if no ZK chroot is
		 *                 used.
		 */
		 ...
		 *
		 * @param zkHost The client endpoint of the ZooKeeper quorum containing the
		 *               cloud state.
		 *
		...
		/**
		 * Provide a Solr URL to be used when configuring {@link CloudSolrClient}
		 * instances.
		 *
		 * Method may be called multiple times. One of the provided values will be used
		 * to fetch the list of live Solr nodes that the underlying
		 * {@link HttpClusterStateProvider} would be maintaining.
		 *
		 * Provided Solr URL is expected to point to the root Solr path
		 * ("http://hostname:8983/solr"); it should not include any collections, cores,
		 ...
		 * {@link CloudSolrClient} instances. One of the provided values will be used to
		 * fetch the list of live Solr nodes that the underlying
		 * {@link HttpClusterStateProvider} would be maintaining.
		 *
		 * Provided Solr URLs are expected to point to the root Solr path
		 * ("http://hostname:8983/solr"); they should not include any collections,
		...
		/**
		 * Provide a series of ZooKeeper client endpoints for the builder to use when
		 * creating clients.
		 *
		 * Method may be called multiple times. All provided values will be used.
		 *
		 * @param zkHosts A Java Collection (List, Set, etc) of HOST:PORT strings, one
		 *                for each host in the ZooKeeper ensemble. Note that with
		 *                certain Collection types like HashSet, the order of hosts in
		 *                the final connect string may not be in the same order you
		 *                added them.
		 *
		 * @deprecated use Zk-host constructor instead
		 */
		...
		/**
		 * Provides a {@link LBHttpSolrClient} for the builder to use when creating
		 * clients.
		 */
		...
		/**
		 * Tells {@link Builder} that created clients should send updates only to shard
		 * leaders.
		 *
		 * WARNING: This method currently has no effect. See SOLR-6312 for more
		 * information.
		 */
		...
		/**
		 * Tells {@link Builder} that created clients should send updates to all
		 * replicas for a shard.
		 *
		 * WARNING: This method currently has no effect. See SOLR-6312 for more
		 * information.
		 */
		...
		/**
		 * Tells {@link Builder} that created clients should send direct updates to
		 * shard leaders only.
		 *
		 * UpdateRequests whose leaders cannot be found will "fail fast" on the client
		 * side with a {@link SolrException}
		 */
		...
		/**
		 * Tells {@link Builder} that created clients can send updates to any shard
		 * replica (shard leaders and non-leaders).
		 *
		 * Shard leaders are still preferred, but the created clients will fallback to
		 * using other replicas if a leader cannot be found.
		 */
		...
		/**
		 * Tells {@link Builder} whether created clients should send shard updates
		 * serially or in parallel
		 *
		 * When an {@link UpdateRequest} affects multiple shards,
		 * {@link CloudSolrClient} splits it up and sends a request to each affected
		 * shard. This setting chooses whether those sub-requests are sent serially or
		 * in parallel.
		 * <p>
		...
		/**
		 * Expert feature where you want to implement a custom cluster discovery
		 * mechanism of the solr nodes as part of the cluster.
		 *
		 * @deprecated since this is an expert feature we don't want to expose this to
		 *             regular users. To use this feature extend CloudSolrClient.Builder
		 *             and pass your custom ClusterStateProvider
		 */
		@Deprecated
