...
public class DiagnoselisteItem extends PersistentObject {
	...
	static {
		addMapping(TABLENAME, FLD_PATIENT, FLD_TYP, FLD_SOURCE, FLD_PARENT, FLD_POSITION, "Datum=S:D:Datum", FLD_ICPC,
				FLD_TEXT);
		checkTable();
	}
	...
	private static final String create = "CREATE TABLE " + TABLENAME + " ("
			...
			+ "  deleted		CHAR(1) DEFAULT '0', " + "  Patient		VARCHAR(25), "
			+ "  Typ            INT DEFAULT 0, " + "  Source	 		VARCHAR(25), "
			+ "  Parent	 		VARCHAR(25), " + "  Position		INT DEFAULT 0, "
			+ "  Datum          CHAR(8) DEFAULT '00000000', " + "  ICPC           TEXT, " + "  Text			TEXT "
			+ ");" + "INSERT INTO " + TABLENAME + " (ID, Position) VALUES " + "	('VERSION', '" + VERSION + "');";
	...
	private static final String up_1to2 = "ALTER TABLE " + TABLENAME
			...
			+ "  ADD ICPC   TEXT                        AFTER Position;" + "UPDATE " + TABLENAME
			+ " SET Position = '2' WHERE" + "  ID LIKE 'VERSION';";

	private static void checkTable() {
		String fm = null;
		try {
			fm = getConnection().queryString("SELECT Position FROM " + TABLENAME + " WHERE ID='VERSION';");
			if (fm.equals("1"))
	...
	public List<DiagnoselisteItem> getChildren() {
		Query<DiagnoselisteItem> q = new Query<DiagnoselisteItem>(DiagnoselisteItem.class);
	...
	/**
	 * Schiebt das Element c ganz nach unten, so dass es einfach geloescht werden
	 * kann.
	 *
