...
public class RequestReplicaListTransformerGenerator {
	...
	private static final ReplicaListTransformer shufflingReplicaListTransformer = new ShufflingReplicaListTransformer(
			r);
	public static final ReplicaListTransformerFactory RANDOM_RLTF = (String configSpec, SolrParams requestParams,
	...
	public RequestReplicaListTransformerGenerator(String defaultShardPreferences, String nodeName,
			String localHostAddress, NodesSysPropsCacher sysPropsCacher) {
		this(null, null, defaultShardPreferences, nodeName, localHostAddress, sysPropsCacher);
	}

	public RequestReplicaListTransformerGenerator(ReplicaListTransformerFactory defaultRltFactory,
			ReplicaListTransformerFactory stableRltFactory, String defaultShardPreferences, String nodeName,
			String localHostAddress, NodesSysPropsCacher sysPropsCacher) {
		this.defaultRltFactory = Optional.ofNullable(defaultRltFactory).orElse(RANDOM_RLTF);
		this.stableRltFactory = Optional.ofNullable(stableRltFactory)
				.orElseGet(AffinityReplicaListTransformerFactory::new);
	...
	public ReplicaListTransformer getReplicaListTransformer(final SolrParams requestParams,
			String defaultShardPreferences, String nodeName, String localHostAddress,
		...
		if (preferLocalShards || !shardsPreferenceSpec.isEmpty()) {
			...
			if (preferLocalShards) {
				preferenceRules.add(
						new PreferenceRule(ShardParams.SHARDS_PREFERENCE_REPLICA_LOCATION, ShardParams.REPLICA_LOCAL));
			}

			NodePreferenceRulesComparator replicaComp = new NodePreferenceRulesComparator(preferenceRules,
					requestParams, Optional.ofNullable(nodeName).orElse(this.nodeName),
					Optional.ofNullable(localHostAddress).orElse(this.localHostAddress),
					Optional.ofNullable(sysPropsCacher).orElse(this.sysPropsCacher), defaultRltFactory,
					stableRltFactory);
	 ...
	 * replica attributes, and subsequently reordering any equivalent replica sets
	 * according to behavior specified by the baseReplicaListTransformer.
	 */
	private static final class TopLevelReplicaListTransformer implements ReplicaListTransformer {
		...
		public TopLevelReplicaListTransformer(NodePreferenceRulesComparator replicaComp,
		...
		public void transform(List<?> choices) {
			if (choices.size() > 1) {
				...
				// Next determine all boundaries between replicas ranked as "equivalent" by the
				// comparator
				...
				// Finally inspect boundaries to apply base transformation, where necessary
				// (separate phase to avoid ConcurrentModificationException)
				int startIdx = 0;
				int endIdx;
