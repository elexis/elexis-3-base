...
public class TardocRefcodesDialog extends Dialog {
	...
	public TardocRefcodesDialog(Shell shell, IBilled tl) {
		super(shell);
	...
	private void addRefcodeEdit() {
		...
		TardocRefcodesDialog.this.getShell().pack(true);
	}
	...
	public void create() {
		super.create();
		getShell().setText("Tardocbezüge herstellen: " + billed.getCode());
	}
	...
	private class RefCodeEditComposite extends Composite {
		...
		private void createContent() {
			...
			amountSpinner.setValues(0, 0, (int) TardocRefcodesDialog.this.billed.getAmount(), 0, 1, 1);
		...
		private List<String> getPossibleRefCodes() {
			...
			if (!encounters.isEmpty()) {
				...
				encounters.forEach(e -> {
					List<String> codes = e.getBilled().stream().filter(b -> b.getBillable() instanceof ITardocLeistung)
