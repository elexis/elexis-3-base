	...
	public static XMLExporterTiers buildTiers(IInvoice invoice, XMLExporter xmlExporter) {
		...
		if (StringUtils.isNotBlank(ConfigServiceHolder.getGlobal(PreferenceConstants.TARMEDBIL_FIX_PROVIDER, null))) {
			IContact contact = CoreModelServiceHolder.get()
					.load(ConfigServiceHolder.getGlobal(PreferenceConstants.TARMEDBIL_FIX_PROVIDER, null),
							IContact.class)
					.get();
		...
		Element guarantor = xmlExporter.buildGuarantor(XMLExporterUtil.getGuarantor(tiers, patient, coverage), patient);
		...
		if (auftraggeber != null) {
			...
			if (ean != null && !ean.isEmpty()) {
				referrer.setAttribute(XMLExporter.ATTR_EAN_PARTY, TarmedRequirements.getEAN(auftraggeber)); // auftraggeber.
			}
