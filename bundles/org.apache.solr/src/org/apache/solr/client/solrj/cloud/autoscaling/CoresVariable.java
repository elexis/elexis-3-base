...
public class CoresVariable extends VariableBase {
	...
	public boolean addViolatingReplicas(Violation.Ctx ctx) {
		for (Row row : ctx.allRows) {
			if (row.node.equals(ctx.currentViolation.node)) {
				row.forEachReplica(
						replicaInfo -> ctx.currentViolation.addReplica(new Violation.ReplicaInfoAndErr(replicaInfo)
	...
	public void getSuggestions(Suggestion.Ctx ctx) {
		...
		if (ctx.violation.replicaCountDelta > 0) {// there are more replicas than necessary
			for (int i = 0; i < Math.abs(ctx.violation.replicaCountDelta); i++) {
				if (!ctx.needMore())
					return;
				Suggester suggester = ctx.session.getSuggester(MOVEREPLICA).hint(Suggester.Hint.SRC_NODE,
						ctx.violation.node);
	...
	public Object computeValue(Policy.Session session, Condition condition, String collection, String shard,
		...
		if (condition.computedType == ComputedType.EQUAL) {
			...
			int numBuckets = condition.clause.tag.op == Operand.IN ? ((Collection) condition.clause.tag.val).size()
					: liveNodes.get();
