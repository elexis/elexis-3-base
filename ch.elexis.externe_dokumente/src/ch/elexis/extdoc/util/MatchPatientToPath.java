...
public class MatchPatientToPath {
	...
	public static String[] getFirstAndFamilyNameFromPathOldConvention(String fullPathname){
		...
		String lastname = cleanName(basename.substring(0, 6));
		if (basename.substring(6).split("[. \\s]", 0).length < 2)
			return new String[] {
				basename, ""
			};
		String firstname = basename.substring(6).split("[. \\s]", 0)[0];
		return new String[] {
			firstname, lastname
		};
	}
	...
	/***
	 * Global: Checks whether the pathname, first and lastname match our conventions Checks basename
	...
	public static String getSubDirPath(Patient pat){
		String s =
			CoreHub.localCfg.get(PreferenceConstants.BASIS_PFAD1, "") + File.separatorChar //$NON-NLS-1$
	...
	public String ShouldBeMovedToThisSubDir(String oldPathname, String geburtsDatum){
		...
		return dirname + File.separatorChar + (new File(getSubDirPath(pat)).getName())
