...
public class StreamExpressionParser {
	...
	private static StreamExpressionParameter generateStreamExpression(String clause) {
		...
		if (!isExpressionClause(working)) {
			throw new IllegalArgumentException(
					String.format(Locale.ROOT, "'%s' is not a proper expression clause", working));
		}
	...
	private static StreamExpressionNamedParameter generateNamedParameterExpression(String clause) {
		...
		// might be overkill as the only place this is called from does this check
		// already
		...
		StreamExpressionNamedParameter namedParameter = new StreamExpressionNamedParameter(
				working.substring(0, firstOpenEquals).trim());
	 ...
	 * mean it begins with ( and ends with ) Expects that the passed in clause has
	 * already been trimmed of leading and trailing spaces
	 */
	private static boolean isExpressionClause(String clause) {
		// operator(.....something.....)

		// must be balanced
		if (!isBalanced(clause)) {
			return false;
		}

		// find first (, then check from start to that location and only accept
		// alphanumeric
	...
	/*
	 * Finds index of the next char equal to findThis that is not within a quote or
	 * set of parens Does not work with the following values of findThis: " ' \ ) --
	 * well, it might but wouldn't really give you what you want. Don't call with
	 * those characters
	 */
	private static int findNextClear(String clause, int startingIdx, char findThis) {
		...
		for (int idx = startingIdx; idx < clause.length(); ++idx) {
			...
			// if we're not in a non-escaped quote or paren state, then we've found the
			// space we want
			...
			case '"':
				// if we're not in a non-escaped single quote state, then invert the double
				// quote state
			...
			case '\'':
				// if we're not in a non-escaped double quote state, then invert the single
				// quote state
			...
			case '(':
				// if we're not in a non-escaped quote state, then increment the # of open
				// parens
			...
			case ')':
				// if we're not in a non-escaped quote state, then decrement the # of open
				// parens
				if (!isEscaped && !isSingleQuote && !isDoubleQuote) {
					openParens -= 1;
				}
	...
	/*
	 * Returns a list of the tokens found. Assumed to be of the form 'foo bar baz'
	 * and not of the for '(foo bar baz)' 'foo bar (baz jaz)' is ok and will return
	 * three tokens of 'foo', 'bar', and '(baz jaz)'
	 */
