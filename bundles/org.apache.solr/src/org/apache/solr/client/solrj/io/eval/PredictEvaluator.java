...
public class PredictEvaluator extends RecursiveObjectEvaluator implements ManyValueWorker {
	...
	public Object doWork(Object... objects) throws IOException {
		...
		if (!(first instanceof BivariateFunction) && !(first instanceof VectorFunction)
		...
		if (first instanceof RegressionEvaluator.RegressionTuple) {
			...
			if (second instanceof Number) {
				...
				return ((List<?>) second).stream().map(value -> regressedTuple.predict(((Number) value).doubleValue()))
