...
public class DeleteOldData {
	...
	public IStatus delete(IProgressMonitor ipm){
		...
		affected = 0;
		if (law != null && !law.isEmpty()) {
			affected = ArzttarifeModelServiceHolder.get().executeNativeUpdate(
				"DELETE FROM TARMED_DEFINITIONEN WHERE Law='" + law + "'  AND ID <> '" //$NON-NLS-1$//$NON-NLS-2$
					+ VersionUtil.VERSION_ENTRY_ID + "'");
		} else {
			affected = ArzttarifeModelServiceHolder.get().executeNativeUpdate(
		...
		if (law != null && !law.isEmpty()) {
			affected = ArzttarifeModelServiceHolder.get().executeNativeUpdate(
				"DELETE FROM TARMED_KUMULATION WHERE Law='" + law + "' AND ID <> '" //$NON-NLS-1$//$NON-NLS-2$
					+ VersionUtil.VERSION_ENTRY_ID + "'");
		} else {
			affected = ArzttarifeModelServiceHolder.get().executeNativeUpdate(
				"DELETE FROM TARMED_KUMULATION WHERE (Law='' or Law IS NULL) AND ID <> '" //$NON-NLS-1$
					+ VersionUtil.VERSION_ENTRY_ID + "'");
		}
		...
		if (law != null && !law.isEmpty()) {
			...
			affected = ArzttarifeModelServiceHolder.get().executeNativeUpdate(
				"DELETE FROM TARMED_GROUP WHERE (Law='' or Law IS NULL) AND ID <> '" //$NON-NLS-1$
					+ VersionUtil.VERSION_ENTRY_ID + "'");
		}
