...
public class ValuePairTimelineCanvas extends Canvas {
	...
	void paintControl(PaintEvent e) {
		...
		// System.out.println("iPixX: " + iPixX);
		...
		for (int i = 1; i < nrOfScaleLines + 1; i++) {
			...
			gc.drawLine(xoffBase, yoffBase - new Double((i * scaleStepWidth) * yFactor).intValue(), iPixX,
					yoffBase - new Double((i * scaleStepWidth) * yFactor).intValue());

			gc.drawText(String.valueOf(i * scaleStepWidth), iPixX + 25,
					yoffBase - new Double((i * scaleStepWidth) * yFactor).intValue() - 6, true);
		...
		if (findings.size() == 1) {
			...
			gc.setForeground(BLUE);
			gc.drawLine(xoff, yoffBase - new Double(findings.get(0).getWert1() * yFactor).intValue(), xoff + iPixX,
					yoffBase - new Double(findings.get(0).getWert1() * yFactor).intValue());

			gc.drawText(String.valueOf(findings.get(0).getWert1()), xoffBase + new Double(iPixX / 2).intValue(),
					yoffBase - new Double(findings.get(0).getWert1() * yFactor).intValue(), true);

			gc.drawText(String.valueOf(findings.get(0).getWert2()), xoffBase + new Double(iPixX / 2).intValue(),
					yoffBase - new Double(findings.get(0).getWert2() * yFactor).intValue(), true);
			...
			for (int x = 0; x < findings.size(); x++) {
				...
				ValuePairTimeline finding = findings.get(x);
				...
				gc.drawText(String.valueOf(finding.getWert1()), xoffBase + new Double(xoff * xFactor).intValue(),
						yoffBase - new Double(finding.getWert1() * yFactor).intValue() - corrY, true);

				gc.setForeground(ORANGE);

				gc.drawText(String.valueOf(finding.getWert2()), xoffBase + new Double(xoff * xFactor).intValue(),
						yoffBase - new Double(finding.getWert2() * yFactor).intValue() - corrY, true);

				gc.setForeground(BLACK);

				if (x < findings.size() - 1) {
					long lSpan = CstService.getNrOfDaysBetween(CstService.getDateFromCompact(findings.get(0).getDate()),
	...
	private double getHighestValueInFindings() {
		...
		double highest = 0;
		for (int x = 0; x < findings.size(); x++) {
			if (findings.get(x).getWert1() > highest) {
				highest = findings.get(x).getWert1();
			}
			if (findings.get(x).getWert2() > highest) {
				highest = findings.get(x).getWert2();
			}
		}
		return highest;
	...
	private double getLowestValueInFindings() {
		...
		for (int x = 1; x < findings.size(); x++) {
			if (findings.get(x).getWert1() < lowest) {
				lowest = findings.get(x).getWert1();
			}
			if (findings.get(x).getWert2() < lowest) {
				lowest = findings.get(x).getWert2();
			}
		}
		return lowest;
	...
	public void setFindings(List<ValuePairTimeline> findings) {
		...
		/*
		 * for (ValuePairTimeline finding : findings) { System.out.println("BdValue: " +
		 * finding.getDate() + "syst/diast:" +finding.getWeightKg()); }
		 */
		...
		scaleStepWidth = rounded / 10;

		// the formula doesn't work for very small values, so we correct:
		...
		/*
		 * if (highest > 0 && highest < 10) { rounded = ((num + 9) / 10) * 10;
		 * scaleStepWidth = 1; } else if (highest > 10 && highest < 100) { rounded =
		 * ((num + 99) / 100) * 100; scaleStepWidth = 10; } else if (highest > 100 &&
		 * highest < 5000) { rounded = ((num + 499) / 5000) * 5000; scaleStepWidth =
		 * 200; } else if (highest > 100 && highest < 1000) { rounded = ((num + 990) /
		 * 1000) * 1000; scaleStepWidth = 100; }
		 *
		 * else if (highest > 1000 && highest < 10000) { rounded = ((num + 9999) /
		 * 10000) * 10000; scaleStepWidth = 1000;
		 *
		 * }
		 */
