...
public class GlobalInboxEntryDetailPart {
	...
	public GlobalInboxEntryDetailPart(Composite parent, IConfigService configService, EHandlerService handlerService) {
		...
		ContentProposalAdapter titleContentProposalAdapter = new ContentProposalAdapter(txtTitle,
				new TitleControlContentAdapter(txtTitle), new TitleContentProposalProvider(txtTitle), null, null);
		...
		archivingDate = new CDateTime(parent, CDT.DATE_SHORT | CDT.DROP_DOWN | SWT.BORDER | CDT.TAB_FIELDS);
		...
		GridData gdlblHintCreationDateCandidateImage = new GridData(SWT.FILL, SWT.CENTER, true, false, 2, 1);
		...
		GridData gdlblHintPatientCandiateInfo = new GridData(SWT.FILL, SWT.CENTER, true, false, 2, 1);
		...
		cvPatient.addSelectionChangedListener(sc -> {
			...
			if (patient != null) {
				...
				if (familyDoctor != null) {
					mandator = CoreModelServiceHolder.get().load(familyDoctor.getId(), IMandator.class).orElse(null);
				} else {
					mandator = EncounterServiceHolder.get().getLatestEncounter(patient).map(enc -> enc.getMandator())
							.orElse(null);
				}
				if (mandator != null) {
		...
		linkSender.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				KontaktSelektor kontaktSelektor = new KontaktSelektor(linkPatient.getShell(), Kontakt.class,
						Messages.Core_Select_Contact, Messages.Core_Please_Select_Contact,
