...
public class SolrStream extends TupleStream {
	...
	/**
	 * Setting trace to true will include the "_CORE_" field in each Tuple emitted
	 * by the stream.
	 **/
	...
	/**
	 * Reads a Tuple from the stream. The Stream is completed when Tuple.EOF ==
	 * true.
	 **/
	...
	public Tuple read() throws IOException {
		try {
			...
			// The Stream source did not provide an exception in a format that the
			// SolrStream could propagate.
			throw new IOException("--> " + this.baseUrl
					+ ": An exception has occurred on the server, refer to server log for details.", e);
		}
	}
	 ...
	 * @throws IOException         if an I/O related error occurs contacting the
	 *                             remote Solr instance
	 * @throws SolrServerException if an error occurs contacting the remote Solr
	 *                             instance
	 */
	@Deprecated
	...
	private TupleStreamParser constructParser(SolrParams requestParams) throws IOException, SolrServerException {
		...
		// in order to reuse HttpSolrClient objects per node, we need to cache them
		// without the core name in the URL
		if (core != null) {
		...
		if (statusCode != 200) {
			...
			httpResponse.close();
			throw new IOException("Query to '" + query.getPath() + "?" + query.getParams() + "' failed due to: ("
					+ statusCode + ") " + errMsg);
		}
