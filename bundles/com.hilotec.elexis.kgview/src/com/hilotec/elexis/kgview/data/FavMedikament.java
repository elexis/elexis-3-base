...
public class FavMedikament extends PersistentObject {
	...
	private static void checkTable() {
		...
		String fm = null;
		try {
			fm = getConnection().queryString("SELECT Bezeichnung FROM " + TABLENAME + " WHERE ID='VERSION';");
		} catch (Exception e) {
		}
	...
	public static FavMedikament load(final String id) {
		...
		// werden.
		String fm = getConnection().queryString("SELECT Bezeichnung FROM " + TABLENAME + " WHERE ID='" + id + "';");
	...
	/**
	 * Erstellt neues FM mit selben Eigenschaften aber neuem verknuepftem Artikel,
	 * das original FM wird geloescht.
	 */
	public FavMedikament relinkTo(Artikel a) {
		FavMedikament fm = new FavMedikament(a, getOrdnungszahl(), getBezeichnung(), getZweck(), getEinheit());
		delete();
		return fm;
	}
}
