	...
	static void suggestNegativeViolations(Suggestion.Ctx ctx, Function<Set<String>, List<String>> shardSorter) {
		...
		for (Row node : ctx.session.matrix)
			node.forEachShard(ctx.violation.coll, (s, ri) -> {
				if (Policy.ANY.equals(ctx.violation.shard) || s.equals(ctx.violation.shard))
					shardSet.add(s);
			});
		...
		outer: for (int i = 0; i < 5; i++) {
			...
			for (String shard : shards) {
				Suggester suggester = ctx.session.getSuggester(MOVEREPLICA)
						.hint(Suggester.Hint.COLL_SHARD, new Pair<>(ctx.violation.coll, shard)).forceOperation(true);
				@SuppressWarnings({ "rawtypes" })
	...
	static void suggestPositiveViolations(Ctx ctx) {
		...
		for (ReplicaInfoAndErr e : ctx.violation.getViolatingReplicas()) {
			...
			Suggester suggester = ctx.session.getSuggester(MOVEREPLICA).forceOperation(true)
					.hint(Suggester.Hint.COLL_SHARD,
							new Pair<>(e.replicaInfo.getCollection(), e.replicaInfo.getShard()))
