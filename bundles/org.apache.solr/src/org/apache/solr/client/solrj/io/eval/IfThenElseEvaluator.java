...
public class IfThenElseEvaluator extends RecursiveObjectEvaluator implements ManyValueWorker {
	...
	public IfThenElseEvaluator(StreamExpression expression, StreamFactory factory) throws IOException {
		...
		if (3 != containedEvaluators.size()) {
			throw new IOException(
					String.format(Locale.ROOT, "Invalid expression %s - expecting three values but found %d",
	...
	public Object doWork(Object... values) throws IOException {
		if (3 != values.length) {
			throw new IOException(String.format(Locale.ROOT, "%s(...) only works with 3 values but %d were provided",
					constructingFactory.getFunctionName(getClass()), values.length));
		}

		if (!(values[0] instanceof Boolean)) {
			throw new IOException(
					String.format(Locale.ROOT, "$s(...) only works with a boolean as the first parameter but found %s",
