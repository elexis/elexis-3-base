...
/**
 * Helper-Klasse fuer Code der auf dem Laufenden bleiben muss bezueglich eines
 * aktuell ausgewaehlten PersistentObjects.
 */
public abstract class POSelectionListener<P extends PersistentObject> implements ElexisEventListener {
	...
	public void init() {
		...
		pclass = (Class<P>) ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0];

		eetmpl = new ElexisEvent(null, pclass,
				ElexisEvent.EVENT_SELECTED | ElexisEvent.EVENT_DESELECTED | ElexisEvent.EVENT_DELETE);
	...
	public void catchElexisEvent(ElexisEvent ev) {
		...
		if (ev.getType() == ElexisEvent.EVENT_DESELECTED && current != null) {
			...
			current = null;
		} else if (ev.getType() == ElexisEvent.EVENT_DELETE && k != null) {
	...
	/**
	 * Wird aufgerufen wenn das aktuelle PersistentObject nicht mehr ausgewaehlt
	 * ist. Wird im GUI Thread gestartet.
	 */
	protected void deselected(P po) {
	}

	/**
	 * Wird aufgerufen wenn ein neues PersistentObject ausgewaehlt wurde. Wird im
	 * GUI Thread gestartet.
	 */
