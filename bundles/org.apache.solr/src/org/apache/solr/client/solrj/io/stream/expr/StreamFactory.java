...
public class StreamFactory implements Serializable {
	...
	public List<String> getValueOperands(StreamExpression expression) {
		return getOperandsOfType(expression, StreamExpressionValue.class).stream()
				.map(item -> ((StreamExpressionValue) item).getValue()).collect(Collectors.toList());
	}

	/**
	 * Given an expression, will return the value parameter at the given index, or
	...
	public List<StreamExpressionNamedParameter> getNamedOperands(StreamExpression expression) {
		...
		for (StreamExpressionParameter parameter : getOperandsOfType(expression,
				StreamExpressionNamedParameter.class)) {
	...
	public List<StreamExpressionParameter> getOperandsOfType(StreamExpression expression,
		...
		parameterLoop: for (StreamExpressionParameter parameter : expression.getParameters()) {
			for (@SuppressWarnings({ "rawtypes" })
			Class clazz : clazzes) {
				if (!clazz.isAssignableFrom(parameter.getClass())) {
					continue parameterLoop; // go to the next parameter since this parameter cannot be assigned to at
	...
	public List<StreamExpression> getExpressionOperandsRepresentingTypes(StreamExpression expression,
		...
		parameterLoop: for (StreamExpression streamExpression : allStreamExpressions) {
			Supplier<Class<? extends Expressible>> classSupplier = functionNames
					.get(streamExpression.getFunctionName());
	...
	public int getIntOperand(StreamExpression expression, String paramName, Integer defaultValue) throws IOException {
		...
		if (null == param || null == param.getParameter() || !(param.getParameter() instanceof StreamExpressionValue)) {
			...
			throw new IOException(String.format(Locale.ROOT,
					"Invalid expression %s - expecting a single '%s' parameter of type integer but didn't find one",
		...
		try {
			...
			throw new IOException(String.format(Locale.ROOT, "invalid expression %s - %s '%s' is not a valid integer.",
					expression, paramName, nStr));
		}
	}

	public boolean getBooleanOperand(StreamExpression expression, String paramName, Boolean defaultValue)
			throws IOException {
		StreamExpressionNamedParameter param = getNamedOperand(expression, paramName);
		...
		if (null == param || null == param.getParameter() || !(param.getParameter() instanceof StreamExpressionValue)) {
			...
			throw new IOException(String.format(Locale.ROOT,
					"Invalid expression %s - expecting a single '%s' parameter of type boolean but didn't find one",
					expression, paramName));
		}
		String nStr = ((StreamExpressionValue) param.getParameter()).getValue();
	...
	public TupleStream constructStream(StreamExpression expression) throws IOException {
		...
		if (classSupplier != null) {
			...
			if (Expressible.class.isAssignableFrom(clazz) && TupleStream.class.isAssignableFrom(clazz)) {
				return (TupleStream) createInstance(clazz, new Class[] { StreamExpression.class, StreamFactory.class },
						new Object[] { expression, this });
			}
		}

		throw new IOException(String.format(Locale.ROOT,
				"Invalid stream expression %s - function '%s' is unknown (not mapped to a valid TupleStream)",
	...
	public Metric constructMetric(StreamExpression expression) throws IOException {
		...
		if (classSupplier != null) {
			...
			if (Expressible.class.isAssignableFrom(clazz) && Metric.class.isAssignableFrom(clazz)) {
				return (Metric) createInstance(clazz, new Class[] { StreamExpression.class, StreamFactory.class },
						new Object[] { expression, this });
			}
		}

		throw new IOException(String.format(Locale.ROOT,
				"Invalid metric expression %s - function '%s' is unknown (not mapped to a valid Metric)", expression,
				expression.getFunctionName()));
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	public StreamComparator constructComparator(String comparatorString,
			@SuppressWarnings({ "rawtypes" }) Class comparatorType) throws IOException {
		if (comparatorString.contains(",")) {
			...
			if (parts.length < 3) {
				throw new IOException(String.format(Locale.ROOT,
						"Invalid comparator expression %s - expecting 'left=right order'", comparatorString));
			}
			...
			return (StreamComparator) createInstance(comparatorType,
					new Class[] { String.class, String.class, ComparatorOrder.class },
					new Object[] { leftFieldName, rightFieldName, ComparatorOrder.fromString(order) });
		} else {
			// expected format is "field order"
			String[] parts = comparatorString.split(" ");
			if (2 != parts.length) {
				throw new IOException(String.format(Locale.ROOT,
						"Invalid comparator expression %s - expecting 'field order'", comparatorString));
			}
			...
			return (StreamComparator) createInstance(comparatorType,
					new Class[] { String.class, ComparatorOrder.class },
					new Object[] { fieldName, ComparatorOrder.fromString(order) });
		}
	}
	...
	public StreamEqualitor constructEqualitor(String equalitorString, Class equalitorType) throws IOException {
		if (equalitorString.contains(",")) {
			...
			if (equalitorString.contains("=")) {
				...
				if (2 != parts.length) {
					throw new IOException(String.format(Locale.ROOT,
							"Invalid equalitor expression %s - expecting fieldName=fieldName", equalitorString));
				}
			...
			return (StreamEqualitor) createInstance(equalitorType, new Class[] { String.class, String.class },
					new Object[] { leftFieldName, rightFieldName });
		}
	}
	...
	public StreamOperation constructOperation(StreamExpression expression) throws IOException {
		...
		if (classSupplier != null) {
			...
			if (Expressible.class.isAssignableFrom(clazz) && StreamOperation.class.isAssignableFrom(clazz)) {
				return (StreamOperation) createInstance(clazz,
						new Class[] { StreamExpression.class, StreamFactory.class }, new Object[] { expression, this });
			}
		}

		throw new IOException(String.format(Locale.ROOT,
				"Invalid operation expression %s - function '%s' is unknown (not mapped to a valid StreamOperation)",
				expression, expression.getFunctionName()));
	}

	public org.apache.solr.client.solrj.io.eval.StreamEvaluator constructEvaluator(String expressionClause)
			throws IOException {
		return constructEvaluator(StreamExpressionParser.parse(expressionClause));
	}

	@SuppressWarnings({ "rawtypes" })
	public org.apache.solr.client.solrj.io.eval.StreamEvaluator constructEvaluator(StreamExpression expression)
			throws IOException {
		String function = expression.getFunctionName();
		Supplier<Class<? extends Expressible>> classSupplier = functionNames.get(function);
		...
		if (classSupplier != null) {
			...
			if (Expressible.class.isAssignableFrom(clazz) && StreamEvaluator.class.isAssignableFrom(clazz)) {
				return (org.apache.solr.client.solrj.io.eval.StreamEvaluator) createInstance(clazz,
						new Class[] { StreamExpression.class, StreamFactory.class }, new Object[] { expression, this });
			}
		}

		throw new IOException(String.format(Locale.ROOT,
				"Invalid evaluator expression %s - function '%s' is unknown (not mapped to a valid StreamEvaluator)",
				expression, expression.getFunctionName()));
	}
	...
	public <T> T createInstance(Class<T> clazz, Class<?>[] paramTypes, Object[] params) throws IOException {
		...
		try {
			...
			if (null != e.getMessage()) {
				throw new IOException(String.format(Locale.ROOT, "Unable to construct instance of %s caused by %s",
						clazz.getName(), e.getMessage()), e);
			} else {
				throw new IOException(String.format(Locale.ROOT, "Unable to construct instance of %s", clazz.getName()),
						e);
			}
		}
	}
	...
	public String getFunctionName(Class<? extends Expressible> clazz) throws IOException {
		...
		throw new IOException(
				String.format(Locale.ROOT, "Unable to find function name for class '%s'", clazz.getName()));
	}
