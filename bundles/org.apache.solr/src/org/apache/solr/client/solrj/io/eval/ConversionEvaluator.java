...
public class ConversionEvaluator extends RecursiveNumericEvaluator implements OneValueWorker {
	...
	enum LENGTH_CONSTANT {
		MILES, YARDS, FEET, INCHES, MILLIMETERS, CENTIMETERS, METERS, KILOMETERS
	};
	...
	public ConversionEvaluator(StreamExpression expression, StreamFactory factory) throws IOException {
		...
		if (3 != containedEvaluators.size()) {
			throw new IOException(
					String.format(Locale.ROOT, "Invalid expression %s - expecting exactly 3 parameters but found %d",
							super.toExpression(constructingFactory), containedEvaluators.size()));
		}

		if (containedEvaluators.subList(0, 2).stream()
				.anyMatch(item -> !(item instanceof RawValueEvaluator) && !(item instanceof FieldValueEvaluator))) {
			throw new IOException(
					String.format(Locale.ROOT, "Invalid expression %s - first two parameters must be strings",
							super.toExpression(constructingFactory)));
		}
		...
		try {
			...
			throw new IOException(String.format(Locale.ROOT,
					"Invalid expression %s - '%s' and '%s' are not both valid conversion types",
					super.toExpression(constructingFactory), fromString, toString));
		}
