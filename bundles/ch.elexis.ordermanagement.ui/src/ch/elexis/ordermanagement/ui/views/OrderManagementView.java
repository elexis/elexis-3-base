package ch.elexis.ordermanagement.ui.views;
...
import ch.elexis.ordermanagement.ui.util.OrderEntryComparators;
import ch.elexis.ordermanagement.ui.util.OrderManagementActionFactory;
import ch.elexis.ordermanagement.ui.util.OrderManagementHelper;
import ch.elexis.ordermanagement.ui.util.OrderManagementUtil;
import ch.elexis.ordermanagement.ui.util.TableSortController;
import ch.elexis.ordermanagement.ui.util.dnd.OrderDropReceiver;
import ch.elexis.ordermanagement.ui.views.provider.EntryTableLabelProvider;
import ch.elexis.ordermanagement.ui.views.provider.GenericOrderEditingSupport;
import ch.elexis.ordermanagement.ui.views.provider.GenericOrderEditingSupport.EditingColumnType;
import ch.elexis.ordermanagement.ui.views.provider.OrderTableLabelProvider;
import jakarta.inject.Inject;


public class OrderManagementView extends ViewPart implements IRefreshable {

	public static final String ID = "ch.elexis.ordermanagement.ui.views.OrderManagementView"; //$NON-NLS-1$
	...
	public void updateOrderEntry(IOrderEntry entry, int newValue) {
	    if (entry.getState() == OrderEntryState.OPEN) {
			...
			OrderCoreLogic.saveSingleDelivery(entry, newValue, orderService);
	    }
	...
	public void loadOpenOrders() {
		List<IOrder> orders = OrderCoreLogic.getOpenOrders();
	...
	public void loadCompletedOrders(Composite completedContainer) {
		...
		List<IOrder> orders = OrderCoreLogic.getCompletedOrders(showAllYears);
	    if (orders.isEmpty()) {
	        return;
	    }
	...
	public void updateOrderDetails(IOrder order) {
		...
		String createdStr = OrderCoreLogic.formatDate(order.getTimestamp());
		IOutputLog usierID = OrderCoreLogic.getOrderLogEntry(order);
	...
	private OrderHistorySummary getHistorySummary(IOrder order) {
		...
		IOutputLog logEntry = OrderCoreLogic.getOrderLogEntry(order);
	...
	public void setActOrder(IOrder selectedOrder) {
		...
		Display.getDefault().asyncExec(() -> {
			...
			boolean isDelivered = (actOrder != null) && OrderCoreLogic.isOrderCompletelyDelivered(actOrder);
