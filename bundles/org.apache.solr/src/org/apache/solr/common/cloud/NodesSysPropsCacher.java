 ...
 * based on the value define in
 * {@link org.apache.solr.common.cloud.ZkStateReader#DEFAULT_SHARD_PREFERENCES }
 * of {@link org.apache.solr.common.cloud.ZkStateReader#CLUSTER_PROPS }. If that
 * key does not present then this cacher will do nothing.
 *
	...
	public NodesSysPropsCacher(NodeStateProvider nodeStateProvider, String currentNode, ZkStateReader stateReader) {
		...
		stateReader.registerClusterPropertiesListener(properties -> {
			...
			if (shardPreferences.contains(ShardParams.SHARDS_PREFERENCE_NODE_WITH_SAME_SYSPROP)) {
				try {
					tags = PreferenceRule.from(shardPreferences).stream()
	...
	private void fetchSysProps(Set<String> newLiveNodes) {
		if (isRunning.get()) {
			...
			for (String node : newLiveNodes) {
				// this might takes some times to finish, therefore if there are a latter change
				// in listener
		...
		for (int i = 0; i < NUM_RETRY; i++) {
			...
			try {
				...
				try {
					...
					log.info("Exception on caching node:{} system.properties:{}, retry {}/{}", node, tags, i + 1,
							NUM_RETRY, e); // nowarn
					break;
				}
				log.info("Exception on caching node:{} system.properties:{}, retry {}/{}", node, tags, i + 1, NUM_RETRY,
						e); // nowarn
			}
		}
	}
