...
public class Consultations extends AbstractTimeSeries {
	...
	/**
	 * This value is the first birth cohort. The value is determined when iterating
	 * through all patients. The initial value is set to the actual year.
	 */
	...
	/**
	 * The size of this computation. Counted as the numbers of patients to be
	 * searched for data.
	 */
	private int size = 0;
	...
	/**
	 * This method executes the query and creates the content which is then
	 * accessible through getContent.
	 */
	@Override
	...
	/**
	 * This method queries the database and returns the results in an ArrayList with
	 * each entry in the list representing a cohort.
	 */
	...
	private void increment_patient_counter(Patient patient, int year) {
		...
		this.getList().get(
				year - this.firstCohort)[index] = ((Integer) this.getList().get(year - this.firstCohort)[index]) + 1;
	}

	private void add_male(int birthYear, Konsultation cons) {
		this.getList()
				.get(birthYear - this.firstCohort)[7] = (Integer) this.getList().get(birthYear - this.firstCohort)[7]
						+ ((Double) cons.getUmsatz()).intValue();

		this.getList().get(
				birthYear - this.firstCohort)[8] = (Integer) this.getList().get(birthYear - this.firstCohort)[8] + 1;
	}

	private void add_female(int birthYear, Konsultation cons) {
		this.getList()
				.get(birthYear - this.firstCohort)[2] = (Integer) this.getList().get(birthYear - this.firstCohort)[2]
						+ ((Double) cons.getUmsatz()).intValue();

		this.getList().get(
				birthYear - this.firstCohort)[3] = (Integer) this.getList().get(birthYear - this.firstCohort)[3] + 1;
	}
	...
	private void showValidationError(Patient patient) {
		String patientData = "Bei der Auswertung der Patientendaten ist ein "
	...
	/* -------------------------- meta data --------------------------------- */
	/*
	 * The following methods are getter and setter pairs which describe all fields
	 * which can be changed by the user. Each pair is annotated since all
