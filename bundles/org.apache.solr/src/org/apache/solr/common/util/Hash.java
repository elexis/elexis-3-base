 ...
 *
 * <p>
 * Development background: I was surprised to discovered that there isn't a good
 * cross-platform hash function defined for strings. MD5, SHA, FVN, etc, all
 * define hash functions over bytes, meaning that it's under-specified for
 * strings.
 * </p>
 *
 * <p>
 * So I set out to create a standard 32 bit string hash that would be well
 * defined for implementation in all languages, have very high performance, and
 * have very good hash properties such as distribution. After evaluating all the
 * options, I settled on using Bob Jenkins' lookup3 as a base. It's a well
 * studied and very fast hash function, and the hashword variant can work with
 * 32 bits at a time (perfect for hashing unicode code points). It's also even
 * faster on the latest JVMs which can translate pairs of shifts into native
 * rotate instructions.
 * </p>
 * <p>
 * The only problem with using lookup3 hashword is that it includes a length in
 * the initial value. This would suck some performance out since directly
 * hashing a UTF8 or UTF16 string (Java) would require a pre-scan to get the
 * actual number of unicode code points. The solution was to simply remove the
 * length factor, which is equivalent to biasing initVal by -(numCodePoints*4).
 * This slightly modified lookup3 I define as lookup3ycs.
 * </p>
 * <p>
 * So the definition of the cross-platform string hash lookup3ycs is as follows:
 * </p>
 * <p>
 * The hash value of a character sequence (a string) is defined to be the hash
 * of its unicode code points, according to lookup3 hashword, with the initval
 * biased by -(length*4).
 * </p>
 * <p>
 * So by definition
 ...
 * </pre>
 * <p>
 * An obvious advantage of this relationship is that you can use lookup3 if you
 * don't have an implementation of lookup3ycs.
 * </p>
 */
public class Hash {
	/**
	...
	/**
	 * Identical to lookup3, except initval is biased by -(length&lt;&lt;2). This is
	 * equivalent to leaving out the length factor in the initial state.
	...
	/**
	 * <p>
	 * The hash value of a character sequence is defined to be the hash of its
	 * unicode code points, according to
	 * {@link #lookup3ycs(int[] k, int offset, int length, int initval)}
	 * </p>
	 * <p>
	 * If you know the number of code points in the {@code CharSequence}, you can
	 * generate the same hash as the original lookup3 via
	 * {@code lookup3ycs(s, start, end, initval+(numCodePoints<<2))}
	 */
	 ...
	 * lookup3 hashlittle2 with initval biased by -(numCodePoints&lt;&lt;2). It is
	 * equivalent to lookup3ycs in that if the high bits of initval==0, then the low
	 * bits of the result will be the same as lookup3ycs.
	 * </p>
	 */
	...
	/**
	 * Returns the MurmurHash3_x86_32 hash of the UTF-8 bytes of the String without
	 * actually encoding the string to a temporary buffer. This is more than 2x
	...
	public static final long getLongLittleEndian(byte[] buf, int offset) {
		return ((long) buf[offset + 7] << 56) // no mask needed
				...
				| ((buf[offset + 4] & 0xffL) << 32) | ((buf[offset + 3] & 0xffL) << 24)
