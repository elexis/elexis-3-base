...
public class Importer extends ImporterPage {
	public static final String MY_LAB = "labtop";
	public static final String PLUGIN_ID = "ch.elexis.laborimport_labtop";
	...
	public Composite createPage(final Composite parent) {
		...
		File jar = LabtopSettings.getJarFile().orElse(null);
	...
	private Result<?> importDirect() {
		...
		String configuredDir = LabtopSettings.get(LabtopSettings.DL_DIR);
		File downloadDir = StringUtils.isBlank(configuredDir) ? CoreHub.getTempDir()
				: LabtopSettings.resolveLocalFile(configuredDir).orElse(null);
		String downloadDirPath = downloadDir != null ? downloadDir.getAbsolutePath() : null;
		String iniPath = LabtopSettings.getIniFile().map(File::getAbsolutePath).orElse(null);
	...
	private class LabImporter extends Composite {
		...
		public LabImporter(final Composite parent, final ImporterPage home) {
			...
			if (type == FILE) {
				...
				String filename = LocalConfigService.get(getFilenameKey(home), StringUtils.EMPTY);
			...
			SelectionAdapter sa = new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					...
					if (type == FILE) {
						...
						LocalConfigService.set(getTypeKey(home), FILE);
						LocalConfigService.set(getFilenameKey(home), filename);
					} else {
						bFile.setSelection(false);
						bDirect.setSelection(true);
						...
						LocalConfigService.set(getTypeKey(home), DIRECT);
						LocalConfigService.set(getFilenameKey(home), StringUtils.EMPTY);
					}
				}
			};
