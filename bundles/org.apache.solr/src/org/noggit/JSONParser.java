...
public class JSONParser {
	 ...
	 * exponent and with string length &lt;= 23 chars not including sign. This
	 * covers all representations of normal values for Double.toString().
	 */
	public static final int NUMBER = 3;
	/**
	 * Event indicating a JSON number value that was not produced by toString of any
	 * Java primitive numerics such as Double or Long. It is either an integer
	 * outside the range of a 64 bit signed integer, or a floating point value with
	 * a string representation of more than 23 chars.
	 */
	...
	/**
	 * ALLOW_EXTRA_COMMAS causes any number of extra commas in arrays and objects to
	 * be ignored Note that a trailing comma in [] would be [,] (hence calling the
	 * feature "trailing" commas is either limiting or misleading. Since trailing
	 * commas is fundamentally incompatible with any future
	 * "fill-in-missing-values-with-null", it was decided to extend this feature to
	 * handle any number of extra commas.
	 */
	...
	public static final int FLAGS_DEFAULT = ALLOW_COMMENTS | ALLOW_SINGLE_QUOTES
			| ALLOW_BACKSLASH_ESCAPING_ANY_CHARACTER | ALLOW_UNQUOTED_KEYS | ALLOW_UNQUOTED_STRING_VALUES
			| ALLOW_EXTRA_COMMAS;
	...
	protected int stringTerm; // The terminator for the last string we read: single quote, double quote, or 0
								// for unterminated.
	...
	private final void push() {
		if (ptr >= stack.length) {
			// doubling here is probably overkill, but anything that needs to double more
			// than
	...
	/**
	 * Returns true if the given character is considered to be whitespace. One
	 * difference between Java's Character.isWhitespace() is that this method
	...
	private static final long WS_MASK = (1L << ' ') | (1L << '\t') | (1L << '\r') | (1L << '\n') | (1L << '#')
			| (1L << '/') | (0x01); // set 1 bit so 0xA0 will be flagged as whitespace

	protected int getCharNWS() throws IOException {
		for (;;) {
			int ch = getChar();
			// getCharNWS is normally called in the context of expecting certain JSON
			// special characters
			// such as ":}"],"
			// all of these characters are below 64 (including comment chars '/' and '#', so
			// we can make this the fast path
			// even w/o checking the range first. We'll only get some false-positives while
			// using bare strings (chars "IJMc")
			if (((WS_MASK >> ch) & 0x01) == 0) {
				...
				getNewlineComment();
			} else if (!isWhitespace(ch)) { // we'll only reach here with certain bare strings, errors, or strange
											// whitespace like 0xa0
				return ch;
			}

			/***
			 * // getCharNWS is normally called in the context of expecting certain JSON
			 * special characters // such as ":}"]," // all of these characters are below 64
			 * (including comment chars '/' and '#', so we can make this the fast path if
			 * (ch < 64) { if (((WS_MASK >> ch) & 0x01) == 0) return ch; if (ch <= ' ')
			 * continue; // whitespace below a normal space if (ch=='/') {
			 * getSlashComment(); } else if (ch=='#') { getNewlineComment(); } } else if
			 * (!isWhitespace(ch)) { // check for higher whitespace like 0xA0 return ch; }
			 ***/

			/**
			 * older code switch (ch) { case ' ' : case '\t' : case '\r' : case '\n' :
			 * continue outer; case '#' : getNewlineComment(); continue outer; case '/' :
			 * getSlashComment(); continue outer; default: return ch; }
			 **/
		}
	}
	...
	protected void getNewlineComment() throws IOException {
		...
		for (;;) {
			int ch = getChar();
			// don't worry about DOS /r/n... we'll stop on the \r and let the rest of the
			// whitespace
			// eater consume the \n
			if (ch == '\n' || ch == '\r' || ch == -1) {
				return;
	...
	protected boolean matchBareWord(char[] arr) throws IOException {
		...
		// check that the string actually terminates... for example trueX should return
		// false
		int ch = getChar();
		if (eof) {
			return true;
	 ...
	 * firstChar should be the first numeric digit read.
	 */
	private long readNumber(int firstChar, boolean isNeg) throws IOException {
		...
		// the positive plane.
		long v = '0' - firstChar;
		// can't overflow a long in 18 decimal digits (i.e. 17 additional after the
		// first).
		// we also need 22 additional to handle double so we'll handle in 2 separate
		// loops.
		int i;
		for (i = 0; i < 17; i++) {
			int ch = getChar();
	...
	private char readEscapedChar() throws IOException {
		...
		case 'b':
			return '\b';
		case 'u':
			return (char) ((hexval(getChar()) << 12) | (hexval(getChar()) << 8) | (hexval(getChar()) << 4)
					| (hexval(getChar())));
		}
	...
	private CharArr readStringChars() throws IOException {
		if (stringTerm == 0) {
			// "out" will already contain the first part of the bare string, so don't reset
			// it
			readStringBare(out);
			return out;
		}
	...
	// this should be faster for strings with fewer escapes, but probably slower for
	// many escapes.
	...
	protected void handleNonDoubleQuoteString(int ch, boolean isName) throws IOException {
		if (ch == '\'') {
			...
			if (isName && (flags & ALLOW_UNQUOTED_KEYS) == 0 || !isName && (flags & ALLOW_UNQUOTED_STRING_VALUES) == 0
					|| eof) {
				if (isName) {
	...
	private static boolean isUnquotedStringChar(int ch) {
		...
		/***
		 * switch (ch) { case -1: case ' ': case '\t': case '\r': case '\n': case '}':
		 * case ']': case ',': case ':': case '=': // reserved for future use case '\\':
		 * // check for backslash should come after this function call return false; }
		 * return true;
		 ***/
	}

	/***
	 * alternate implementation // middle is the pointer to the middle of a buffer
	 * to start scanning for a non-string // character ('"' or "/").
	 * start<=middle<end private void readStringChars2a(CharArr arr, int middle)
	 * throws IOException { int ch=0; for(;;) { // find the next non-string char for
	 * (; middle<end; middle++) { ch = buf[middle]; if (ch=='"' || ch=='\\') break;
	 * }
	 *
	...
	private int next(int ch) throws IOException {
		// TODO: try my own form of indirect jump... look up char class and index
		// directly into handling implementation?
		for (;;) {
			switch (ch) {
	 ...
	 * characters. The underlying buffer of the returned <code>CharArr</code> should
	 * *not* be modified as it may be shared with the input buffer. The returned
	 * <code>CharArr</code> will only be valid up until the next JSONParser method
	 ...
	 * <p>
	 * The returned <code>CharArr</code> will only be valid up until the next
